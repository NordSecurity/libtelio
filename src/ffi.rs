...
use telio_model::{
    config::{Config, ConfigParseError, Peer},
    event::*,
    features::Features,
    mesh::{ExitNode, Node},
};
...
struct AdapterAdapter(Arc<dyn Adapter>);
...
impl Telio {
    ...
    pub fn start_custom(&self, private_key: SecretKey, adapter: Arc<dyn Adapter>) -> FfiResult<()> {
        let adapter: Arc<dyn telio_wg::Adapter> = Arc::new(AdapterAdapter(adapter));
