...
use self::types::*;
pub use telio_core::adapter::{
    CustomAdapter, TelioCustomAdapter, WgCmd, WgDevice, WgInterface, WgPeer, WgResponse,
};
use telio_core::device::{Device, DeviceConfig, Result as DevResult};
use telio_model::{
...
impl Telio {
    ...
    pub fn disable_tp_lite_stats_collection(&self) -> FfiResult<()> {
        telio_log_info!(
            "Telio::disable_tp_lite_stats_collection entry with instance id: {}.",
            self.id
        );
        catch_ffi_panic(|| {
            self.device_op(true, |dev| {
                dev.disable_tp_lite_stats_collection()
                    .log_result("Telio::enable_tp_lite_stats_collection")
            })
        })
    }
