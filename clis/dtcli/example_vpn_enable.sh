...
# This is a simple example script on how to use DTCLI to set up VPN.
...
# Keeping this in a separate variable so that a Mac OS compatible tunnel name could be added later.
TUNNEL_NAME="tun10"
...
# Starting WireGuard.
eval ${DTCLI_PATH} dev start boringtun ${TUNNEL_NAME}
...
# Connecting to VPN.
eval ${DTCLI_PATH} dev con ${recommended_server_public_key} ${recommended_server_ip}:51820

# Creating VPN route.
ip route add default dev ${TUNNEL_NAME} table 73110
ip route add ${recommended_server_ip} dev ${TUNNEL_NAME} table 73110
ip rule add priority 32111 not from all fwmark 11673110 lookup 73110
