...
use rust_cgi::{http::Method, http::StatusCode, text_response, Response};
...
use super::{
    constants::{MESHNET_LOG, PID_FILE, TELIOD_BIN, TELIOD_CFG, TELIOD_LOG},
    CgiRequest,
};
...
pub fn handle_api(request: &CgiRequest) -> Option<Response> {
    match (request.method(), request.route(), request.uri().query()) {
        (&Method::POST, "", _) => Some(start_daemon()),
        (&Method::DELETE, "", _) => Some(stop_daemon()),
        (&Method::PATCH, "", _) => {
        ...
        (&Method::GET, "", Some("info=get-status")) => Some(get_status()),
        (&Method::GET, "", Some("info=get-teliod-logs")) => Some(get_teliod_logs()),
        (&Method::GET, "", Some("info=get-meshnet-logs")) => Some(get_meshnet_logs()),
        _ => None,
    }
}
...
fn update_config(body: &str) -> Response {
    ...
    let updated_config = match serde_json::from_str::<TeliodDaemonConfigPartial>(body) {
        ...
        Err(e) => {
            ...
            return text_response(StatusCode::BAD_REQUEST, "Invalid JSON payload");
        }
    };
...
#[cfg(test)]
mod tests {
    use std::{fs, num::NonZeroU64, path::PathBuf, sync::Mutex};
    ...
    lazy_static! {
        static ref TEST_SEQ_MUTEX: Mutex<()> = Mutex::new(());
    }
