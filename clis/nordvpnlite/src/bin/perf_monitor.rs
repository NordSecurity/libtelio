...
fn read_proc_stat(pid: u32) -> io::Result<ProcessStats> {
    ...
    if parts.len() < 24 {
        return Err(io::Error::new(io::ErrorKind::InvalidData, "Invalid stat format"));
    }
...
fn calculate_cpu_usage(prev_stats: &ProcessStats, curr_stats: &ProcessStats, elapsed_ms: u64) -> f64 {
    let total_time_diff = (curr_stats.utime + curr_stats.stime) - (prev_stats.utime + prev_stats.stime);
    let clock_ticks_per_sec = 100.0; // Usually 100 on Linux
    let cpu_usage = (total_time_diff as f64 / clock_ticks_per_sec) / (elapsed_ms as f64 / 1000.0) * 100.0;
    cpu_usage
}
...
fn find_nordvpnlite_pid() -> io::Result<u32> {
    ...
    Err(io::Error::new(io::ErrorKind::NotFound, "nordvpnlite process not found"))
}
...
async fn collect_metrics(
    ...
    let metrics = PerformanceMetrics {
        ...
        context_switches: status_stats.voluntary_ctxt_switches + status_stats.nonvoluntary_ctxt_switches,
        open_fds,
    };
...
async fn main() -> io::Result<()> {
    ...
    // CSV header
    println!("timestamp,cpu_percent,rss_mb,vms_mb,rx_mbps,tx_mbps,rx_pps,tx_pps,ctx_switches,open_fds");
        ...
        match collect_metrics(pid, interface, &mut prev_stats, &mut prev_time).await {
            Ok(metrics) => {
                ...
                let (rx_mbps, tx_mbps, rx_pps, tx_pps) = if let Some(ref prev) = prev_net_stats {
                    let rx_mbps = ((metrics.network_rx_bytes - prev.rx_bytes) as f64 * 8.0) / 1_000_000.0;
                    let tx_mbps = ((metrics.network_tx_bytes - prev.tx_bytes) as f64 * 8.0) / 1_000_000.0;
