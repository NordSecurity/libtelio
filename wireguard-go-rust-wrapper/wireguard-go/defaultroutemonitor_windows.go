...
var last_default_iface_counter = 0
...
func bindSocketRoute(family winipcfg.AddressFamily, binder conn.BindSocketToInterface, ourLUID winipcfg.LUID, lastLUID *winipcfg.LUID, lastIndex *uint32, blackholeWhenLoop bool) error {
	...
	current_default_interface_counter := 0
	for i := range r {
		if r[i].DestinationPrefix.PrefixLength == 0 {
			current_default_interface_counter += 1
		}
	...
	if luid == *lastLUID && index == *lastIndex && last_default_iface_counter == current_default_interface_counter {
		return nil
	}
	*lastLUID = luid
	*lastIndex = index
	last_default_iface_counter = current_default_interface_counter
	blackhole := blackholeWhenLoop && index == 0
	if family == windows.AF_INET {
		infof("Binding v4 socket to interface %d (blackhole=%v)", index, blackhole)
		return binder.BindSocketToInterface4(index, blackhole)
	} else if family == windows.AF_INET6 {
		infof("Binding v6 socket to interface %d (blackhole=%v)", index, blackhole)
		return binder.BindSocketToInterface6(index, blackhole)
	}
	return nil
}

func monitorDefaultRoutes(family winipcfg.AddressFamily, binder conn.BindSocketToInterface, autoMTU bool, blackholeWhenLoop bool, tun *tun.NativeTun, watcher *interfaceWatcher) ([]winipcfg.ChangeCallback, uint32, error) {
	var minMTU uint32
	if family == windows.AF_INET {
		minMTU = 576
	} else if family == windows.AF_INET6 {
	...
	doIt := func() error {
		...
		if family == windows.AF_INET {
			watcher.default4 = lastIndex
		} else if family == windows.AF_INET6 {
			watcher.default6 = lastIndex
		}
	...
	if err != nil {
		errorf("Failed to register route change callback: %v", err)
		return nil, lastIndex, err
	}
	...
	return []winipcfg.ChangeCallback{cbr, cbi}, lastIndex, nil
}
