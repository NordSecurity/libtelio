...
func (s *StdNetBind) receiveIP(
	...
	if runtime.GOOS == "linux" || runtime.GOOS == "android" {
		if rxOffload {
			readAt := len(*msgs) - (conn.IdealBatchSize / udpSegmentMaxDatagrams)
...
func (s *StdNetBind) makeReceiveIPv4(pc *ipv4.PacketConn, con *net.UDPConn, rxOffload bool) conn.ReceiveFunc {
	return func(bufs [][]byte, sizes []int, eps []conn.Endpoint) (n int, err error) {
		return s.receiveIP(pc, con, rxOffload, bufs, sizes, eps)
	}
}

func (s *StdNetBind) makeReceiveIPv6(pc *ipv6.PacketConn, con *net.UDPConn, rxOffload bool) conn.ReceiveFunc {
	return func(bufs [][]byte, sizes []int, eps []conn.Endpoint) (n int, err error) {
		return s.receiveIP(pc, con, rxOffload, bufs, sizes, eps)
	}
}
...
func (s *StdNetBind) Send(bufs [][]byte, endpoint conn.Endpoint) error {
	s.mu.Lock()
