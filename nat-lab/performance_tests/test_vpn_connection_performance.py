...
from tests.utils import iperf3
...
async def collect_upload_metrics(
    ...
    output_unit : str, optional
        ...
        Example: "k" → Kbits/sec, "m" → Mbits/sec (default: "k").
    ...
    """
    server = iperf3.IperfServer(
        connection=server_connection,
        log_prefix="server",
        verbose=True,
        protocol=iperf3.Protocol.Tcp,
    )
    client = iperf3.IperfClient(
    ...
    upload_speed = client.get_speed(unit=f"{output_unit.upper()}bits")
    log.info("Upload Speed: %s %sbits/sec", upload_speed, output_unit.upper())
    ...
    return upload_speed, retransmits, min_rtt, max_rtt, mean_rtt
...
async def collect_download_metrics(
    ...
    async with server.run():
        ...
        async with client.run():
            await client.done()
    download_speed = client.get_speed(unit=f"{output_unit.upper()}bits")
    log.info("Download Speed: %s %sbits/sec", download_speed, output_unit.upper())
...
async def test_vpn_connection_performance() -> None:
    ...
    async with AsyncExitStack() as exit_stack:
        ...
        upload_speed, retransmits, min_rtt, max_rtt, mean_rtt = (
            await collect_upload_metrics(
                photo_album_connection, client_conn, output_unit="m"
            )
        )
        download_speed = await collect_download_metrics(
            photo_album_connection, client_conn, output_unit="m"
        )
        baseline_metrics = {
        ...
        upload_speed_vpn, retransmits_vpn, min_rtt_vpn, max_rtt_vpn, mean_rtt_vpn = (
            await collect_upload_metrics(
                photo_album_connection, client_conn, output_unit="m"
            )
        )
        download_speed_vpn = await collect_download_metrics(
            photo_album_connection, client_conn, output_unit="m"
        )
        vpn_metrics = {
            "upload_speed": upload_speed_vpn,
            "retransmits": retransmits_vpn,
            "min_rtt": min_rtt_vpn,
            "max_rtt": max_rtt_vpn,
            "mean_rtt": mean_rtt_vpn,
            "download_speed": download_speed_vpn,
        }
