...
async def test_default_direct_features() -> None:
    async with AsyncExitStack() as exit_stack:
        env = await setup_mesh_nodes(
            exit_stack,
            [SetupParameters(features=features(direct=feature_direct(providers=None)))],
        )
    ...
    async with AsyncExitStack() as exit_stack:
        env = await setup_mesh_nodes(
            ...
            [
                SetupParameters(
                    features=features(
                        direct=feature_direct(providers=ALL_DIRECT_FEATURES)
                    )
                )
            ],
        )
...
async def test_check_features_with_empty_direct_providers() -> None:
    async with AsyncExitStack() as exit_stack:
        env = await setup_mesh_nodes(
            ...
            [
                SetupParameters(
                    features=features(direct=feature_direct(providers=EMPTY_PROVIDER))
                )
            ],
        )
        started_tasks = env.clients[0].get_runtime().get_started_tasks()
