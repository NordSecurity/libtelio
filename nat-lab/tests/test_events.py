...
from telio import AdapterType, PathType, PeerInfo, State, Client, LinkState
from telio_features import TelioFeatures, Direct, NoLinkDetection
    ...
    [
        pytest.param(
            SetupParameters(
                ...
                features=TelioFeatures(no_link_detection=NoLinkDetection()),
            )
        ),
        pytest.param(
            SetupParameters(
...
async def test_event_link_state(
    alpha_setup_params: SetupParameters, beta_setup_params: SetupParameters
) -> None:
    async with AsyncExitStack() as exit_stack:
        env = await setup_mesh_nodes(
        ...
        beta_state = client_alpha.get_node_state(beta.public_key)
        assert beta_state is not None and beta_state.link_state == LinkState.Up
        assert beta_state == PeerInfo(
            identifier=beta.id,
            public_key=beta.public_key,
            state=State.Connected,
        ...
        # As far as beta node is up there should be no link_state event
        with pytest.raises(asyncio.TimeoutError):
            await client_alpha.wait_for_link_state_event(
                beta.public_key, [LinkState.Down], timeout=30
            )
        ...
        # Kill beta node and expect link_state down event
        asyncio.gather(
            client_beta.stop_device(),
            client_alpha.wait_for_link_state_event(
                beta.public_key, [LinkState.Down], timeout=30
            ),
        )

        # Restart the beta node and expect link_state up event
        asyncio.gather(
            client_beta.simple_start(),
            client_alpha.wait_for_link_state_event(
                beta.public_key, [LinkState.Up], timeout=10
            ),
        )
...
async def test_event_link_state_with_configured_rtt(
    alpha_setup_params: SetupParameters, beta_setup_params: SetupParameters
) -> None:
    async with AsyncExitStack() as exit_stack:
        env = await setup_mesh_nodes(
        ...
        # The event should be emitted in around 11 seconds
        await client_alpha.wait_for_link_state_event(
            beta.public_key, [LinkState.Down], timeout=13
        )
