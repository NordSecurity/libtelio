...
async def test_event_content_meshnet_selected_cipher_is_none(
    alpha_setup_params: SetupParameters, beta_setup_params: SetupParameters
) -> None:
    ...
    async with AsyncExitStack() as exit_stack:
        ...
        await client_alpha.vpn.connect_to_exit_node(
            beta.public_key,
            supported_ciphers=supported_ciphers,
        )

        logs = await client_alpha.log.get()
        print(logs)

        beta_node_state = client_alpha.get_node_state(beta.public_key)
        assert beta_node_state is not None
        assert beta_node_state.selected_cipher is None, (
            f"expected selected_cipher=None for meshnet exit peer "
            f"(supported_ciphers={supported_ciphers!r}), "
            f"got {beta_node_state.selected_cipher!r}"
        )
