import asyncio
import pytest
from natlab import asyncio_util, ping
...
async def test_mesh_plus_vpn_both_peers(
    ...
    await asyncio.gather(
        alpha_client.vpn.connect(
            str(wg_server["ipv4"]),
            int(wg_server["port"]),
            str(wg_server["public_key"]),
        ),
        beta_client.vpn.connect(
            str(wg_server["ipv4"]),
            int(wg_server["port"]),
            str(wg_server["public_key"]),
        ),
...
class TestThreeNode:
    ...
    async def test_vpn_plus_mesh_over_different_connection_types(
        ...
        await asyncio.gather(
            client_alpha.vpn.connect(
                str(wg_server["ipv4"]),
                int(wg_server["port"]),
                str(wg_server["public_key"]),
            ),
            client_beta.vpn.connect(
                str(wg_server["ipv4"]),
                int(wg_server["port"]),
                str(wg_server["public_key"]),
            ),
            client_gamma.vpn.connect(
                str(wg_server["ipv4"]),
                int(wg_server["port"]),
                str(wg_server["public_key"]),
            ),
