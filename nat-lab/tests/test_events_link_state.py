    ...
    async with AsyncExitStack() as exit_stack:
        ...
        with pytest.raises(asyncio.TimeoutError):
            await wait_for_any_with_timeout(
                [
                    asyncio.create_task(
                        client_alpha.events.wait_for_link_state(
                            beta.public_key, LinkState.DOWN
                        )
                    ),
                    asyncio.create_task(
                        client_beta.events.wait_for_link_state(
        ...
        assert client_alpha.events.get_link_state_events(beta.public_key) == [
            LinkState.DOWN,
            LinkState.UP,
        ]
        assert client_beta.events.get_link_state_events(alpha.public_key) == [
            LinkState.DOWN,
            LinkState.UP,
        ]
...
async def send_ping_and_wait_for_tx_after_rx_log(
    client, connection, ip_to_ping
) -> None:
    """
    This function assumes that no ICMP reply from the target will arrive in the same polling window of the ICMP request, which will trigger the tx_after_rx condition.
    ...
    ping_task = asyncio.create_task(ping(connection, ip_to_ping, None))
...
async def test_event_link_state_peer_goes_offline(
    ...
    async with AsyncExitStack() as exit_stack:
        ...
        await send_ping_and_wait_for_tx_after_rx_log(
            client_alpha, connection_alpha, beta.ip_addresses[0]
        )

        # Expect the link to still be UP for the duration of WG Keepalive timeout + Max. RTT Allowed
        with pytest.raises(asyncio.TimeoutError):
            await client_alpha.events.wait_for_link_state(
        ...
        await client_alpha.events.wait_for_link_state(beta.public_key, LinkState.DOWN)
...
async def test_event_link_state_feature_disabled(
    ...
    async with AsyncExitStack() as exit_stack:
        ...
        alpha_events = client_beta.events.get_link_state_events(alpha.public_key)
        beta_events = client_alpha.events.get_link_state_events(beta.public_key)
...
async def test_event_link_state_peer_doesnt_respond(
    ...
    async with AsyncExitStack() as exit_stack:
        ...
        async with ICMP_control(connection_beta):
            # Trigger TX-after-RX monitoring (ping will be dropped by beta but WG keepalives will maintain link)
            await send_ping_and_wait_for_tx_after_rx_log(
                client_alpha, connection_alpha, beta.ip_addresses[0]
            )
...
async def test_event_link_state_delayed_packet(
    ...
    async with AsyncExitStack() as exit_stack:
        ...
        await client_alpha.events.wait_for_link_state(beta.public_key, LinkState.UP)
...
async def test_event_link_detection_after_disabling_ethernet_adapter(
    ...
    async with AsyncExitStack() as exit_stack:
        ...
        await client_alpha.events.wait_for_event_on_any_derp([RelayState.CONNECTED])
...
async def test_event_link_detection_after_disabling_ethernet_adapter_with_vpn(
    ...
    async with AsyncExitStack() as exit_stack:
        ...
        # Trigger TX-after-RX monitoring (ping should fail since adapter is disabled)
        await send_ping_and_wait_for_tx_after_rx_log(
            client_alpha, connection_alpha, config.PHOTO_ALBUM_IP
        )
        ...
        assert client_alpha.events.get_link_state_events(vpn_public_key) == [
            LinkState.DOWN,
            LinkState.UP,
            LinkState.DOWN,
        ]
