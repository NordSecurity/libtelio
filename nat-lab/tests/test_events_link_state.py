...
# As observed, a 50% tolerance is more than enough to detect down link state, with or without ED, whether on personal setup or on CI.
...
async def test_event_link_detection_after_disabling_ethernet_adapter_direct_path(
    ...
    if setup_params[0].connection_tag is ConnectionTag.VM_WINDOWS_1:
        pytest.xfail("Expected failure on Windows, see LLT-5073")
    async with AsyncExitStack() as exit_stack:
        env = await setup_mesh_nodes(exit_stack, setup_params)
        alpha, beta = env.nodes
        client_alpha, client_beta = env.clients
...
async def test_event_link_detection_after_disabling_ethernet_adapter_with_vpn(
    ...
    if alpha_setup_params[0].connection_tag is ConnectionTag.VM_WINDOWS_1:
        pytest.xfail("Expected failure on Windows, see LLT-5073")
    async with AsyncExitStack() as exit_stack:
