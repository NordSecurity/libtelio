...
class Interface:
    ...
    async def is_enabled(self, connection: Connection) -> InterfaceState:
    ...
    async def get_enabled_network_interfaces(
        ...
        for match in matches:
            ...
            if interface:
                ...
                if interface.state is InterfaceState.Enabled and not ipv4_address:
                    raise ValueError(
                        f"Enabled interface without address assigned: {interface}, {ipv4_address}"
                    )
    ...
    async def disable(self, connection: Connection) -> None:
        ...
        if self.is_enabled(connection):
            raise Exception("Tried to disable but system still reports it as enabled.")
