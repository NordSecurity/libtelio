...
def parse_input(input_string, container_name: Optional[str] = None) -> ConntrackerEvent:
    ...
    if container_name:
        log.info("[%s] Conntracker reported event: %s", container_name, input_string)
    else:
        log.info("Conntracker reported event: %s", input_string)
...
class ConnectionTracker:
    ...
    async def on_stdout(self, stdout: str) -> None:
        ...
        for line in stdout.splitlines():
            event = parse_input(line, self._connection.target_name())
