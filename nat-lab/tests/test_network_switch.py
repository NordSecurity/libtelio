...
Configuration for interface "Loopback Pseudo-Interface 1"
    ...
    InterfaceMetric:                      75"""
        ...
        assert await ifc.is_enabled(mock_connection) is InterfaceState.Disabled
        assert ifc.ipv4 is None

        ifc = WinInterface.find_interface_by_name(interfaces, "Ethernet Instance 0 2")
        assert ifc, interfaces
        assert await ifc.is_enabled(mock_connection) is InterfaceState.Enabled
...
class TestNetworkSwitcherWindows:
    ...
    async def test_create_missing_interface(self, interfaces):
        mock_connection = Mock()
        strerr = f"Couldn't find {'secondary' if interfaces[0].name == 'Primary' else 'primary'}"
        with patch.object(
...
Configuration for interface "Loopback Pseudo-Interface 1"
    ...
    InterfaceMetric:                      75"""
        ...
        with pytest.raises(
            ValueError, match="Enabled interface without address assigned:"
        ):
            await NetworkSwitcherWindows.create(mock_connection)
