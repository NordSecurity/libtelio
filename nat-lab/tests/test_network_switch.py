...
class TestNetworkSwitcherWindows:
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                [Interface("Primary", True, "10.55.0.10")],
            ),
            pytest.param(
                [Interface("Secondary", True, "10.66.0.13")],
            ),
    ...
    async def test_create_missing_interface(self, interfaces):
        mock_connection = Mock()
        strerr = f"Couldn't find {'secondary' if interfaces[0].name == 'Primary' else 'primary'} .* is it enabled?"
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                [
                    Interface("Primary", False, "10.55.0.10"),
                    Interface("Secondary", True, "10.66.0.13"),
                ],
            ),
            pytest.param(
                [
                    Interface("Primary", True, "10.55.0.10"),
                    Interface("Secondary", False, "10.66.0.13"),
                ],
            ),
            pytest.param(
                [
            ...
            pytest.param(
                [
                    Interface("Primary", True, "10.55.0.10"),
                    Interface("Secondary", True, ""),
                ],
            ),
        ],
    )
    async def test_create_with_invalid_interfaces(self, interfaces):
        mock_connection = Mock()
        missing_strerr = f"Couldn't find {'primary' if not interfaces[0].enabled else 'secondary'} .* is it enabled?"
        with patch.object(Interface, "get_network_interfaces", return_value=interfaces):
            if any(interface.enabled is False for interface in interfaces):
                with pytest.raises(AssertionError, match=missing_strerr):
                    await NetworkSwitcherWindows.create(mock_connection)
            else:
                with pytest.raises(
                    ValueError, match="Interface enabled but IPv4 is unassigned"
                ):
                    await NetworkSwitcherWindows.create(mock_connection)
    ...
    async def test_switch_network_failure(self):
        ...
        mock_primary = Mock(spec=Interface("Ethernet Instance 0", True))
        mock_primary.delete_route = AsyncMock()
