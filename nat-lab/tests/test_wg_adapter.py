...
from tests.config import WINDOWS_NETWORK_ADAPTER_REGISTRY_KEY, UNIFFI_PATH_WINDOWS_VM
...
async def test_wg_adapter_cleanup_loop_dirty_shutdown(
    ...
    for i in range(iterations):
        ...
        async with AsyncExitStack() as exit_stack:
            ...
            assert ORPHANED_ADAPTER_REMOVED_LOG in await client.log.get()
...
async def restore_dll_after_first_failed_attempt(
    ...
    async with asyncio.timeout(FIRST_ADAPTER_CREATION_ATTEMPT_TIMEOUT_S):
        ...
        await client.log.wait_for(ADAPTER_CREATION_RETRY_LOG)
...
async def test_wg_adapter_creation_retry(conn_tag: ConnectionTag) -> None:
    ...
    async with AsyncExitStack() as exit_stack:
        ...
        await save_logs(conn, name_suffix=FIRST_START_LOG_SUFFIX)
