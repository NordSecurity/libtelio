...
class TestDnsWhitelisting:
    ...
    async def test_dns_whitelisting_reblock_is_effective(
        ...
        changes: the firewall only ever sees queries that miss the cache, so as
        long as a whitelisted answer stays cached the domain remains reachable
        no matter what the firewall chain says.
        ...
        derives a negative_ttl of min(ttl, minimum) = 300 and caches the NXDOMAIN
        for five minutes. Such a query would poison the cache and make step 2 fail,
