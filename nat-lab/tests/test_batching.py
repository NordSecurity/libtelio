...
def _generate_setup_parameters(
    ...
    endpoint_providers: List[EndpointProvider],
    direct_enabled: bool,
    batching_enabled: bool,
) -> SetupParameters:
    if direct_enabled:
        features = features_with_endpoint_providers(endpoint_providers)
    else:
        features = default_features()
    features.wireguard.persistent_keepalive = FeaturePersistentKeepalive(
        vpn=70, direct=70, proxying=70, stun=70
    )
    features.link_detection = FeatureLinkDetection(
        rtt_seconds=1, no_of_pings=1, use_for_downgrade=True
    )
    if batching_enabled:
        features.batching = FeatureBatching(direct_connection_threshold=35)
