...
async def test_vpn_dns(alpha_ip_stack: IPStack) -> None:
    async with AsyncExitStack() as exit_stack:
        ...
        await client_alpha.enable_magic_dns(["10.0.80.82"])
        await client_alpha.set_meshnet_config(api.get_meshnet_config(alpha.id, derp_servers=[]))
...
async def test_dns_change_nickname() -> None:
    async with AsyncExitStack() as exit_stack:
        ...
        # Set new meshnet_config with different nicknames
        api.assign_nickname(alpha.id, "rotten")
        api.assign_nickname(beta.id, "ono")
        await client_alpha.set_meshnet_config(api.get_meshnet_config(alpha.id, derp_servers=[]))
        await client_beta.set_meshnet_config(api.get_meshnet_config(beta.id, derp_servers=[]))
        ...
        # Set new meshnet_config removing nicknames
