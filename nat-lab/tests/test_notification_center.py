...
basic_authentication_credentials = f"{CREDENTIALS_CONFIG['core_api_credentials']['username']}:{CREDENTIALS_CONFIG['core_api_credentials']['password']}"
...
async def run_mqtt_listener(
    ...
    return await wait_for_mqtt_executing(mqtt_process)


def wait_with_timeout(max_wait=5, initial_wait_time=0.1, factor=2, on_timeout=None):
    def decorator(func):
        @wraps(func)
        async def wrapper(*args, **kwargs):
            total_wait_time = 0
            wait_interval = initial_wait_time
            ...
            while total_wait_time < max_wait:
                ...
                wait_interval = min(wait_interval * factor, max_wait - total_wait_time)
...
@wait_with_timeout(
    on_timeout="No stdout received from MQTT process within max wait time."
)
async def wait_for_mqtt_output_condition(mqtt_process, target_string):
    stdout = mqtt_process.get_stdout()
    if target_string in stdout:
        return stdout
    return None


@wait_with_timeout(on_timeout="MQTT process didn't start within max wait time.")
async def wait_for_mqtt_executing_condition(mqtt_process):
    if mqtt_process.is_executing():
        return mqtt_process
    return None


async def wait_for_mqtt_output(mqtt_process, target_string, **kwargs):
    return await wait_for_mqtt_output_condition(mqtt_process, target_string, **kwargs)


async def wait_for_mqtt_executing(mqtt_process, **kwargs):
    return await wait_for_mqtt_executing_condition(mqtt_process, **kwargs)


async def get_mqtt_broker_credentials(connection):
    endpoint = f"{CORE_API_URL}/v1/notifications/token"
    json_data = {
        "user_hash": "user-hash",
...
async def test_nc_register():
    async with AsyncExitStack() as exit_stack:
        ...
        mqtt_payload = await wait_for_mqtt_output(mqtt_process, "message_id")
        assert mqtt_payload
