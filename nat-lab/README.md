...
- Windows captures use the in-box `pktmon` tool with `--comp all` (full packet size, all adapters including virtual ones — `--comp nics` only sees physical NICs); each .etl log is converted to pcapng on the VM before download (see [python.PktmonCapture](tests/utils/tcpdump.py)). pktmon only enumerates adapters when a session starts, so to capture libtelio's wintun/wireguard tunnel adapter (created later) we poll for new adapters and roll the session when one appears — this splits the capture into numbered pcap segments (`<guest>.pcap`, `<guest>-2.pcap`, ...), the later of which contain the decrypted tunnel traffic (best-effort: depends on the tunnel adapter living long enough for the roll to fire). After download each segment is normalised on the test runner by [python.rewrite_windows_pcap()](tests/utils/tcpdump.py): pktmon emits the tunnel adapter's bare-IP packets under an Ethernet-typed interface, so they are wrapped in a synthetic Ethernet header (otherwise Wireshark renders them as malformed), and the SSH control channel (unfilterable at capture time) is dropped. Best-effort: the original is kept if normalisation fails.
