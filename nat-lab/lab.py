...
# Auto-generated by lab.py, do not edit or commit.
# Docker >= 28.2 adds per-container raw/prerouting rules dropping packets
# addressed directly to container IPs ("direct routing" hardening). With the
# br_netfilter module loaded these rules also match bridged frames forwarded
# by natlab's gateway containers, cutting them off from the 'internet'
# network. 'nat-unprotected' keeps NAT behaviour but skips those drop rules.
#
# Without br_netfilter the bridged gateway traffic bypasses iptables entirely,
# so the override is unnecessary - and harmful: clients keep a default route
# via the docker host bridge, and 'nat-unprotected' would let the host forward
# their packets straight into 'internet', bypassing the gateway containers
# (breaks link-state tests that cut the gateway path and expect silence).
networks:
  internet:
    driver_opts:
...
def docker_server_version() -> Tuple[int, int]:
    """Return the docker engine (server) version as a (major, minor) tuple."""
    output = run_command_with_output(
        ["docker", "version", "--format", "{{.Server.Version}}"], hide_output=True
    ).strip()
    match = re.match(r"(\d+)\.(\d+)", output)
    if not match:
        raise RuntimeError(f"Cannot parse docker server version: {output!r}")
    return int(match.group(1)), int(match.group(2))


def br_netfilter_active() -> bool:
    """Return True if br_netfilter is loaded and hooking bridged frames into
    iptables. The sysctl file only exists while the module is loaded, and the
    hook can be disabled with the module still loaded (value 0)."""
    try:
        with open(BRIDGE_NF_CALL_IPTABLES, "r", encoding="utf-8") as file:
            return file.read().strip() == "1"
    except FileNotFoundError:
        return False


def configure_direct_routing_override():
    """Create or remove the compose override opting the 'internet' network out
    of Docker's direct-routing hardening (see DIRECT_ROUTING_OVERRIDE).

    Compose merges docker-compose.override.yml automatically for every command.
    The 'nat-unprotected' gateway mode only exists since Docker 28.0 and older
    engines reject unknown values, so the override is gated on the engine
    version and on br_netfilter actually diverting bridged frames to iptables
    (the state is snapshotted here - reload the environment if it changes).
    """
    if docker_server_version() >= (28, 0) and br_netfilter_active():
        print("br_netfilter active: applying 'nat-unprotected' compose override")
        with open(DIRECT_ROUTING_OVERRIDE_FILE, "w", encoding="utf-8") as file:
            file.write(DIRECT_ROUTING_OVERRIDE)
    else:
        try:
            os.remove(DIRECT_ROUTING_OVERRIDE_FILE)
        except FileNotFoundError:
            pass
...
def start(
    ...
    configure_direct_routing_override()
...
def shard_args():
    ...
    `lab.py recreate` starting everything.
    """
