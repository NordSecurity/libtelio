...
class NotificationTokenRequest:
    ...
    app_user_id = str
    platform_id = int
    pns_id: Optional[int] = None
    pns_token: Optional[str] = None
...
class CoreApiHandler(BaseHTTPRequestHandler):
    def __init__(self, request, client_address, server: CoreServer):
        ...
        self.notifications_path = "/v1/notifications/token"
    ...
    def validate_bearer_token(self):
        ...
        if (
            username != CREDENTIALS_CONFIG["core_api_credentials"]["username"]
            or token != CREDENTIALS_CONFIG["core_api_credentials"]["password"]
        ):
            self._send_error_response(
    ...
    def validate_basic_authorization(self):
        ...
        if (
            CREDENTIALS_CONFIG["core_api_credentials"]["username"] == username
            and CREDENTIALS_CONFIG["core_api_credentials"]["password"] == token
        ):
            return True
    ...
    def handle_get_notifications_token(self):
        ...
        response = {
            ...
            "username": CREDENTIALS_CONFIG["mqtt_credentials"]["username"],
            "password": CREDENTIALS_CONFIG["mqtt_credentials"]["password"],
            "expires_in": 60,
        }
