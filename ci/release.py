...
# Accepted release tags: a stable vX.Y.Z or a pre-release vX.Y.Z-rcN
...
def is_stable_tag(tag: str) -> bool:
    """True for a stable release (v8.1.0), False for a pre-release (v8.0.0-rc2)."""
...
def open_cleanup_pr(args, token, source_branch, tmp) -> None:
    """PR on source_branch: remove used .unreleased/ files (+ Cargo bump on stable tags)."""
...
def main():
    ...
    print(
        ...
        f"({'stable' if is_stable_tag(args.tag) else 'pre-release'})"
    )
