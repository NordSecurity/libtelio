...
1. **Generate the changelog** for the tag from the `.unreleased/` files at the tagged commit
   (via `ci/generate_changelog.py`) and insert it at the top of the changelog
...
- `--out-file` is the changelog to insert into (the block is prepended to the top, so the
