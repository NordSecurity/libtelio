...
fn bench_request_crypto(c: &mut Criterion) {
    ...
    for size in PACKET_SIZES {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(size), size, |b, &size| {
            ...
            b.iter(|| {
                // Benchmark request encryption/decryption
                let encrypted = encrypt_request(&data, &mut rand::thread_rng(), &secret1, &public2).unwrap();
                let (decrypted, _) = decrypt_request(&encrypted, &secret2, |pk| pk == &public1).unwrap();
                black_box(decrypted)
            });
        });
    }
...
fn bench_response_crypto(c: &mut Criterion) {
    ...
    for size in PACKET_SIZES {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(size), size, |b, &size| {
            ...
            b.iter(|| {
                // Benchmark response encryption/decryption
                let encrypted = encrypt_response(&data, &mut rand::thread_rng(), &secret1, &public2).unwrap();
