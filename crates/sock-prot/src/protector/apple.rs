...
extern "C" {
    ...
    fn nw_path_enumerate_interfaces(path: nw_path_t, enumerate_block: *const c_void);
}

const DISPATCH_QUEUE_PRIORITY_BACKGROUND: c_long = -1 << 15;
...
fn bind_to_tun(sock: NativeSocket, tunnel_interface: u64) -> io::Result<()> {
    log::debug!(
...
fn bind(interface_index: u32, socket: i32) -> io::Result<()> {
    if let Some(sock_addr) = getsockname::<SockaddrStorage>(socket)?.family() {
        ...
        log::warn!("Failed to find sock addr for socket : {}", socket);
    }
...
impl NativeProtector {
    ...
    /// # Safety
    /// The interface index must be valid
    pub unsafe fn set_tunnel_interface(&self, interface: u64) {
    ...
    /// # Safety
    /// The interface file descriptor must be valid
    pub unsafe fn set_tunnel_from_fd(&self, tun: RawFd) -> io::Result<()> {
        self.set_tunnel_interface(interface_index_from_tun(tun)?);
        Ok(())
    }

    pub fn set_tunnel_from_name(&self, name: &str) -> io::Result<()> {
        unsafe { self.set_tunnel_interface(interface_index_from_name(name)?) }
        Ok(())
    }
}
...
impl Protector for NativeProtector {
    ...
    unsafe fn make_internal(&self, socket: NativeSocket) -> io::Result<()> {
...
impl Sockets {
    ...
    fn rebind_all(&mut self, force: bool) {
        log::debug!("Rebinding all relay sockets, force: {}", force);
    ...
    fn rebind(&mut self, sock: i32, force: bool) {
        log::debug!("Rebinding socket {}, force: {}", sock, force);
        ...
        async move {
            loop {
                match notify.recv().await {
                    ...
                    Err(e) => {
                        log::warn!("Failed to receive new path for sockets ({sockets:?}): {e}");
                    }
                }
            }
        }
...
fn setup_network_path_monitor() {
    ...
    unsafe {
        ...
        if monitor.is_null() {
            log::warn!("Failed to start network path monitor");
            return;
        }

        let queue = dispatch::ffi::dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0);
        if queue.is_null() {
            log::warn!("Failed to get global background queue");
            return;
        }
...
fn interface_index_from_name(name: &str) -> io::Result<u64> {
    use std::ffi::CString;
...
unsafe fn interface_index_from_tun(tun_fd: RawFd) -> io::Result<u64> {
    let index = unsafe {
...
mod tests {
    ...
    fn test_ipv6_sk_bind() {
        ...
        bind(get_primary_interface(0).unwrap() as u32, socket.as_raw_fd()).expect("Test failed");
    }
}
