...
pub struct TUNHealthMonitor {
    ...
    /// Max UAPI poll gap expected to detect libtelio(device) suspension.
...
impl State {
    ...
    async fn uapi_request(&mut self, cmd: &Cmd) -> Result<Response, Error> {
        ...
        telio_log_debug!(
            "UAPI request: {}, response: {:?}, TUN status: {:?}",
            format!("{}", &cmd).trim_end(),
            &ret,
            status
        );
...
pub mod tests {
    ...
    async fn test_wg_health_monitor() {
        ...
        let feature_health = FeatureInterfaceHealth::default();
        let mut monitor = TUNHealthMonitor::new(&feature_health);

        // Healthy (no failure)
        assert_eq!(monitor.update(&ok_resp), TUNHealthStatus::Healthy);

        // Healthy (first failure is considered transient)
        tokio::time::advance(Duration::from_secs(1)).await;
        assert_eq!(monitor.update(&err_resp), TUNHealthStatus::Healthy);
        ...
        tokio::time::advance(Duration::from_secs(1)).await;
        assert_eq!(monitor.update(&err_resp), TUNHealthStatus::InterfaceGone);
