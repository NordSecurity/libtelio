...
use telio_model::{
    ...
    features::{FeatureBatching, FeatureInterfaceHealth, FeatureLinkDetection},
...
/// Used to detect tunnel interface problems. Filters out transient
/// errors that happen with WireguardNT around power-transitions
pub struct TUNHealthMonitor {
    /// Minimum time separation between UAPI failures required to classify
    /// a failure as non-transient
    transient_failure_separation_threshold: Duration,
    ...
    uapi_poll_suspension_threshold: Duration,
...
impl TUNHealthMonitor {
    ...
    fn update(&mut self, resp: &Response) -> TUNHealthStatus {
        ...
        // out those errors or else we will exceed transient_failure_separation_threshold
        // and produce incorrect status after power-resume.
        let suspend_detected = prev_update
            .is_none_or(|t| now.duration_since(t) >= self.uapi_poll_suspension_threshold);
        ...
        match self.first_failure_after_success {
            Some(t) => {
                if now.duration_since(t) >= self.transient_failure_separation_threshold
...
impl DynamicWg {
    ...
    ///         },
    ///         None,
    ///         TUNHealthMonitor::new(Duration::from_secs(10), Duration::from_secs(5)),
...
pub mod tests {
    ...
    const INTERFACE_GONE_ERROR_THRESHOLD_S: u64 = 10;
    const INTERFACE_GONE_MAX_UAPI_INERVAL_S: u64 = 60;
    ...
    pub async fn setup(#[cfg(all(not(test), feature = "test-adapter"))] cfg: Config) -> Env {
        ...
        let health_monitor = TUNHealthMonitor::new(
            Duration::from_secs(INTERFACE_GONE_ERROR_THRESHOLD_S),
            Duration::from_secs(INTERFACE_GONE_MAX_UAPI_INERVAL_S),
        );
    ...
    async fn health_monitor_interface_gone() {
        ...
        for _ in 0..INTERFACE_GONE_ERROR_THRESHOLD_S {
    ...
    async fn health_monitor_healthy() {
        let health_monitor = TUNHealthMonitor::new(
            Duration::from_secs(INTERFACE_GONE_ERROR_THRESHOLD_S.into()),
        ...
        for _ in 0..INTERFACE_GONE_ERROR_THRESHOLD_S * 2 {
            advance_time_and_yield(Duration::from_secs(1)).await;
        }
    ...
    async fn test_wg_health_monitor() {
        ...
        let mut tracker = TUNHealthMonitor::new(
            Duration::from_secs(INTERFACE_GONE_ERROR_THRESHOLD_S),
            Duration::from_secs(INTERFACE_GONE_MAX_UAPI_INERVAL_S),
        );
        ...
        tokio::time::advance(Duration::from_secs(INTERFACE_GONE_MAX_UAPI_INERVAL_S)).await;
        assert_eq!(tracker.update(&err_resp), TUNHealthStatus::Healthy);

        // Healthy (failures within transient failure window)
        for _ in 1..INTERFACE_GONE_ERROR_THRESHOLD_S {
            tokio::time::advance(Duration::from_secs(1)).await;
            assert_eq!(tracker.update(&err_resp), TUNHealthStatus::Healthy);
        }
