...
const CIPHERTEXT_LEN: u32 = KYBER_CIPHERTEXTBYTES as _;
...
pub async fn fetch_keys(
    ...
    let Keypair {
        public: pq_public,
        secret: pq_secret,
    } = keypair(&mut rand_core_compat::rand_core_0_6::OsRng);
    ...
    let pq_shared = decapsulate(&ciphertext, &pq_secret).map_err(|err| {
        super::Error::Generic(format!("Failed to decapsulate PQ shared secret: {err:?}"))
    })?;
    let pq_shared = telio_crypto::PresharedKey::new(pq_shared);
...
) -> super::Result<[u8; CIPHERTEXT_LEN as usize]> {
...
fn push_get_method_udp_payload_without_auth_tag(
    ...
    pkgbuf.extend_from_slice(&(pq_public.len() as u32).to_le_bytes());
    pkgbuf.extend_from_slice(pq_public);
}
...
mod tests {
    ...
    use base64::prelude::*;
    use safe_pqc_kyber::decapsulate;

    #[test]
    // Round-3 Kyber768 known-answer test. This pins wire compatibility of the KEM backend:
    // the vector was produced by the previous pqcrypto-kyber 0.7.6 implementation, so it also
    // guards against a future crate bump silently switching to an incompatible (e.g. FIPS-203
    // ML-KEM) algorithm.
    fn decapsulate_kyber_ciphertext() {
        ...
        let shared = {
            ...
            let shared = decapsulate(&ct_bytes, &sk_bytes).unwrap();

            BASE64_STANDARD.encode(shared)
        };
