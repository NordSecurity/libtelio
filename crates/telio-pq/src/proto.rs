...
mod tests {
    ...
    fn parse_get_response() {
        ...
        let cipher = super::parse_get_response(testpkg, 62535).unwrap();
    ...
    fn parset_get_response_invalid() {
        ...
        assert!(super::parse_get_response(testpkg, 62535).is_err());
    }
    ...
    #[tokio::test]
    async fn silent_server_behavior_fetch_keys() {
        tokio::time::pause();
        ...
        // no server is running so expect timeout
        let res = super::fetch_keys(
            &pool,
            "10.5.0.13:7891", // meaningless IP and port
            &SecretKey::gen(),
            &SecretKey::gen().public(),
        )
        .await;

        assert!(matches!(res, Err(crate::Error::Timeout)));
    }

    #[tokio::test]
    async fn silent_server_behavior_rekey() {
        tokio::time::pause();
        ...
        // no server is running so expect timeout
        let res = super::rekey(&pool, &pqcrypto_kyber::kyber768_keypair().1).await;
