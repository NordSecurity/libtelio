...
mod tests {
    ...
    async fn test_handle_mapped_unicast_packet() {
        ...
        let mapped_addr = SocketAddr::V4(SocketAddrV4::new(src_ip, src_port));

        let packet = make_udp_v4(&transport_addr.to_string(), &mapped_addr.to_string());
        scaffold.channel.tx.send(packet.clone()).await.unwrap();
        ...
        assert_eq!(ip_packet.get_source(), transport_addr.ip());
        assert_eq!(ip_packet.get_destination(), peer1_addr.ip());

        let udp_packet = UdpPacket::new(&buffer[IPV4_HEADER_MIN_LENGTH..]).unwrap();
        assert_eq!(udp_packet.get_source(), transport_addr.port());
        assert_eq!(udp_packet.get_destination(), peer1_addr.port());
