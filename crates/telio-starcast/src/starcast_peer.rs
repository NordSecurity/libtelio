...
impl StarcastPeer {
    ...
    pub async fn start(channel: Chan<Vec<u8>>, allowed_ips: Vec<IpNetwork>) -> Result<Self, Error> {
        ...
        const TIMER_UPDATE_PERIOD: Duration = Duration::from_millis(25);
        ...
        Ok(StarcastPeer {
            task: Task::start(State {
                ...
                sending_buffer: vec![0u8; MAX_PACKET],
                receiving_buffer: vec![0u8; MAX_PACKET],
                allowed_ips,
            }),
        })
    }

    // TODO figure out the implications and possible side effects of reconfiguring the starcast peer mid operation.
                ...
                tokio::select! {
                    ...
                    _ = self.wg_timer_tick_interval.tick() => {
                        match peer.update_timers(&mut self.sending_buffer) {
                            ...
                            _ => telio_log_warn!("[Starcast] Update Tunn timers returned unexpected result"),
                        }
                    },
                    // Outbound packets
                    Some(ip_packet) = rx.recv() => {
                        match peer.encapsulate(&ip_packet, &mut self.sending_buffer) {
                            ...
                            _ => {
                                telio_log_warn!("[Starcast] Encapsulate packet returned unexpected result");
                            },
                        }
                    },
...
mod tests {
    ...
    use pnet_packet::{
        ip::IpNextHeaderProtocols, ipv4::MutableIpv4Packet, udp::MutableUdpPacket, Packet,
    };
    use std::net::{IpAddr, Ipv4Addr, Ipv6Addr};
    use telio_test::await_timeout;

    fn create_udp_packet(source_port: u16, destination_port: u16) -> Vec<u8> {
        // Virtual peer, so all communication happens on loopback.
        let source_ip = Ipv4Addr::new(127, 0, 0, 1);
        let destination_ip = Ipv4Addr::new(127, 0, 0, 1);
        let payload = b"Hello, UDP!";

        // Create a buffer for the IPv4 packet.
        let mut ipv4_packet_buf = vec![0u8; 20 + 8 + payload.len()]; // IPv4 header + UDP header + payload.
        let mut ipv4_packet = MutableIpv4Packet::new(&mut ipv4_packet_buf).unwrap();
        ...
        let mut udp_packet_buf = vec![0u8; 8 + payload.len()]; // UDP header + payload.
        let mut udp_packet = MutableUdpPacket::new(&mut udp_packet_buf).unwrap();
        ...
        // For simplicity, checksum is set to zero. In real applications, you should compute it.
        udp_packet.set_checksum(0);

        // Set the payload.
        udp_packet.set_payload(payload);

        // Insert UDP packet as IPV4 packet payload.
    ...
    async fn test_two_virtual_peers_intercommunication() {
        ...
        let peer_a_port = a_peer.endpoint.unwrap().port();
        let peer_b_port = b_peer.endpoint.unwrap().port();
        ...
        let udp_packet = create_udp_packet(peer_a_port, peer_b_port);
        ...
        let received_bytes = await_timeout!(chan_b.rx.recv()).unwrap();

        assert!(udp_packet == received_bytes)
    }
}
