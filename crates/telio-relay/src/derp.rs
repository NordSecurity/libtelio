                    ...
                    (err, _, _) = conn_join => {
                        ...
                        self.last_disconnection_reason = match err {
                            Ok(Ok(())) => {
                                ...
                                telio_log_error!("DERP transmission task Ok: {:?} -> {reason:?}", err);
                                reason
                            },
                            Ok(Err(err)) => {
                                telio_log_error!("DERP transmission task error: {:?}", err);
                                err.into()
                            },
                            Err(join_err) => {
                                let reason = RelayConnectionChangeReason::ClientError;
                                telio_log_error!("DERP transmission task join error: {:?} -> {reason:?}", join_err);
                                reason
                            },
                        };
                        self.disconnect().await;
