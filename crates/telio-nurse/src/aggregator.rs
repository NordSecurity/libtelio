...
use telio_utils::{telio_log_debug, telio_log_warn, DualTarget};
use telio_wg::{
...
static RESERVED_IPS: Lazy<HashSet<IpAddr>> = Lazy::new(|| {
    let mut ips = HashSet::new();

    ips.insert(IpAddr::V4(Ipv4Addr::new(100, 64, 0, 1)));
    ips.insert(IpAddr::V4(Ipv4Addr::new(100, 64, 0, 2)));
    ips.insert(IpAddr::V4(Ipv4Addr::new(100, 64, 0, 3)));
    ips.insert(IpAddr::V4(Ipv4Addr::new(100, 64, 0, 4)));
    ips.insert(IpAddr::V4(Ipv4Addr::new(100, 64, 0, 5)));
    ips.insert(IpAddr::V4(Ipv4Addr::new(100, 64, 0, 6)));
    ips.insert(IpAddr::V4(Ipv4Addr::new(100, 64, 0, 7)));

    ips.insert(IpAddr::V6(Ipv6Addr::new(
        0xfd74, 0x656c, 0x696f, 0, 0, 0, 0, 1,
    )));
    ips.insert(IpAddr::V6(Ipv6Addr::new(
        0xfd74, 0x656c, 0x696f, 0, 0, 0, 0, 2,
    )));
    ips.insert(IpAddr::V6(Ipv6Addr::new(
        0xfd74, 0x656c, 0x696f, 0, 0, 0, 0, 3,
    )));
    ips.insert(IpAddr::V6(Ipv6Addr::new(
        0xfd74, 0x656c, 0x696f, 0, 0, 0, 0, 4,
    )));
    ips.insert(IpAddr::V6(Ipv6Addr::new(
        0xfd74, 0x656c, 0x696f, 0, 0, 0, 0, 5,
    )));
    ips.insert(IpAddr::V6(Ipv6Addr::new(
        0xfd74, 0x656c, 0x696f, 0, 0, 0, 0, 6,
    )));
    ips.insert(IpAddr::V6(Ipv6Addr::new(
        0xfd74, 0x656c, 0x696f, 0, 0, 0, 0, 7,
    )));
    ips
});
...
impl ConnectivityDataAggregator {
    ...
    async fn change_peer_state_common(&self, event: &AnalyticsEvent, endpoints: PeerEndpointTypes) {
        ...
        if Some(&data_guard.local_key) != winning_key([&event.public_key, &data_guard.local_key]) {
            telio_log_debug!(
...
fn is_for_reserved_peer(event: &AnalyticsEvent) -> bool {
    fn is_reserved_address(address: &DualTarget) -> bool {
        match address.target {
            (None, None) => false,
            (None, Some(ip)) => RESERVED_IPS.contains(&IpAddr::V6(ip)),
            (Some(ip), None) => RESERVED_IPS.contains(&IpAddr::V4(ip)),
            (Some(ip1), Some(ip2)) => {
                RESERVED_IPS.contains(&IpAddr::V4(ip1)) || RESERVED_IPS.contains(&IpAddr::V6(ip2))
            }
        }
    }
    event.dual_ip_addresses.iter().any(is_reserved_address)
}
...
mod tests {
    ...
    async fn setup(
        ...
        assert_eq!(Some(&smaller_key), winning_key([&smaller_key, &bigger_key]));
    ...
    async fn test_aggregator_force_close() {
        ...
        let AggregatorCollectedSegments {
            ...
            .connectivity_data_aggregator
            .collect_unacknowledged_segments(true)
            .await;
    ...
    async fn direct_connection_reestablished_multiple_times() {
        ...
        current_peer_event.timestamp = segment_start + Duration::from_secs(30);
        ...
        current_peer_event.timestamp = segment_start + Duration::from_secs(30 + 31);
        current_peer_event.tx_bytes = 1000 + 1001;
        env.connectivity_data_aggregator
            .change_peer_state_relayed(&current_peer_event)
            .await;

        current_peer_event.timestamp = segment_start + Duration::from_secs(30 + 31 + 32);
        ...
        current_peer_event.timestamp = segment_start + Duration::from_secs(30 + 31 + 32 + 33);
        current_peer_event.tx_bytes = 1000 + 1001 + 1002 + 1003;
        env.connectivity_data_aggregator
            .change_peer_state_relayed(&current_peer_event)
            .await;

        current_peer_event.timestamp = segment_start + Duration::from_secs(30 + 31 + 32 + 33 + 34);
