...
pub(crate) const TCP_LISTEN_POOL: usize = 16;
const TCP_SOCKET_BUF: usize = 16 * 1024;
const PENDING_TO_CLIENT_MAX: usize = 2 * DNS_TCP_MAX_FRAME;
const CLIENT_CONNECTION_TIMEOUT: Duration = Duration::from_secs(10);
pub(crate) const CLIENT_REQUEST_TIMEOUT: Duration = Duration::from_secs(5);
...
fn new_listen_socket() -> tcp::Socket<'static> {
    ...
    socket.set_timeout(Some(smoltcp::time::Duration::from_secs(
        CLIENT_CONNECTION_TIMEOUT.as_secs(),
    )));
...
async fn service_sockets(
    ...
    for &handle in handles {
        ...
        if let Some(connection) = connections.get_mut(&handle) {
            ...
            // A client that starts a message and stops holds a pool socket for
            // the whole `CLIENT_CONNECTION_TIMEOUT`.
            if connection.held.is_some() || connection.from_client.partial_len() == 0 {
                connection.request_deadline = None;
            } else {
                match connection.request_deadline {
                    None => {
                        connection.request_deadline = Some(Instant::now() + client_request_timeout)
                    }
            ...
            if !connection.client_eof_sent && !socket.may_recv() && !socket.can_recv() {
...
pub(crate) async fn engine_loop(
    ...
    let handles: Vec<SocketHandle> = (0..TCP_LISTEN_POOL)
...
mod tests {
    ...
    #[test]
    fn listen_socket_reaps_idle_clients_and_keeps_doing_so_across_relisten() {
        let expected_timeout =
            smoltcp::time::Duration::from_secs(CLIENT_CONNECTION_TIMEOUT.as_secs());

        let mut socket = new_listen_socket();
        assert_eq!(socket.keep_alive(), None);
        assert_eq!(socket.timeout(), Some(expected_timeout));

        socket.abort();
        assert!(socket.listen(DNS_PORT).is_ok());
        assert_eq!(socket.keep_alive(), None);
        assert_eq!(socket.timeout(), Some(expected_timeout));
    }
}
