...
/// Duration from connection until a response must be received
...
pub(crate) struct Timeouts {
    ...
    client_request: Duration,
}

impl Timeouts {
    pub(crate) fn new(upstream_reply: Duration, client_request: Duration) -> Self {
        Timeouts {
            upstream_reply,
            client_request,
        }
...
mod tests {
    ...
    fn new_forwarder_with_client_timeout(
        ...
        TcpForwarder::new(
            ...
            Timeouts::new(upstream_reply_timeout, client_request_timeout),
        )
    }
    ...
    async fn saturated_pool_rsts_a_new_client() {
        ...
        let mut refused = TestClient::new_with_port(CLIENT_PORT + TCP_POOL_SIZE as u16);
