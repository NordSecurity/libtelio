...
impl LocalNameServer {
    ...
    /// configured for zone `.`.
    pub async fn new(forward_ips: &[IpAddr]) -> crate::error::Result<Arc<RwLock<Self>>> {
    ...
    async fn resolve_dns_request(
        ...
        let dns_request = match &mut request_info.payload {
            PayloadRequestInfo::Udp {
                ...
                .take()
                .ok_or(PacketError::MissingDnsRequest)?,
            _ => return Ok(Vec::new()),
        };
    ...
    fn process_ip_packet<'a, P: IpPacket<'a>>(
        ...
        let ip = P::try_from(request_packet)
            .ok_or(PacketError::BuildIpPacketFailed)?;
        ...
        let payload = match ip.next_level_protocol() {
            IpNextHeaderProtocols::Udp => {
                let udp_request = UdpPacket::new(ip.payload())
                    .ok_or(PacketError::BuildUdpPacketFailed)?;
            ...
            IpNextHeaderProtocols::Tcp => {
                let tcp_request = TcpPacket::new(ip.payload())
                    .ok_or(PacketError::BuildTcpPacketFailed)?;
    ...
    fn build_ip_header(
        ...
        match &self.ip {
            IpRequestInfo::V4 {
                ...
                let mut ip_response = MutableIpv4Packet::new(response_packet).ok_or(
                    PacketError::BuildMutableIpv4PacketFailed
                )?;
            ...
            IpRequestInfo::V6 {
                ...
                let mut ip_response = MutableIpv6Packet::new(response_packet).ok_or(
                    PacketError::BuildMutableIpv6PacketFailed
                )?;
        ...
        match self.payload {
            PayloadRequestInfo::Udp {
                ...
                let mut udp_response = MutableUdpPacket::new(buffer_slice)
                    .ok_or(PacketError::BuildMutableUdpPacketFailed)?;
            ...
            PayloadRequestInfo::Tcp {
                ...
                .ok_or(PacketError::BuildMutableTcpPacketFailed)?;
