...
async fn update_wg_timers(
    ...
    let mut sending_buffer = [0u8; MAX_PACKET];
    loop {
...
impl LocalNameServer {
    ...
    async fn dns_service(
        ...
        let mut receiving_buffer = [0u8; MAX_PACKET];
        ...
        let mut idle_timer = telio_utils::interval(Duration::from_secs(IDLE_TIME_SECONDS));
            ...
            tokio::select! {
                res = socket.recv_from(&mut receiving_buffer) => {
                    ...
                    tokio::spawn(async move {
                        let mut sending_buffer = [0u8; MAX_PACKET];
                        let mut task_receiving_buffer = [0u8; MAX_PACKET];
