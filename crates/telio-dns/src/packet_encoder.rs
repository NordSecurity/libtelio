...
const DNS_HEADER_OFFSET: usize = 0xC;
...
fn create_nord_soa_bytes(ttl: u32) -> Result<Vec<u8>, DnsBuildError> {
    ...
    bytes.extend_from_slice(
        &i32::try_from(ttl)
            .map_err(|_| DnsBuildError::SoaTtlOutOfRange)?
            .to_be_bytes(),
    );
...
impl DnsResponseBuilder {
    ...
    pub fn new(id: u16, query: DnsQuery, ttl: u32, kind: ResponseKind) -> Self {
        Self {
            id,
            query,
            ttl,
...
fn try_push_rr(
    ...
    p.set_ttl(ttl);
    p.set_data_len(data.len() as u16);
    p.set_data(data);
...
#[cfg(test)]
mod tests {
    use crate::packet_decoder::{find_nord_query, parse_dns_packet};
    ...
    fn build_response_answer_a() {
        ...
        let packet = parse_dns_packet(&request_bytes).unwrap();
    ...
    fn build_response_nodata() {
        ...
        let response_bytes = DnsResponseBuilder::new(0x1234, query, 60, ResponseKind::NoData)
            .build()
            .unwrap();
    ...
    fn build_response_nxdomain() {
        ...
        let response_bytes = DnsResponseBuilder::new(0x1234, query, 60, ResponseKind::NxDomain)
            .build()
            .unwrap();
    ...
    fn build_response_soa() {
        ...
        let response_bytes = DnsResponseBuilder::new(0x1234, query, 60, ResponseKind::SoaAnswer)
            .build()
            .unwrap();
