...
impl UpnpEpCommands for IgdGateway {
    ...
    async fn add_endpoint_routes(
        ...
        let now = Instant::now();
        const RENEW_AFTER: f64 = 0.75;
        let should_renew_after =
            now + Duration::from_secs_f64(self.lease_duration.as_secs_f64() * RENEW_AFTER);
        ...
        *self.needs_lease_renew_after.lock() = Some(should_renew_after);
        ...
        telio_log_debug!(
            "This lease will expire on {expires_at:?} (now + {:?})",
            self.lease_duration
        );

        tokio::spawn(async move {
            tokio::time::sleep_until(expires_at).await;
            telio_log_debug!("Lease expired");
        });

        tokio::spawn(async move {
            let d = should_renew_after - Instant::now();
            telio_log_debug!("Should renew the lease in {d:?} at {should_renew_after:?}");
            tokio::time::sleep_until(should_renew_after).await;
            telio_log_debug!("Should renew the lease");
        });
