...
impl SessionKeeperTrait for SessionKeeper {
    ...
    // TODO: this function should _probably_ remove the key from the RepeatedActions in non-batching scenarios
    async fn remove_node(&self, key: &PublicKey) -> Result<()> {
        let key = *key;
        task_exec!(&self.task, async move |s| {
            let _ = s.batched_actions.remove(&key);
            Ok(())
        })
        .await?;
...
impl Runtime for State {
        ...
        F: Future<Output = BoxAction<Self, std::result::Result<(), Self::Err>>> + Send,
    {
        tokio::select! {
            ...
            batched_actions = self.batched_actions.get_future() => {
                let batched_actions = batched_actions;
...
mod tests {
    ...
    async fn send_keepalives() {
        ...
        sess_keep
            .add_node(AddNodeParameters::NonBatching {
                public_key: pk,
                target: (Some(Ipv4Addr::LOCALHOST), Some(Ipv6Addr::LOCALHOST)),
                interval: PERIOD,
            })
            .await
            .unwrap();
