...
pub trait UpgradeSyncTrait {
    ...
    async fn downgrade(&self, public_key: &PublicKey) -> Result<()>;
}
...
impl UpgradeSyncTrait for UpgradeSync {
    ...
    async fn downgrade(&self, public_key: &PublicKey) -> Result<()> {
        telio_log_debug!("Will record a downgrade to the relayed connection for {public_key:?}");
        let public_key = *public_key;
        task_exec!(&self.task, async move |s| {
            s.downgrade(public_key).await;
            Ok(())
        })
        .await
        .map_err(Error::Task)
...
impl State {
    ...
    async fn downgrade(&self, public_key: PublicKey) {
            ...
            Ok(true) => {
                let decision = match self.upgrade_requests.get(&Direction::Sent(*public_key)) {
                    Some(v) => {
                        ...
                        } else if Some(&self.our_public_key)
