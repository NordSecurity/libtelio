    ...
    ($level:expr, $text:literal) => {
        {
            ...
            if *MIN_LOG_LEVEL.read() <= $level {
                if let Some(logger) = LOG_CALLBACK.read().as_ref() {
                    use std::ffi::CString;
                    let log_message = match CString::new($text) {
                        Ok(message) => message,
                        Err(err) => CString::new(format!("INVALID LOG MESSAGE: NULL byte on position: {}", err.nul_position())).expect("There can't be any NULL characters in this string")
                    };
                    logger($level, log_message.as_ptr());
                }
            }
        }
    };
    ($level:expr, $text:literal, $($arg:expr),*) => {
        {
            ...
            if *MIN_LOG_LEVEL.read() <= $level {
                if let Some(logger) = LOG_CALLBACK.read().as_ref() {
                    use std::ffi::CString;
                    let log_message = match CString::new(format!($text, $($arg),*)) {
                        Ok(message) => message,
                        Err(err) => CString::new(format!("INVALID LOG MESSAGE: NULL byte on position: {}", err.nul_position())).expect("There can't be any NULL characters in this string")
                    };
                    logger($level, log_message.as_ptr());
