...
pub mod tests {
    ...
    fn ip_address_set_multiple_times() {
        ...
        assert_eq!(fw.get_config().ip_addresses.len(), 0);

        let mut config = fw.get_config();
        config.ip_addresses = ip_vec.clone();
        fw.apply_config(config);
        assert_eq!(fw.get_config().ip_addresses.len(), 2);
        assert_eq!(fw.get_config().ip_addresses, ip_vec);
        ...
        let mut config = fw.get_config();
        config.ip_addresses = new_ip_vec.clone();
        fw.apply_config(config);
        assert_eq!(fw.get_config().ip_addresses.len(), 2);
        assert_eq!(fw.get_config().ip_addresses, new_ip_vec);
    }
}
