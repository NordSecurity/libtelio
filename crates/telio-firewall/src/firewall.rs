...
impl StatefullFirewall {
    ...
    pub fn new(use_ipv6: bool, tcp_record_whitelisted: bool, feature: FeatureFirewall) -> Self {
        StatefullFirewall::new_custom(
            LRU_CAPACITY,
            LRU_TIMEOUT,
            use_ipv6,
        ...
        match udp_cache.entry(key, true) {
            Entry::Occupied(mut occ) => {
                ...
                if connection_info.is_remote_initiated {
                    match is_conn_policy_compliant(*peer, local_port) {
                        PacketAction::Drop => {
        ...
        match local_interfaces::gather_local_interfaces(&self.get_if_addr) {
            Ok(local_itf) => {
                if local_itf
                    ...
                    .ne(&0)
                {
                    telio_log_trace!("Packet destination is for local node");
                    return false;
                }
            }
            Err(e) => {
                telio_log_warn!("Error in gathering local interfaces {}", e);
            }
        }
