...
pub(crate) fn configure_chain(
    config: &FirewallConfig,
    dynamic_state: &FirewallDynamicState,
    config_state: &FirewallConfiguredState,
    local_ifs_addrs: &[StdIpAddr],
) -> FfiChainGuard {
    build_chain_rules(config, dynamic_state, config_state, local_ifs_addrs)
        .as_slice()
        .into()
}
...
mod tests {
    ...
    fn test_reject_rule_not_emitted_without_tunnel_ip() {
        ...
        let dynamic_state = FirewallDynamicState {
            tunnel_ips: vec![],
            ..Default::default()
        };
        let config_state = FirewallConfiguredState::default();
        let rules = build_chain_rules(&config, &dynamic_state, &config_state, &[]);
        assert!(
            rules
                .iter()
