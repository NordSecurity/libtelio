use std::{
    convert::TryInto,
    fmt::{Debug, Formatter},
    net::IpAddr as StdIpAddr,
    time::Duration,
};

use crate::{
    error::{Error, Result},
    ffi_chain::{
        LIBFW_CONTRACK_STATE_CLOSED, LIBFW_CONTRACK_STATE_ESTABLISHED,
        LIBFW_CONTRACK_STATE_INVALID, LIBFW_CONTRACK_STATE_NEW, LIBFW_CONTRACK_STATE_RELATED,
        LIBFW_DIRECTION_INBOUND, LIBFW_DIRECTION_OUTBOUND,
    },
    log::{libfw_log_debug, libfw_log_trace, libfw_log_warn},
};
use num_enum::TryFromPrimitive;
use parking_lot::Mutex;
use pnet_packet::{
    icmp::{destination_unreachable::IcmpCodes, IcmpTypes, MutableIcmpPacket},
    icmpv6::{Icmpv6Code, Icmpv6Types, MutableIcmpv6Packet},
    ip::IpNextHeaderProtocols,
    ipv4::{Ipv4Flags, MutableIpv4Packet},
    ipv6::MutableIpv6Packet,
    tcp::{MutableTcpPacket, TcpFlags, TcpPacket},
    udp::UdpPacket,
    Packet,
};
use smallvec::{SmallVec, ToSmallVec};

use crate::packet::{
    GenericIcmpPacket, GenericIcmpType, IcmpPacketGroup, IpAddr, IpPacket, TCP_FIRST_PKT_MASK,
};
use telio_utils::lru_cache::{Entry, LruCache};

macro_rules! unwrap_option_or_return {
    ( $option:expr, $retval:expr ) => {
        match $option {
            Some(x) => x,
            None => return $retval,
        }
    };
    ( $option:expr ) => {
        unwrap_option_or_return!($option, ())
    };
}

pub(crate) use unwrap_option_or_return;

const KEY_SIZE: usize = 32;

pub(crate) const LRU_CACHE_SIZE: usize = 8192;
pub(crate) const LRU_CACHE_TCP_TTL: Duration = Duration::from_secs(7440); // 2 hours 4 minutes as per  REQ-5 of RFC5382 section-8
pub(crate) const LRU_CACHE_UDP_TTL: Duration = Duration::from_secs(120); // 2 minutes as per REQ-5 of RFC4787 section-12
pub(crate) const LRU_CACHE_ICMP_TTL: Duration = Duration::from_secs(60); // 1 minute as per REQ-2 of RFC5508 section-9

#[derive(Clone, Ord, PartialOrd, Eq, PartialEq, Hash)]
pub(crate) struct IpConnWithPort {
    pub(crate) remote_addr: IpAddr,
    pub(crate) remote_port: u16,
    pub(crate) local_addr: IpAddr,
    pub(crate) local_port: u16,
}

impl Debug for IpConnWithPort {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("IpConnWithPort")
            .field("remote_addr", &StdIpAddr::from(self.remote_addr))
            .field("remote_port", &self.remote_port)
            .field("local_addr", &StdIpAddr::from(self.local_addr))
            .field("local_port", &self.local_port)
            .finish()
    }
}

pub(crate) type AssociatedData = Option<SmallVec<[u8; KEY_SIZE]>>;

#[derive(Debug, Clone, Ord, PartialOrd, Eq, PartialEq, Hash)]
pub(crate) struct Connection {
    pub(crate) link: IpConnWithPort,
    // Data associated with the packets, usually the public keys refering
    // to source peer for inbound connections and destination peer for outbound
    // connections, which may be used for tracking and filtering packets
    pub(crate) associated_data: AssociatedData,
}

#[derive(Clone, Ord, PartialOrd, Eq, PartialEq, Hash)]
pub(crate) struct IcmpConn {
    src_addr: IpAddr,
    dst_addr: IpAddr,
    response_type: u8,
    identifier_sequence: u16,
    // Data associated with the packets, usually the public keys refering
    // to source peer for inbound connections and destination peer for outbound
    // connections, which may be used for tracking and filtering packets
    associated_data: AssociatedData,
}

enum IcmpKey {
    Conn(IcmpConn),
    Err(IcmpErrorKey),
}

#[derive(Clone, Debug)]
pub enum IcmpErrorKey {
    Udp(IpConnWithPort),
    Tcp(IpConnWithPort),
    Icmp(Box<IcmpConn>),
}

impl Debug for IcmpConn {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("IcmpConn")
            .field("src_addr", &StdIpAddr::from(self.src_addr))
            .field("dst_addr", &StdIpAddr::from(self.dst_addr))
            .field("response_type", &self.response_type)
            .field("identifier_sequence", &self.identifier_sequence)
            .finish()
    }
}

///
/// Packet direction
///
#[derive(Copy, Clone, Debug, Eq, PartialEq, TryFromPrimitive)]
#[repr(u8)]
pub(crate) enum Direction {
    /// Outgoing packets
    Outbound = LIBFW_DIRECTION_OUTBOUND,
    /// Incoming packets
    Inbound = LIBFW_DIRECTION_INBOUND,
}

///
/// Connection state
///
#[allow(clippy::enum_variant_names)]
#[repr(u8)]
#[derive(Clone, Copy, Debug, PartialEq, Eq, TryFromPrimitive)]
pub enum ConnectionState {
    /// Connection established
    Established = LIBFW_CONTRACK_STATE_ESTABLISHED,
    /// One-direction connections
    New = LIBFW_CONTRACK_STATE_NEW,
    /// Malformed and unexpected packets
    Invalid = LIBFW_CONTRACK_STATE_INVALID,
    /// Packets related to a different connections
    Related = LIBFW_CONTRACK_STATE_RELATED,
    /// TODO(LLT-6590): We should get rid of this state or make it returnable only for TCP packets
    /// Finished connections (TCP only!)
    Closed = LIBFW_CONTRACK_STATE_CLOSED,
}

#[derive(Clone, Debug)]
pub struct UdpConnectionInfo {
    pub(crate) is_remote_initiated: bool,
    pub(crate) last_out_pkg_chunk: Option<Vec<u8>>,
    pub(crate) state: ConnectionState,
}

impl UdpConnectionInfo {
    /// UDP packet header contains at least 8 bytes and IP packet header is
    /// 20-60 bytes long, so we need to store up to 68 bytes of previus
    /// packet
    /// For IPv6 the fixed header is 40 bytes long. There is no upper limit for
    /// a header with extensions. Let's stick with the value for IPv4. In the worst
    /// case we will not be able to reset IPv6 connections with unusually long header.
    pub(crate) const LAST_PKG_MAX_CHUNK_LEN: usize = 60 + 8;
}

#[derive(PartialEq, Debug, Clone)]
pub(crate) struct TcpConnectionInfo {
    pub(crate) tx_alive: bool,
    pub(crate) rx_alive: bool,
    pub(crate) conn_remote_initiated: bool,
    pub(crate) next_seq: Option<u32>,
    pub(crate) state: ConnectionState,
}

pub(crate) struct Conntrack {
    /// Recent udp connections
    pub(crate) udp: Mutex<LruCache<Connection, UdpConnectionInfo>>,
    /// Recent tcp connections
    pub(crate) tcp: Mutex<LruCache<Connection, TcpConnectionInfo>>,
    /// Recent icmp connections
    pub(crate) icmp: Mutex<LruCache<IcmpConn, ConnectionState>>,
}

impl Conntrack {
    pub(crate) fn new() -> Self {
        Self::new_with_capacity_and_ttl(
            LRU_CACHE_SIZE,
            LRU_CACHE_TCP_TTL,
            LRU_CACHE_UDP_TTL,
            LRU_CACHE_ICMP_TTL,
        )
    }

    #[cfg(test)]
    /// Return the size of conntrack entries for tcp and udp (for testing only).                                 
    pub(crate) fn get_connections_count(&self) -> (usize, usize) {
        (self.tcp.lock().len(), self.udp.lock().len())
    }

    /// Constructs a new conntracker
    pub(crate) fn new_with_capacity_and_ttl(
        capacity: usize,
        tcp_ttl: Duration,
        udp_ttl: Duration,
        icmp_ttl: Duration,
    ) -> Self {
        Self {
            tcp: Mutex::new(LruCache::new(tcp_ttl, capacity)),
            udp: Mutex::new(LruCache::new(udp_ttl, capacity)),
            icmp: Mutex::new(LruCache::new(icmp_ttl, capacity)),
        }
    }

    fn handle_outbound_udp<'a>(
        &self,
        ip: &impl IpPacket<'a>,
        associated_data: Option<&[u8]>,
    ) -> Result<ConnectionState> {
        let link = Self::build_conn_info(ip, Direction::Outbound)?.0;
        let key = Connection {
            link,
            associated_data: associated_data.map(|data| data.to_smallvec()),
        };

        libfw_log_trace!("Processing outbound UDP packet with {:?}", key);

        let Some(pkg_chunk) = ip
            .packet()
            .chunks(UdpConnectionInfo::LAST_PKG_MAX_CHUNK_LEN)
            .next()
        else {
            libfw_log_warn!("Failed to extract headers of UDP packet");
            return Err(Error::MalformedUdpPacket);
        };

        let mut udp_cache = self.udp.lock();
        // If key already exists, dont change the value, just update timer with entry
        match udp_cache.entry(key, true) {
            Entry::Vacant(e) => {
                let mut last_chunk = Vec::with_capacity(UdpConnectionInfo::LAST_PKG_MAX_CHUNK_LEN);
                last_chunk.extend_from_slice(pkg_chunk);

                let conninfo = UdpConnectionInfo {
                    is_remote_initiated: false,
                    last_out_pkg_chunk: Some(last_chunk),
                    state: ConnectionState::New,
                };
                libfw_log_trace!("Inserting new UDP conntrack entry {:?}", e.key());
                e.insert(conninfo);

                Ok(ConnectionState::New)
            }
            Entry::Occupied(mut o) => {
                libfw_log_trace!("Matched UDP conntrack entry {:?}", o.key());
                let value = o.get_mut();

                // Take out the old one clear and copy data from new packet in
                // order to avoid allocation
                let mut last_chunk = value.last_out_pkg_chunk.take().unwrap_or_else(|| {
                    Vec::with_capacity(UdpConnectionInfo::LAST_PKG_MAX_CHUNK_LEN)
                });
                last_chunk.clear();
                last_chunk.extend_from_slice(pkg_chunk);

                if value.is_remote_initiated && value.state == ConnectionState::New {
                    value.state = ConnectionState::Established
                }

                value.last_out_pkg_chunk = Some(last_chunk);

                Ok(value.state)
            }
        }
    }

    fn handle_outbound_tcp<'a>(
        &self,
        ip: &impl IpPacket<'a>,
        associated_data: Option<&[u8]>,
    ) -> Result<ConnectionState> {
        let (link, packet) = Self::build_conn_info(ip, Direction::Outbound)?;
        let key = Connection {
            link,
            associated_data: associated_data.map(|data| data.to_smallvec()),
        };

        libfw_log_trace!(
            "Processing outbound TCP packet with {:?} -> {:?}",
            key,
            packet
        );

        let flags = packet.map(|p| p.get_flags()).unwrap_or(0);
        let mut tcp_cache = self.tcp.lock();

        if flags & TCP_FIRST_PKT_MASK == TcpFlags::SYN {
            libfw_log_trace!("Inserting TCP conntrack entry {:?}", key.link);
            tcp_cache.insert(
                key,
                TcpConnectionInfo {
                    tx_alive: true,
                    rx_alive: true,
                    conn_remote_initiated: false,
                    next_seq: None,
                    state: ConnectionState::New,
                },
            );
            return Ok(ConnectionState::New);
        }

        if flags & TcpFlags::RST == TcpFlags::RST {
            libfw_log_trace!("Removing TCP conntrack entry {:?}", key);
            let current_state = if let Some(conn_entry) = tcp_cache.remove(&key) {
                conn_entry.state
            } else {
                ConnectionState::New
            };
            return Ok(current_state);
        }

        if flags & TcpFlags::FIN == TcpFlags::FIN {
            // For FIN packets we shouldn't bump the connection timer
            if let Entry::Occupied(mut e) = tcp_cache.entry(key.clone(), false) {
                libfw_log_trace!("Connection {:?} closing", key);
                let TcpConnectionInfo {
                    tx_alive,
                    rx_alive,
                    state,
                    ..
                } = e.get_mut();
                *tx_alive = false;
                if !*rx_alive {
                    *state = ConnectionState::Closed
                }
                return Ok(*state);
            }
        }

        if let Some(tcp_connection_info) = tcp_cache.get_mut(&key) {
            libfw_log_trace!("Matched TCP conntrack entry {:?}", key);
            if tcp_connection_info.conn_remote_initiated
                && (flags & TCP_FIRST_PKT_MASK == TCP_FIRST_PKT_MASK)
            {
                tcp_connection_info.state = ConnectionState::Established;
            }
            Ok(tcp_connection_info.state)
        } else {
            Ok(ConnectionState::New)
        }
    }

    #[allow(dead_code)]
    pub(crate) fn cleanup_udp_conn_on_drop<'a, P: IpPacket<'a>>(
        &self,
        ip: P,
        associated_data: Option<&[u8]>,
    ) -> Result<()> {
        let (link, _) = Self::build_conn_info(&ip, Direction::Inbound)?;
        let key = Connection {
            link,
            associated_data: associated_data.map(|data| data.to_smallvec()),
        };

        self.udp.lock().remove(&key);

        Ok(())
    }

    fn handle_inbound_udp<'a>(
        &self,
        ip: &impl IpPacket<'a>,
        associated_data: Option<&[u8]>,
    ) -> Result<ConnectionState> {
        let (link, _) = Self::build_conn_info(ip, Direction::Inbound)?;
        let key = Connection {
            link,
            associated_data: associated_data.map(|data| data.to_smallvec()),
        };

        libfw_log_trace!("Processing inbound UDP packet with {:?}", key);

        let mut udp_cache = self.udp.lock();

        match udp_cache.entry(key, true) {
            Entry::Occupied(mut occ) => {
                libfw_log_trace!(
                    "Matched UDP conntrack entry {:?} {:?}",
                    occ.key(),
                    occ.get()
                );

                if ConnectionState::New == occ.get().state && !occ.get().is_remote_initiated {
                    occ.get_mut().state = ConnectionState::Established;
                }
                Ok(occ.get().state)
            }
            Entry::Vacant(vacc) => {
                let key = vacc.key();

                libfw_log_trace!(
                    "Updating UDP conntrack entry {:?} {:?}",
                    key,
                    associated_data
                );
                vacc.insert(UdpConnectionInfo {
                    is_remote_initiated: true,
                    last_out_pkg_chunk: None,
                    state: ConnectionState::New,
                });
                Ok(ConnectionState::New)
            }
        }
    }

    #[allow(dead_code)]
    pub(crate) fn cleanup_tcp_conn<'a, P: IpPacket<'a>>(
        &self,
        ip: P,
        associated_data: Option<&[u8]>,
        accepted: bool,
    ) -> Result<()> {
        let (link, _) = Self::build_conn_info(&ip, Direction::Inbound)?;
        let key = Connection {
            link,
            associated_data: associated_data.map(|data| data.to_smallvec()),
        };

        let mut cache = self.tcp.lock();
        if let Entry::Occupied(mut occ) = cache.entry(key, false) {
            if !accepted && occ.get().conn_remote_initiated {
                occ.remove();
            }
        }

        Ok(())
    }

    // Conntracker entries are ment for packets handled locally - so
    fn handle_inbound_tcp<'a>(
        &self,
        ip: &impl IpPacket<'a>,
        associated_data: Option<&[u8]>,
    ) -> Result<ConnectionState> {
        let (link, packet) = Self::build_conn_info(ip, Direction::Inbound)?;
        let key = Connection {
            link,
            associated_data: associated_data.map(|data| data.to_smallvec()),
        };
        let local_port = key.link.local_port;
        libfw_log_trace!(
            "Processing inbound TCP packet with {:?} -> {:?}",
            key,
            packet
        );

        let mut cache = self.tcp.lock();
        // Dont update last access time in tcp, as it is handled by tcp flags
        match cache.entry(key, false) {
            Entry::Occupied(mut occ) => {
                let connection_info = occ.get_mut();
                libfw_log_trace!(
                    "Matched conntrack entry {:?} {:?}",
                    associated_data,
                    connection_info
                );

                if let Some(pkt) = packet {
                    let flags = pkt.get_flags();
                    if flags & TcpFlags::RST == TcpFlags::RST {
                        libfw_log_trace!(
                            "Removing TCP conntrack entry with {:?} for {:?}",
                            associated_data,
                            local_port
                        );
                        let current_state = occ.get().state;
                        occ.remove();
                        return Ok(current_state);
                    }

                    if (flags & TcpFlags::FIN) == TcpFlags::FIN {
                        libfw_log_trace!(
                            "Connection with {:?} for {:?} closing",
                            associated_data,
                            local_port
                        );
                        connection_info.rx_alive = false;
                    }

                    if !connection_info.tx_alive && !connection_info.rx_alive {
                        if (flags & TcpFlags::ACK) == TcpFlags::ACK {
                            libfw_log_trace!(
                                "Removing TCP conntrack entry with {:?} for {:?}",
                                associated_data,
                                local_port
                            );
                            occ.remove();
                        } else {
                            occ.get_mut().state = ConnectionState::Closed;
                        }

                        return Ok(ConnectionState::Closed);
                    }

                    // restarts cache entry timeout
                    let next_seq = if (flags & TcpFlags::SYN) == TcpFlags::SYN {
                        pkt.get_sequence().overflowing_add(1).0
                    } else {
                        pkt.get_sequence()
                            .overflowing_add(pkt.payload().len() as u32)
                            .0
                    };

                    connection_info.next_seq = Some(next_seq);
                    if !connection_info.conn_remote_initiated
                        && (flags & TCP_FIRST_PKT_MASK == TCP_FIRST_PKT_MASK)
                    {
                        // Connection which is not removed and packets are going in both directions should be established
                        occ.get_mut().state = ConnectionState::Established;
                    }
                }

                Ok(occ.get().state)
            }
            Entry::Vacant(vacc) => {
                // Accept and should handle locally
                if let Some(pkt) = packet {
                    let key = vacc.key();
                    let flags = pkt.get_flags();
                    if flags & TCP_FIRST_PKT_MASK == TcpFlags::SYN {
                        let conn_info = TcpConnectionInfo {
                            tx_alive: true,
                            rx_alive: true,
                            conn_remote_initiated: true,
                            next_seq: Some(pkt.get_sequence().overflowing_add(1).0),
                            state: ConnectionState::New,
                        };
                        libfw_log_trace!(
                            "Updating TCP conntrack entry {:?} {:?} {:?}",
                            key,
                            associated_data,
                            conn_info
                        );
                        vacc.insert(conn_info.clone());
                    }
                }
                Ok(ConnectionState::New)
            }
        }
    }

    fn handle_icmp<'a, P: IpPacket<'a>>(
        &self,
        ip: &P,
        associated_data: Option<&[u8]>,
    ) -> Result<ConnectionState> {
        match Self::build_icmp_key(ip, associated_data, false) {
            Ok((IcmpKey::Conn(key), icmp_packet_group)) => {
                libfw_log_trace!("Processing inbound ICMP packet with {:?}", key);
                let mut icmp_cache = self.icmp.lock();
                match icmp_cache.entry(key, true) {
                    Entry::Occupied(mut occupied_entry) => {
                        if icmp_packet_group == IcmpPacketGroup::Reply {
                            *(occupied_entry.get_mut()) = ConnectionState::Established;
                        }
                        Ok(*occupied_entry.get())
                    }
                    Entry::Vacant(vacant_entry) => {
                        libfw_log_trace!(
                            "Inserting new ICMP conntrack entry {:?}",
                            vacant_entry.key()
                        );
                        if icmp_packet_group == IcmpPacketGroup::Reply {
                            Ok(ConnectionState::Invalid)
                        } else {
                            vacant_entry.insert(ConnectionState::New);
                            Ok(ConnectionState::New)
                        }
                    }
                }
            }
            Ok((IcmpKey::Err(icmp_error_key), _)) => {
                libfw_log_trace!(
                    "Encountered ICMP error packet {:?}, checking nested packet",
                    icmp_error_key
                );
                Ok(self.handle_inbound_icmp_error(icmp_error_key, associated_data))
            }
            Err(Error::UnexpectedPacketType) => {
                libfw_log_trace!("Encountered ICMP packet type not tracked in this direction");
                Ok(ConnectionState::Invalid)
            }
            Err(err) => Err(err),
        }
    }

    fn handle_inbound_icmp_error(
        &self,
        error_key: IcmpErrorKey,
        associated_data: Option<&[u8]>,
    ) -> ConnectionState {
        match error_key {
            IcmpErrorKey::Icmp(icmp_key) => {
                let mut icmp_cache = self.icmp.lock();
                if icmp_cache.get(&icmp_key).is_some() {
                    return ConnectionState::Related;
                }
            }
            IcmpErrorKey::Tcp(link) => {
                let tcp_key = Connection {
                    link,
                    associated_data: associated_data.map(|data| data.to_smallvec()),
                };

                let mut tcp_cache = self.tcp.lock();

                if tcp_cache.get(&tcp_key).is_some() {
                    libfw_log_trace!("Removing TCP conntrack entry {:?}", tcp_key);
                    tcp_cache.remove(&tcp_key);
                    return ConnectionState::Related;
                }
            }
            IcmpErrorKey::Udp(link) => {
                let udp_key = Connection {
                    link,
                    associated_data: associated_data.map(|data| data.to_smallvec()),
                };

                let mut udp_cache = self.udp.lock();
                if udp_cache.get(&udp_key).is_some() {
                    libfw_log_trace!("Removing UDP conntrack entry {:?}", udp_key);
                    udp_cache.remove(&udp_key);
                    return ConnectionState::Related;
                }
            }
        }
        ConnectionState::Invalid
    }

    pub fn build_conn_info<'a, P: IpPacket<'a>>(
        ip: &P,
        direction: Direction,
    ) -> Result<(IpConnWithPort, Option<TcpPacket<'_>>)> {
        let proto = ip.get_next_level_protocol();
        let (src, dest, tcp_packet) = match proto {
            IpNextHeaderProtocols::Udp => match UdpPacket::new(ip.payload()) {
                Some(packet) => (packet.get_source(), packet.get_destination(), None),
                _ => {
                    libfw_log_trace!("Could not create UDP packet from IP packet {:?}", ip);
                    return Err(Error::MalformedUdpPacket);
                }
            },
            IpNextHeaderProtocols::Tcp => match TcpPacket::new(ip.payload()) {
                Some(packet) => (packet.get_source(), packet.get_destination(), Some(packet)),
                _ => {
                    libfw_log_trace!("Could not create TCP packet from IP packet {:?}", ip);
                    return Err(Error::MalformedTcpPacket);
                }
            },
            _ => return Err(Error::UnexpectedProtocol),
        };

        let key = if let Direction::Inbound = direction {
            IpConnWithPort {
                remote_addr: ip.get_source().into(),
                remote_port: src,
                local_addr: ip.get_destination().into(),
                local_port: dest,
            }
        } else {
            IpConnWithPort {
                remote_addr: ip.get_destination().into(),
                remote_port: dest,
                local_addr: ip.get_source().into(),
                local_port: src,
            }
        };

        match proto {
            IpNextHeaderProtocols::Udp => Ok((key, None)),
            IpNextHeaderProtocols::Tcp => Ok((key, tcp_packet)),
            _ => Err(Error::UnexpectedProtocol),
        }
    }

    fn build_icmp_key<'a, P: IpPacket<'a>>(
        ip: &P,
        associated_data: Option<&[u8]>,
        ignore_checksum: bool,
    ) -> Result<(IcmpKey, IcmpPacketGroup)> {
        let icmp_packet = ip.try_into_icmp_packet(ignore_checksum)?;

        let Some((response_type, packet_group)) = ({
            let packet_type = icmp_packet.get_icmp_type();
            match packet_type.get_packet_group() {
                Some(IcmpPacketGroup::Error) => {
                    return Self::build_icmp_error_key(&icmp_packet, associated_data)
                        .map(|key| (key, IcmpPacketGroup::Error));
                }
                Some(group) => packet_type
                    .get_response_type()
                    .map(|resp_type| (resp_type, group)),
                None => {
                    return Err(Error::MalformedIcmpPacket);
                }
            }
        }) else {
            return Err(Error::MalformedIcmpPacket);
        };

        let identifier_sequence = {
            let bytes = icmp_packet
                .payload()
                // The ICMP identifier is stored in the first two bytes of the payload
                .get(0..2)
                .and_then(|b| b.try_into().ok());
            let bytes = unwrap_option_or_return!(bytes, Err(Error::MalformedIcmpPacket));
            u16::from_ne_bytes(bytes)
        };

        let src_addr = if packet_group == IcmpPacketGroup::Reply {
            ip.get_source().into()
        } else {
            ip.get_destination().into()
        };

        let dst_addr = if packet_group == IcmpPacketGroup::Reply {
            ip.get_destination().into()
        } else {
            ip.get_source().into()
        };

        let key = IcmpConn {
            src_addr,
            dst_addr,
            response_type,
            identifier_sequence,
            associated_data: associated_data.map(|data| data.to_smallvec()),
        };

        Ok((IcmpKey::Conn(key), packet_group))
    }

    fn build_icmp_error_key<'a, P: GenericIcmpPacket<'a>>(
        icmp_packet: &P,
        associated_data: Option<&[u8]>,
    ) -> Result<IcmpKey> {
        let inner_packet = match icmp_packet.payload().get(4..) {
            Some(bytes) => bytes,
            None => {
                libfw_log_trace!("ICMP error body does not contain a nested packet");
                return Err(Error::MalformedIcmpPacket);
            }
        };
        Ok(IcmpKey::Err({
            let packet = match <P::IpPacketType<'_> as IpPacket>::try_from_truncated(inner_packet) {
                Some(packet) => packet,
                _ => {
                    libfw_log_trace!("ICMP error contains invalid IP packet");
                    return Err(Error::InvalidIcmpErrorPayload);
                }
            };
            match packet.get_next_level_protocol() {
                IpNextHeaderProtocols::Udp => {
                    IcmpErrorKey::Udp(Self::build_conn_info(&packet, Direction::Outbound)?.0)
                }
                IpNextHeaderProtocols::Tcp => {
                    IcmpErrorKey::Tcp(Self::build_conn_info(&packet, Direction::Outbound)?.0)
                }
                proto => {
                    // TODO: make it a match arm when generic_const_exprs will be in the stable compiler
                    if proto == P::ICMP_NEXT_HEADER_ID {
                        if let (IcmpKey::Conn(conn), _) =
                            Self::build_icmp_key(&packet, associated_data, true)?
                        {
                            IcmpErrorKey::Icmp(Box::new(conn))
                        } else {
                            return Err(Error::MalformedIcmpPacket);
                        }
                    } else {
                        return Err(Error::UnexpectedProtocol);
                    }
                }
            }
        }))
    }

    pub(crate) fn reset_tcp_conns<F>(
        &self,
        associated_data: Option<&[u8]>,
        inject_packet_cb: F,
    ) -> Result<()>
    where
        F: FnMut(&[u8]),
    {
        let mut tcp_conn_cache = self.tcp.lock();

        libfw_log_debug!(
            "Inspecting {} TCP connections for reset",
            tcp_conn_cache.len()
        );

        let mut keys_to_remove = vec![];
        let iter = tcp_conn_cache.iter().filter_map(|(k, v)| {
            match (&k.associated_data, associated_data) {
                (Some(conn_assoc_data), Some(assoc_data)) if &**conn_assoc_data == assoc_data => {
                    keys_to_remove.push(k.clone());
                    // When we don't have the sequence number that means the
                    // connection is half open and we don't have a response
                    // from the remote peer yet. We need to skip this kind of
                    // connections
                    v.next_seq.map(|next_seq| (&k.link, next_seq, None))
                }
                (None, None) => {
                    keys_to_remove.push(k.clone());
                    // When we don't have the sequence number - see above
                    v.next_seq.map(|next_seq| (&k.link, next_seq, None))
                }
                _ => None,
            }
        });

        let result = self.send_tcp_rst_packets(iter, inject_packet_cb);

        for key in keys_to_remove.iter() {
            tcp_conn_cache.remove(key);
        }

        result
    }

    fn send_tcp_rst_packets<'a, I, F>(&self, connections: I, mut inject_packet_cb: F) -> Result<()>
    where
        I: IntoIterator<Item = (&'a IpConnWithPort, u32, Option<u32>)>,
        F: FnMut(&[u8]),
    {
        const IPV4_LEN: usize = 20;
        const IPV6_LEN: usize = 40;
        const TCP_LEN: usize = 20;

        let mut ipv4buf = [0u8; IPV4_LEN];
        let mut ipv6buf = [0u8; IPV6_LEN];
        let mut tcpbuf = [0u8; TCP_LEN];

        // Write most of the fields for TCP and IP packets upfront
        #[allow(clippy::expect_used)]
        let mut tcppkg =
            MutableTcpPacket::new(&mut tcpbuf).expect("TCP buffer should not be too small");
        tcppkg.set_data_offset(5);

        #[allow(clippy::expect_used)]
        let mut ipv4pkg =
            MutableIpv4Packet::new(&mut ipv4buf).expect("IPv4 buffer should not be too small");
        ipv4pkg.set_version(4);
        ipv4pkg.set_header_length(5);
        ipv4pkg.set_total_length((IPV4_LEN + TCP_LEN) as _);
        ipv4pkg.set_flags(Ipv4Flags::DontFragment);
        ipv4pkg.set_ttl(0xFF);
        ipv4pkg.set_next_level_protocol(IpNextHeaderProtocols::Tcp);

        #[allow(clippy::expect_used)]
        let mut ipv6pkg =
            MutableIpv6Packet::new(&mut ipv6buf).expect("IPv4 buffer should not be too small");
        ipv6pkg.set_version(6);
        ipv6pkg.set_payload_length(TCP_LEN as _);
        ipv6pkg.set_next_header(IpNextHeaderProtocols::Tcp);
        ipv6pkg.set_hop_limit(0xff);

        let mut ipv4pkgbuf = [0u8; IPV4_LEN + TCP_LEN];
        let mut ipv6pkgbuf = [0u8; IPV6_LEN + TCP_LEN];

        for (key, next_seq, ack) in connections {
            libfw_log_debug!("Killing TCP connection {:?}", key);
            tcppkg.set_source(key.remote_port);
            tcppkg.set_destination(key.local_port);
            tcppkg.set_sequence(next_seq);
            if let Some(ack) = ack {
                tcppkg.set_flags(TcpFlags::RST | TcpFlags::ACK);
                tcppkg.set_acknowledgement(ack);
            } else {
                tcppkg.set_flags(TcpFlags::RST);
            }

            match (key.local_addr, key.remote_addr) {
                (IpAddr::Ipv4(local_addr), IpAddr::Ipv4(remote_addr)) => {
                    let src = remote_addr.into();
                    let dst = local_addr.into();

                    ipv4pkg.set_source(src);
                    ipv4pkg.set_destination(dst);

                    tcppkg.set_checksum(pnet_packet::tcp::ipv4_checksum(
                        &tcppkg.to_immutable(),
                        &src,
                        &dst,
                    ));

                    ipv4pkg.set_checksum(0);
                    ipv4pkg.set_checksum(pnet_packet::ipv4::checksum(&ipv4pkg.to_immutable()));

                    libfw_log_debug!("Injecting IPv4 TCP RST packet {key:#?}");

                    ipv4pkgbuf[..IPV4_LEN].copy_from_slice(ipv4pkg.packet());

                    ipv4pkgbuf[IPV4_LEN..].copy_from_slice(tcppkg.packet());

                    inject_packet_cb(&ipv4pkgbuf);
                }
                (IpAddr::Ipv6(local_addr), IpAddr::Ipv6(remote_addr)) => {
                    let src = remote_addr.into();
                    let dst = local_addr.into();

                    ipv6pkg.set_source(src);
                    ipv6pkg.set_destination(dst);

                    tcppkg.set_checksum(pnet_packet::tcp::ipv6_checksum(
                        &tcppkg.to_immutable(),
                        &src,
                        &dst,
                    ));

                    libfw_log_debug!("Injecting IPv4 TCP RST packet {key:#?}");

                    ipv6pkgbuf[..IPV6_LEN].copy_from_slice(ipv6pkg.packet());

                    ipv6pkgbuf[IPV6_LEN..].copy_from_slice(tcppkg.packet());
                    inject_packet_cb(&ipv6pkgbuf);
                }
                _ => libfw_log_warn!(
                    "Local and remote IP addrs version missmatch, this should never happen"
                ),
            }
        }

        Ok(())
    }

    fn send_icmp_port_unreachable_packets<'a, I, F>(
        &self,
        connections: I,
        mut inject_packet_cb: F,
    ) -> Result<()>
    where
        I: IntoIterator<Item = (&'a IpConnWithPort, &'a [u8])>,
        F: FnMut(&[u8]),
    {
        const IPV4_HEADER_LEN: usize = 20;
        const IPV6_HEADER_LEN: usize = 40;
        const ICMP_HEADER_LEN: usize = 8;
        const ICMP_MAX_DATA_LEN: usize = UdpConnectionInfo::LAST_PKG_MAX_CHUNK_LEN;

        let mut ipv4pkgbuf = [0u8; IPV4_HEADER_LEN + ICMP_HEADER_LEN + ICMP_MAX_DATA_LEN];
        let mut ipv6pkgbuf = [0u8; IPV6_HEADER_LEN + ICMP_HEADER_LEN + ICMP_MAX_DATA_LEN];

        // Write most of the fields for ICMP and IP packets upfront
        #[allow(clippy::expect_used)]
        let mut ipv4pkg = MutableIpv4Packet::new(&mut ipv4pkgbuf[..IPV4_HEADER_LEN])
            .expect("IPv4 buffer should not be too small");
        ipv4pkg.set_version(4);
        ipv4pkg.set_header_length(5);
        ipv4pkg.set_flags(Ipv4Flags::DontFragment);
        ipv4pkg.set_ttl(0xFF);
        ipv4pkg.set_next_level_protocol(IpNextHeaderProtocols::Icmp);

        #[allow(clippy::expect_used)]
        let mut ipv6pkg = MutableIpv6Packet::new(&mut ipv6pkgbuf[..IPV6_HEADER_LEN])
            .expect("IPv6 buffer should not be too small");
        ipv6pkg.set_version(6);
        ipv6pkg.set_next_header(IpNextHeaderProtocols::Icmpv6);
        ipv6pkg.set_hop_limit(0xff);

        #[allow(clippy::expect_used)]
        let mut icmppkg = MutableIcmpPacket::new(
            &mut ipv4pkgbuf[IPV4_HEADER_LEN..(IPV4_HEADER_LEN + ICMP_HEADER_LEN)],
        )
        .expect("ICMP buffer should not be too small");
        icmppkg.set_icmp_type(IcmpTypes::DestinationUnreachable);
        icmppkg.set_icmp_code(IcmpCodes::DestinationPortUnreachable);

        #[allow(clippy::expect_used)]
        let mut icmpv6pkg = MutableIcmpv6Packet::new(
            &mut ipv6pkgbuf[IPV6_HEADER_LEN..(IPV6_HEADER_LEN + ICMP_HEADER_LEN)],
        )
        .expect("ICMP buffer should not be too small");
        icmpv6pkg.set_icmpv6_type(Icmpv6Types::DestinationUnreachable);
        let icmpv6_port_unreachable = 4;
        icmpv6pkg.set_icmpv6_code(Icmpv6Code::new(icmpv6_port_unreachable));

        for (key, last_headers) in connections {
            libfw_log_debug!("Killing UDP or ICMP stream {:?}", key);
            match (key.local_addr, key.remote_addr) {
                (IpAddr::Ipv4(local_addr), IpAddr::Ipv4(remote_addr)) => {
                    let end = IPV4_HEADER_LEN + ICMP_HEADER_LEN + last_headers.len();

                    let ipv4pkgbuf = &mut ipv4pkgbuf[..end];
                    ipv4pkgbuf[(IPV4_HEADER_LEN + ICMP_HEADER_LEN)..].copy_from_slice(last_headers);

                    let src = remote_addr.into();
                    let dst = local_addr.into();

                    #[allow(clippy::expect_used)]
                    let mut icmppkg = MutableIcmpPacket::new(&mut ipv4pkgbuf[IPV4_HEADER_LEN..])
                        .expect("ICMP buffer should not be too small");
                    icmppkg.set_checksum(0);
                    icmppkg.set_checksum(pnet_packet::icmp::checksum(&icmppkg.to_immutable()));
                    drop(icmppkg);

                    #[allow(clippy::expect_used)]
                    let mut ipv4pkg = MutableIpv4Packet::new(&mut ipv4pkgbuf[..IPV4_HEADER_LEN])
                        .expect("IPv4 buffer should not be too small");
                    ipv4pkg.set_source(src);
                    ipv4pkg.set_destination(dst);
                    ipv4pkg.set_total_length(
                        (IPV4_HEADER_LEN + ICMP_HEADER_LEN + last_headers.len()) as _,
                    );

                    ipv4pkg.set_checksum(0);
                    ipv4pkg.set_checksum(pnet_packet::ipv4::checksum(&ipv4pkg.to_immutable()));

                    libfw_log_debug!("Injecting IPv4 ICMP (for UDP) packet {key:#?}");
                    inject_packet_cb(ipv4pkgbuf);
                }
                (IpAddr::Ipv6(local_addr), IpAddr::Ipv6(remote_addr)) => {
                    let end = IPV6_HEADER_LEN + ICMP_HEADER_LEN + last_headers.len();

                    let ipv6pkgbuf = &mut ipv6pkgbuf[..end];
                    ipv6pkgbuf[(IPV6_HEADER_LEN + ICMP_HEADER_LEN)..].copy_from_slice(last_headers);

                    let src = remote_addr.into();
                    let dst = local_addr.into();

                    #[allow(clippy::expect_used)]
                    let mut icmpv6pkg =
                        MutableIcmpv6Packet::new(&mut ipv6pkgbuf[IPV6_HEADER_LEN..])
                            .expect("ICMPv6 buffer should not be too small");
                    icmpv6pkg.set_checksum(0);
                    icmpv6pkg.set_checksum(pnet_packet::icmpv6::checksum(
                        &icmpv6pkg.to_immutable(),
                        &src,
                        &dst,
                    ));

                    #[allow(clippy::expect_used)]
                    let mut ipv6pkg = MutableIpv6Packet::new(&mut ipv6pkgbuf[..IPV6_HEADER_LEN])
                        .expect("IPv4 buffer should not be too small");
                    ipv6pkg.set_source(src);
                    ipv6pkg.set_destination(dst);
                    ipv6pkg.set_payload_length((ICMP_HEADER_LEN + last_headers.len()) as _);

                    libfw_log_debug!("Injecting IPv6 ICMP (for UDP) packet {key:#?}");
                    inject_packet_cb(ipv6pkgbuf);
                }
                _ => libfw_log_warn!(
                    "Local and remote IP addrs version mismatch, this should never happen"
                ),
            }
        }

        Ok(())
    }

    pub(crate) fn reset_udp_conns<F>(
        &self,
        associated_data: Option<&[u8]>,
        inject_packet_cb: F,
    ) -> Result<()>
    where
        F: FnMut(&[u8]),
    {
        let mut udp_conn_cache = self.udp.lock();

        libfw_log_debug!(
            "Inspecting {} UDP connections for reset",
            udp_conn_cache.len()
        );

        let mut keys_to_remove = vec![];
        let iter = udp_conn_cache.iter().filter_map(|(k, v)| {
            // Skip connection without outbounded packages
            let last_headers = v.last_out_pkg_chunk.as_deref()?;

            match (&k.associated_data, associated_data) {
                (Some(conn_assoc_data), Some(assoc_data)) if &**conn_assoc_data == assoc_data => {
                    keys_to_remove.push(k.clone());
                    Some((&k.link, last_headers))
                }
                (None, None) => {
                    keys_to_remove.push(k.clone());
                    Some((&k.link, last_headers))
                }
                _ => None,
            }
        });

        let result = self.send_icmp_port_unreachable_packets(iter, inject_packet_cb);

        for key in keys_to_remove.iter() {
            udp_conn_cache.remove(key);
        }

        result
    }

    pub(crate) fn reject_outbound_ip_packet<'a, P: IpPacket<'a>, F>(
        &self,
        buffer: &'a [u8],
        inject_packet_cb: F,
    ) -> Result<()>
    where
        F: FnMut(&[u8]),
    {
        let ip = unwrap_option_or_return!(P::try_from(buffer), Err(Error::MalformedIpPacket));
        match ip.get_next_level_protocol() {
            IpNextHeaderProtocols::Udp => {
                let link = Conntrack::build_conn_info(&ip, Direction::Outbound)?.0;
                let Some(first_chunk) = ip
                    .packet()
                    .chunks(UdpConnectionInfo::LAST_PKG_MAX_CHUNK_LEN)
                    .next()
                else {
                    libfw_log_warn!("Failed to extract headers of UDP packet for {peer:?}");
                    return Err(Error::MalformedUdpPacket);
                };

                _ = self.send_icmp_port_unreachable_packets(
                    std::iter::once((&link, first_chunk)),
                    inject_packet_cb,
                );

                Ok(())
            }
            IpNextHeaderProtocols::Tcp => {
                let (link, tcp_packet) = Conntrack::build_conn_info(&ip, Direction::Outbound)?;
                let tcp_packet =
                    unwrap_option_or_return!(tcp_packet, Err(Error::MalformedTcpPacket));
                _ = self.send_tcp_rst_packets(
                    std::iter::once((&link, 0, Some(tcp_packet.get_sequence() + 1))),
                    inject_packet_cb,
                );
                Ok(())
            }
            _ => Err(Error::UnexpectedPacketType),
        }
    }

    pub(crate) fn track_inbound_ip_packet<'a, P: IpPacket<'a>>(
        &self,
        associated_data: Option<&[u8]>,
        buffer: &'a [u8],
    ) -> Result<ConnectionState> {
        let ip = unwrap_option_or_return!(P::try_from(buffer), Err(Error::MalformedIpPacket));
        let proto = ip.get_next_level_protocol();

        // Initially, we want to track the state of all of the connections
        match proto {
            IpNextHeaderProtocols::Udp => self.handle_inbound_udp(&ip, associated_data),
            IpNextHeaderProtocols::Tcp => self.handle_inbound_tcp(&ip, associated_data),
            IpNextHeaderProtocols::Icmp => self.handle_icmp(&ip, associated_data),
            IpNextHeaderProtocols::Icmpv6 => self.handle_icmp(&ip, associated_data),
            _ => Err(Error::UnexpectedProtocol),
        }
    }

    pub(crate) fn track_outbound_ip_packet<'a, P: IpPacket<'a>>(
        &self,
        associated_data: Option<&[u8]>,
        buffer: &'a [u8],
    ) -> Result<ConnectionState> {
        let ip = unwrap_option_or_return!(P::try_from(buffer), Err(Error::MalformedIpPacket));
        let proto = ip.get_next_level_protocol();

        // Initially, we want to track the state of all of the connections
        match proto {
            IpNextHeaderProtocols::Udp => self.handle_outbound_udp(&ip, associated_data),
            IpNextHeaderProtocols::Tcp => self.handle_outbound_tcp(&ip, associated_data),
            IpNextHeaderProtocols::Icmp => self.handle_icmp(&ip, associated_data),
            IpNextHeaderProtocols::Icmpv6 => self.handle_icmp(&ip, associated_data),
            _ => Err(Error::UnexpectedProtocol),
        }
    }
}

#[cfg(test)]
mod tests {
    use std::{
        convert::TryInto,
        ffi::{c_char, CStr},
        io::{self, Write},
        net::{Ipv4Addr, Ipv6Addr, SocketAddrV4, SocketAddrV6},
        time::Duration,
    };

    use pnet_packet::{
        icmp::{
            self,
            destination_unreachable::{self, DestinationUnreachablePacket},
            echo_reply::IcmpCodes,
            IcmpCode, IcmpPacket, IcmpType, IcmpTypes, MutableIcmpPacket,
        },
        icmpv6::{
            self, echo_reply::Icmpv6Codes, Icmpv6Code, Icmpv6Packet, Icmpv6Type, Icmpv6Types,
            MutableIcmpv6Packet,
        },
        ip::{IpNextHeaderProtocol, IpNextHeaderProtocols},
        ipv4::{Ipv4Packet, MutableIpv4Packet},
        ipv6::{Ipv6Packet, MutableIpv6Packet},
        tcp::{MutableTcpPacket, TcpFlags, TcpPacket},
        udp::MutableUdpPacket,
        MutablePacket, Packet,
    };

    use crate::{
        conntrack::{
            ConnectionState, Conntrack, LRU_CACHE_ICMP_TTL, LRU_CACHE_TCP_TTL, LRU_CACHE_UDP_TTL,
        },
        log::LibfwLogLevel,
        packet::IpPacket,
    };

    const IPV4_HEADER_MIN: usize = 20; // IPv4 header minimal length in bytes
    const IPV6_HEADER_MIN: usize = 40; // IPv6 header minimal length in bytes
    const TCP_HEADER_MIN: usize = 20; // TCP header minimal length in bytes
    const UDP_HEADER: usize = 8; // UDP header length in bytes
    const ICMP_HEADER: usize = 8; // ICMP header length in bytes
    const ICMPV6_HEADER: usize = 4; // ICMPv6 header length in bytes

    /// File sending port
    pub const FILE_SEND_PORT: u16 = 49111;

    fn advance_time(time: Duration) {
        sn_fake_clock::FakeClock::advance_time(time.as_millis() as u64);
    }

    fn set_ipv4(
        ip: &mut MutableIpv4Packet,
        protocol: IpNextHeaderProtocol,
        header_length: usize,
        total_length: usize,
    ) {
        ip.set_next_level_protocol(protocol);
        ip.set_version(4);
        ip.set_header_length((header_length / 4) as u8);
        ip.set_total_length(total_length.try_into().unwrap());
        ip.set_flags(2);
        ip.set_ttl(64);
        ip.set_source(Ipv4Addr::LOCALHOST);
        ip.set_destination(Ipv4Addr::new(8, 8, 8, 8));
    }

    fn set_ipv6(
        raw: &mut [u8],
        src: &Ipv6Addr,
        dst: &Ipv6Addr,
        protocol: IpNextHeaderProtocol,
        total_length: usize,
    ) {
        let mut ip = MutableIpv6Packet::new(raw).expect("ICMP: Bad IP buffer");
        ip.set_next_header(protocol);
        ip.set_version(6);
        ip.set_traffic_class(0);
        ip.set_flow_label(0);
        ip.set_payload_length((total_length - IPV6_HEADER_MIN).try_into().unwrap());
        ip.set_hop_limit(64);
        ip.set_source(*src);
        ip.set_destination(*dst);
    }

    pub fn make_udp(src: &str, dst: &str) -> Vec<u8> {
        let msg: &str = "Some message";
        let msg_len = msg.as_bytes().len();
        let ip_len = IPV4_HEADER_MIN + UDP_HEADER + msg_len;
        let mut raw = vec![0u8; ip_len];

        let mut ip = MutableIpv4Packet::new(&mut raw).expect("UDP: Bad IP buffer");
        set_ipv4(&mut ip, IpNextHeaderProtocols::Udp, IPV4_HEADER_MIN, ip_len);
        let source: SocketAddrV4 = src
            .parse()
            .expect(&format!("UDPv4: Bad src address: {src}"));
        let destination: SocketAddrV4 = dst
            .parse()
            .expect(&format!("UDPv4: Bad dst address: {dst}"));
        ip.set_source(*source.ip());
        ip.set_destination(*destination.ip());

        let mut udp =
            MutableUdpPacket::new(&mut raw[IPV4_HEADER_MIN..]).expect("UDP: Bad UDP buffer");
        udp.set_source(source.port());
        udp.set_destination(destination.port());
        udp.set_length((UDP_HEADER + msg_len) as u16);
        udp.set_checksum(0);
        udp.payload_mut().copy_from_slice(msg.as_bytes());

        raw
    }

    pub fn make_udp6(src: &str, dst: &str) -> Vec<u8> {
        let msg: &str = "Some message";

        let msg_len = msg.as_bytes().len();
        let ip_len = IPV6_HEADER_MIN + UDP_HEADER + msg_len;
        let mut raw = vec![0u8; ip_len];

        let source: SocketAddrV6 = src
            .parse()
            .expect(&format!("UDPv6: Bad src address: {src}"));
        let destination: SocketAddrV6 = dst
            .parse()
            .expect(&format!("UDPv6: Bad dst address: {dst}"));

        set_ipv6(
            &mut raw,
            source.ip(),
            destination.ip(),
            IpNextHeaderProtocols::Udp,
            ip_len,
        );

        let mut udp =
            MutableUdpPacket::new(&mut raw[IPV6_HEADER_MIN..]).expect("UDP: Bad UDP buffer");
        udp.set_source(source.port());
        udp.set_destination(destination.port());
        udp.set_length((UDP_HEADER + msg_len) as u16);
        udp.set_checksum(0);
        udp.payload_mut().copy_from_slice(msg.as_bytes());

        raw
    }

    pub fn make_tcp(src: &str, dst: &str, flags: u8) -> Vec<u8> {
        let msg: &str = "Some message";
        let msg_len = msg.as_bytes().len();
        let ip_len = IPV4_HEADER_MIN + TCP_HEADER_MIN + msg_len;
        let mut raw = vec![0u8; ip_len];

        let mut ip = MutableIpv4Packet::new(&mut raw).expect("TCP: Bad IP buffer");
        set_ipv4(&mut ip, IpNextHeaderProtocols::Tcp, IPV4_HEADER_MIN, ip_len);
        let source: SocketAddrV4 = src
            .parse()
            .expect(&format!("TCPv4: Bad src address: {src}"));
        let destination: SocketAddrV4 = dst
            .parse()
            .expect(&format!("TCPv4: Bad dst address: {dst}"));
        ip.set_source(*source.ip());
        ip.set_destination(*destination.ip());

        let mut tcp =
            MutableTcpPacket::new(&mut raw[IPV4_HEADER_MIN..]).expect("TCP: Bad TCP buffer");
        tcp.set_source(source.port());
        tcp.set_destination(destination.port());
        tcp.set_checksum(0);
        tcp.payload_mut().copy_from_slice(msg.as_bytes());
        tcp.set_flags(flags);

        raw
    }

    pub fn make_tcp6(src: &str, dst: &str, flags: u8) -> Vec<u8> {
        let msg: &str = "Some message";
        let msg_len = msg.as_bytes().len();
        let ip_len = IPV6_HEADER_MIN + TCP_HEADER_MIN + msg_len;
        let mut raw = vec![0u8; ip_len];

        let source: SocketAddrV6 = src
            .parse()
            .expect(&format!("TCPv6: Bad src address: {src}"));
        let destination: SocketAddrV6 = dst
            .parse()
            .expect(&format!("TCPv6: Bad dst address: {dst}"));

        set_ipv6(
            &mut raw,
            source.ip(),
            destination.ip(),
            IpNextHeaderProtocols::Tcp,
            ip_len,
        );

        let mut tcp =
            MutableTcpPacket::new(&mut raw[IPV6_HEADER_MIN..]).expect("TCP: Bad TCP buffer");
        tcp.set_source(source.port());
        tcp.set_destination(destination.port());
        tcp.set_checksum(0);
        tcp.payload_mut().copy_from_slice(msg.as_bytes());
        tcp.set_flags(flags);

        raw
    }

    fn make_icmp4_with_body(
        src: Ipv4Addr,
        dst: Ipv4Addr,
        icmp_type: IcmpType,
        icmp_code: IcmpCode,
        body: &[u8],
        fix_checksum: bool,
    ) -> Vec<u8> {
        // Because the last 4 bytes of ICMP header may differ depending on the packet type, pnet uses
        // first 4 bytes of payload for them, so they would be counted in the packet length twice
        let ip_len = IPV4_HEADER_MIN + ICMP_HEADER + body.len().saturating_sub(4);
        let mut raw = vec![0u8; ip_len];

        let mut packet =
            MutableIcmpPacket::new(&mut raw[IPV4_HEADER_MIN..]).expect("ICMP: Bad ICMP buffer");
        packet.set_icmp_type(icmp_type);
        packet.set_icmp_code(icmp_code);
        packet.set_payload(&body);
        packet.set_checksum(0);

        let icmp_packet =
            IcmpPacket::new(packet.packet()).expect("It should be a valid ICMP packet");

        if fix_checksum {
            packet.set_checksum(icmp::checksum(&icmp_packet));
        }

        let mut ip = MutableIpv4Packet::new(&mut raw).expect("ICMP: Bad IP buffer");
        set_ipv4(
            &mut ip,
            IpNextHeaderProtocols::Icmp,
            IPV4_HEADER_MIN,
            ip_len,
        );
        ip.set_source(src);
        ip.set_destination(dst);

        raw
    }

    fn make_icmp4(
        src: Ipv4Addr,
        dst: Ipv4Addr,
        icmp_type: IcmpType,
        icmp_code: IcmpCode,
    ) -> Vec<u8> {
        make_icmp4_with_body(src, dst, icmp_type, icmp_code, &[], true)
    }

    fn make_icmp6_with_body(
        src: Ipv6Addr,
        dst: Ipv6Addr,
        icmp_type: Icmpv6Type,
        icmp_code: Icmpv6Code,
        body: &[u8],
        fix_checksum: bool,
    ) -> Vec<u8> {
        let ip_len = IPV6_HEADER_MIN + ICMPV6_HEADER + body.len();
        let mut raw = vec![0u8; ip_len];

        set_ipv6(&mut raw, &src, &dst, IpNextHeaderProtocols::Icmpv6, ip_len);

        let mut packet =
            MutableIcmpv6Packet::new(&mut raw[IPV6_HEADER_MIN..]).expect("ICMP: Bad ICMP buffer");
        packet.set_icmpv6_type(icmp_type);
        packet.set_icmpv6_code(icmp_code);
        packet.set_payload(body);
        packet.set_checksum(0);

        if fix_checksum {
            let icmp_packet =
                Icmpv6Packet::new(packet.packet()).expect("It should be a valid ICMP packet");
            packet.set_checksum(icmpv6::checksum(&icmp_packet, &src, &dst));
        }

        raw
    }

    /// Helper to create an ICMP Echo Request packet
    fn make_icmp4_echo_request(
        src: Ipv4Addr,
        dst: Ipv4Addr,
        identifier: u16,
        sequence: u16,
    ) -> Vec<u8> {
        let id_bytes = identifier.to_ne_bytes();
        let seq_bytes = sequence.to_ne_bytes();
        make_icmp4_with_body(
            src,
            dst,
            IcmpTypes::EchoRequest,
            IcmpCodes::NoCode,
            &[id_bytes[0], id_bytes[1], seq_bytes[0], seq_bytes[1]],
            true,
        )
    }

    /// Helper to create an ICMP Echo Reply packet
    fn make_icmp4_echo_reply(
        src: Ipv4Addr,
        dst: Ipv4Addr,
        identifier: u16,
        sequence: u16,
    ) -> Vec<u8> {
        let id_bytes = identifier.to_ne_bytes();
        let seq_bytes = sequence.to_ne_bytes();
        make_icmp4_with_body(
            src,
            dst,
            IcmpTypes::EchoReply,
            IcmpCodes::NoCode,
            &[id_bytes[0], id_bytes[1], seq_bytes[0], seq_bytes[1]],
            true,
        )
    }

    /// Helper to create an ICMPv6 Echo Request packet
    fn make_icmp6_echo_request(
        src: Ipv6Addr,
        dst: Ipv6Addr,
        identifier: u16,
        sequence: u16,
    ) -> Vec<u8> {
        let id_bytes = identifier.to_ne_bytes();
        let seq_bytes = sequence.to_ne_bytes();
        make_icmp6_with_body(
            src,
            dst,
            Icmpv6Types::EchoRequest,
            Icmpv6Codes::NoCode,
            &[id_bytes[0], id_bytes[1], seq_bytes[0], seq_bytes[1]],
            true,
        )
    }

    /// Helper to create an ICMPv6 Echo Reply packet
    fn make_icmp6_echo_reply(
        src: Ipv6Addr,
        dst: Ipv6Addr,
        identifier: u16,
        sequence: u16,
    ) -> Vec<u8> {
        let id_bytes = identifier.to_ne_bytes();
        let seq_bytes = sequence.to_ne_bytes();
        make_icmp6_with_body(
            src,
            dst,
            Icmpv6Types::EchoReply,
            Icmpv6Codes::NoCode,
            &[id_bytes[0], id_bytes[1], seq_bytes[0], seq_bytes[1]],
            true,
        )
    }

    fn handle_inbound_ipv4_packet(
        ctk: &Conntrack,
        packet: &[u8],
        assoc_data: Option<&[u8]>,
    ) -> ConnectionState {
        ctk.track_inbound_ip_packet::<Ipv4Packet>(assoc_data, packet)
            .expect("Unexpected conntrack error")
    }

    fn handle_outbound_ipv4_packet(
        ctk: &Conntrack,
        packet: &[u8],
        assoc_data: Option<&[u8]>,
    ) -> ConnectionState {
        ctk.track_outbound_ip_packet::<Ipv4Packet>(assoc_data, packet)
            .expect("Unexpected conntrack error")
    }

    fn handle_inbound_ipv6_packet(
        ctk: &Conntrack,
        packet: &[u8],
        assoc_data: Option<&[u8]>,
    ) -> ConnectionState {
        ctk.track_inbound_ip_packet::<Ipv6Packet>(assoc_data, packet)
            .expect("Unexpected conntrack error")
    }

    fn handle_outbound_ipv6_packet(
        ctk: &Conntrack,
        packet: &[u8],
        assoc_data: Option<&[u8]>,
    ) -> ConnectionState {
        ctk.track_outbound_ip_packet::<Ipv6Packet>(assoc_data, packet)
            .expect("Unexpected conntrack error")
    }

    #[test]
    fn conntrack_ipv4_packet_validation_outbound() {
        let conntrack = Conntrack::new();

        let src_ip = Ipv4Addr::new(127, 0, 0, 1);
        let dst_ip = Ipv4Addr::new(8, 8, 8, 8);

        let mut raw = make_icmp4(src_ip, dst_ip, IcmpTypes::EchoRequest, IcmpCodes::NoCode);
        let mut ip = MutableIpv4Packet::new(&mut raw).expect("PRE: Bad IP buffer");

        assert_eq!(
            conntrack.track_outbound_ip_packet::<Ipv4Packet>(None, ip.packet()),
            Ok(ConnectionState::New)
        );

        ip.set_version(4);
        assert_eq!(
            conntrack.track_outbound_ip_packet::<Ipv4Packet>(None, ip.packet()),
            Ok(ConnectionState::New)
        );

        ip.set_version(0); // Invalid IP version
        assert_eq!(
            conntrack.track_outbound_ip_packet::<Ipv4Packet>(None, ip.packet()),
            Err(crate::error::Error::MalformedIpPacket)
        );

        ip.set_version(6); // Only Ipv4 supported
        assert_eq!(
            conntrack.track_outbound_ip_packet::<Ipv4Packet>(None, ip.packet()),
            Err(crate::error::Error::MalformedIpPacket)
        );

        ip.set_version(4);
        ip.set_header_length(4); // Ipv4->IHL must be [5..15]
        assert_eq!(
            conntrack.track_outbound_ip_packet::<Ipv4Packet>(None, ip.packet()),
            Err(crate::error::Error::MalformedIpPacket)
        );
    }

    #[test]
    fn conntrack_ipv4_packet_validation_inbound() {
        let conntrack = Conntrack::new();

        let src_ip = Ipv4Addr::new(8, 8, 8, 8);
        let dst_ip = Ipv4Addr::new(127, 0, 0, 1);

        let mut raw = make_icmp4(src_ip, dst_ip, IcmpTypes::EchoRequest, IcmpCodes::NoCode);
        let mut ip = MutableIpv4Packet::new(&mut raw).expect("PRE: Bad IP buffer");

        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv4Packet>(None, ip.packet()),
            Ok(ConnectionState::New)
        );

        ip.set_version(4);
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv4Packet>(None, ip.packet()),
            Ok(ConnectionState::New)
        );

        ip.set_version(0); // Invalid IP version
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv4Packet>(None, ip.packet()),
            Err(crate::error::Error::MalformedIpPacket)
        );

        ip.set_version(6); // Only Ipv4 supported
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv4Packet>(None, ip.packet()),
            Err(crate::error::Error::MalformedIpPacket)
        );

        ip.set_version(4);
        ip.set_header_length(4); // Ipv4->IHL must be [5..15]
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv4Packet>(None, ip.packet()),
            Err(crate::error::Error::MalformedIpPacket)
        );
    }

    #[test]
    fn conntrack_ipv6_packet_validation_outbound() {
        let conntrack = Conntrack::new();

        let src = Ipv6Addr::new(0, 0, 0, 0, 0, 0, 0, 1);
        let dst = Ipv6Addr::new(0xfc, 0x0a, 0, 0, 0, 0, 0, 1);

        let mut raw = make_icmp6_with_body(
            src,
            dst,
            Icmpv6Types::EchoRequest.into(),
            Icmpv6Codes::NoCode,
            &[1, 2, 3, 4],
            true,
        );
        let mut ip = MutableIpv6Packet::new(&mut raw).expect("PRE: Bad IP buffer");

        assert_eq!(
            conntrack.track_outbound_ip_packet::<Ipv6Packet>(None, ip.packet()),
            Ok(ConnectionState::New)
        );

        ip.set_version(6);
        assert_eq!(
            conntrack.track_outbound_ip_packet::<Ipv6Packet>(None, ip.packet()),
            Ok(ConnectionState::New)
        );

        let payload_len = ip.get_payload_length();
        ip.set_payload_length(payload_len + 1);
        assert_eq!(
            conntrack.track_outbound_ip_packet::<Ipv6Packet>(None, ip.packet()),
            Err(crate::error::Error::MalformedIpPacket)
        );

        ip.set_payload_length(payload_len - 1);
        assert_eq!(
            conntrack.track_outbound_ip_packet::<Ipv6Packet>(None, ip.packet()),
            Err(crate::error::Error::MalformedIpPacket)
        );

        // Return to original payload length
        ip.set_payload_length(payload_len);

        ip.set_version(0); // Invalid IP version
        assert_eq!(
            conntrack.track_outbound_ip_packet::<Ipv6Packet>(None, ip.packet()),
            Err(crate::error::Error::MalformedIpPacket)
        );

        ip.set_version(4); // Only Ipv6 supported
        assert_eq!(
            conntrack.track_outbound_ip_packet::<Ipv6Packet>(None, ip.packet()),
            Err(crate::error::Error::MalformedIpPacket)
        );

        // With original payload length and corrent version it should work again
        ip.set_version(6); // Only Ipv6 supported
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv6Packet>(None, ip.packet()),
            Ok(ConnectionState::New)
        );
    }

    #[test]
    fn conntrack_ipv6_packet_validation_inbound() {
        let conntrack = Conntrack::new();

        let src = Ipv6Addr::new(0xfc, 0x0a, 0, 0, 0, 0, 0, 1);
        let dst = Ipv6Addr::new(0, 0, 0, 0, 0, 0, 0, 1);

        let mut raw = make_icmp6_with_body(
            src,
            dst,
            Icmpv6Types::EchoRequest.into(),
            Icmpv6Codes::NoCode,
            &[1, 2, 3, 4],
            true,
        );
        let mut ip = MutableIpv6Packet::new(&mut raw).expect("PRE: Bad IP buffer");

        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv6Packet>(None, ip.packet()),
            Ok(ConnectionState::New)
        );

        ip.set_version(6);
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv6Packet>(None, ip.packet()),
            Ok(ConnectionState::New)
        );

        let payload_len = ip.get_payload_length();
        ip.set_payload_length(payload_len + 1);
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv6Packet>(None, ip.packet()),
            Err(crate::error::Error::MalformedIpPacket)
        );

        ip.set_payload_length(payload_len - 1);
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv6Packet>(None, ip.packet()),
            Err(crate::error::Error::MalformedIpPacket)
        );

        // Return to original payload length
        ip.set_payload_length(payload_len);

        ip.set_version(0); // Invalid IP version
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv6Packet>(None, ip.packet()),
            Err(crate::error::Error::MalformedIpPacket)
        );

        ip.set_version(4); // Only Ipv6 supported
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv6Packet>(None, ip.packet()),
            Err(crate::error::Error::MalformedIpPacket)
        );

        // With original payload length and corrent version it should work again
        ip.set_version(6); // Only Ipv6 supported
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv6Packet>(None, ip.packet()),
            Ok(ConnectionState::New)
        );
    }

    #[test]
    fn conntrack_ipv4_validation_max_header_inbound() {
        const IPV4_MAX_IHL: usize = 15; //Ipv4->IHL is 4 bits, can be [5..15]
        const HEADER_LENGTH: usize = 4 * IPV4_MAX_IHL;
        const PACKET_LENGTH: usize = HEADER_LENGTH + UDP_HEADER;

        let conntrack = Conntrack::new();

        let mut raw = [0u8; PACKET_LENGTH];
        let mut ip = MutableIpv4Packet::new(&mut raw).expect("PRE: Bad IP buffer");

        set_ipv4(
            &mut ip,
            IpNextHeaderProtocols::Udp,
            HEADER_LENGTH,
            PACKET_LENGTH,
        );

        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv4Packet>(None, &raw),
            Ok(ConnectionState::New)
        );
    }

    #[test]
    fn conntrack_ipv4_validation_max_header_outbound() {
        const IPV4_MAX_IHL: usize = 15; //Ipv4->IHL is 4 bits, can be [5..15]
        const HEADER_LENGTH: usize = 4 * IPV4_MAX_IHL;
        const PACKET_LENGTH: usize = HEADER_LENGTH + UDP_HEADER;

        let conntrack = Conntrack::new();

        let mut raw = [0u8; PACKET_LENGTH];
        let mut ip = MutableIpv4Packet::new(&mut raw).expect("PRE: Bad IP buffer");

        set_ipv4(
            &mut ip,
            IpNextHeaderProtocols::Udp,
            HEADER_LENGTH,
            PACKET_LENGTH,
        );

        assert_eq!(
            conntrack.track_outbound_ip_packet::<Ipv4Packet>(None, &raw),
            Ok(ConnectionState::New)
        );
    }

    #[test]
    fn conntrack_establishing_outbound_udp_connection() {
        let conntrack = Conntrack::new();

        let src = "127.0.0.1:1111";
        let dst = "8.8.8.8:8888";

        let outbound_udp_packet = make_udp(src, dst);
        let inbound_udp_packet = make_udp(dst, src);

        // Initiate the connection
        let conn_state = conntrack
            .track_outbound_ip_packet::<Ipv4Packet>(None, &outbound_udp_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::New);
        assert_eq!(conntrack.udp.lock().len(), 1);

        // Another outbound packet
        let conn_state = conntrack
            .track_outbound_ip_packet::<Ipv4Packet>(None, &outbound_udp_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::New);
        assert_eq!(conntrack.udp.lock().len(), 1);

        // Inbound packet should change conn state to established
        let conn_state = conntrack
            .track_inbound_ip_packet::<Ipv4Packet>(None, &inbound_udp_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::Established);
        assert_eq!(conntrack.udp.lock().len(), 1);
    }

    #[test]
    fn conntrack_establishing_inbound_udp_connection() {
        let conntrack = Conntrack::new();

        let src = "127.0.0.1:1111";
        let dst = "8.8.8.8:8888";

        let outbound_udp_packet = make_udp(src, dst);
        let inbound_udp_packet = make_udp(dst, src);

        // Initiate the connection
        let conn_state = conntrack
            .track_inbound_ip_packet::<Ipv4Packet>(None, &inbound_udp_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::New);
        assert_eq!(conntrack.udp.lock().len(), 1);

        // Another outbound packet
        let conn_state = conntrack
            .track_inbound_ip_packet::<Ipv4Packet>(None, &inbound_udp_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::New);
        assert_eq!(conntrack.udp.lock().len(), 1);

        // Inbound packet should change conn state to established
        let conn_state = conntrack
            .track_outbound_ip_packet::<Ipv4Packet>(None, &outbound_udp_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::Established);
        assert_eq!(conntrack.udp.lock().len(), 1);
    }

    #[rustfmt::skip]
    #[test]
    fn multiple_udp_connections() {
        struct TestInput {
            src1: &'static str,
            src2: &'static str,
            src3: &'static str,
            src4: &'static str,
            src5: &'static str,
            dst1: &'static str,
            dst2: &'static str,
            make_udp: fn(&str, &str) -> Vec<u8>,
            handle_inbound_packet: fn(&Conntrack, &[u8], Option<&[u8]>) -> ConnectionState,
            handle_outbound_packet: fn(&Conntrack, &[u8], Option<&[u8]>) -> ConnectionState,
        }

        let test_inputs = vec![
            TestInput{
                src1: "127.0.0.1:1111", src2: "127.0.0.1:2222",
                src3: "127.0.0.1:3333", src4: "192.168.0.2:4444",
                src5: "192.168.0.2:5555",
                dst1: "8.8.8.8:8888", dst2: "192.168.0.1:7777",
                make_udp: make_udp,
                handle_inbound_packet: handle_inbound_ipv4_packet,
                handle_outbound_packet: handle_outbound_ipv4_packet,
            },
            TestInput{
                src1: "[::1]:1111", src2: "[::1]:2222",
                src3: "[::1]:3333", src4: "[fe80::2]:4444",
                src5: "[fe80::2]:5555",
                dst1: "[2001:4860:4860::8888]:8888",
                dst2: "[2001:4860:4860::7777]:7777",
                make_udp: make_udp6,
                handle_inbound_packet: handle_inbound_ipv6_packet,
                handle_outbound_packet: handle_outbound_ipv6_packet,
            },
        ];

        for TestInput { src1, src2, src3, src4, src5, dst1, dst2, make_udp, handle_inbound_packet, handle_outbound_packet } in test_inputs {
            let ctk = Conntrack::new_with_capacity_and_ttl(3, LRU_CACHE_TCP_TTL, LRU_CACHE_UDP_TTL, LRU_CACHE_ICMP_TTL);

            assert_eq!(handle_inbound_packet(&ctk, &make_udp(dst1, src1), None), ConnectionState::New);
            assert_eq!(ctk.udp.lock().len(), 1);
            assert_eq!(handle_inbound_packet(&ctk, &make_udp(dst1, src2), None), ConnectionState::New);
            assert_eq!(ctk.udp.lock().len(), 2);
            assert_eq!(handle_inbound_packet(&ctk, &make_udp(dst1, src3), None), ConnectionState::New);
            assert_eq!(ctk.udp.lock().len(), 3);
            assert_eq!(handle_inbound_packet(&ctk, &make_udp(dst1, src4), None), ConnectionState::New);
            assert_eq!(ctk.udp.lock().len(), 3);

            assert_eq!(handle_outbound_packet(&ctk, &make_udp(src1, dst1), None), ConnectionState::New);
            assert_eq!(handle_outbound_packet(&ctk, &make_udp(src2, dst1), None), ConnectionState::New);
            assert_eq!(handle_outbound_packet(&ctk, &make_udp(src1, dst1), None), ConnectionState::New);
            assert_eq!(ctk.udp.lock().len(), 3);

            assert_eq!(handle_outbound_packet(&ctk, &make_udp(src3, dst1), None), ConnectionState::New);
            assert_eq!(handle_outbound_packet(&ctk, &make_udp(src1, dst1), None), ConnectionState::New);
            assert_eq!(ctk.udp.lock().len(), 3);

            assert_eq!(handle_outbound_packet(&ctk, &make_udp(src4, dst1), None), ConnectionState::New);
            assert_eq!(handle_outbound_packet(&ctk, &make_udp(src1, dst1), None), ConnectionState::New);
            assert_eq!(ctk.udp.lock().len(), 3);

            assert_eq!(handle_inbound_packet(&ctk, &make_udp(dst1, src3), None), ConnectionState::Established);
            assert_eq!(handle_inbound_packet(&ctk, &make_udp(dst1, src4), None), ConnectionState::Established);
            assert_eq!(handle_inbound_packet(&ctk, &make_udp(dst1, src1), None), ConnectionState::Established);
            assert_eq!(ctk.udp.lock().len(), 3);

            assert_eq!(handle_inbound_packet(&ctk, &make_udp(dst1, src2), None), ConnectionState::New);
            assert_eq!(handle_inbound_packet(&ctk, &make_udp(dst1, src2), None), ConnectionState::New);
            assert_eq!(ctk.udp.lock().len(), 3);

            assert_eq!(handle_inbound_packet(&ctk, &make_udp(dst1, src5), None), ConnectionState::New);
            assert_eq!(handle_inbound_packet(&ctk, &make_udp(dst2, src1), None), ConnectionState::New);
            assert_eq!(handle_inbound_packet(&ctk, &make_udp(dst2, src1), None), ConnectionState::New);
            assert_eq!(ctk.udp.lock().len(), 3);
        }
    }

    #[test]
    fn conntrack_establishing_outbound_tcp_connection() {
        let conntrack = Conntrack::new();

        let src = "127.0.0.1:1111";
        let dst = "8.8.8.8:8888";

        let outbound_syn_packet = make_tcp(src, dst, TcpFlags::SYN);
        let outbound_syn_ack_packet = make_tcp(src, dst, TcpFlags::SYN | TcpFlags::ACK);
        let inbound_syn_ack_packet = make_tcp(dst, src, TcpFlags::SYN | TcpFlags::ACK);
        let inbound_fin_packet = make_tcp(dst, src, TcpFlags::FIN);
        let outbound_fin_packet = make_tcp(src, dst, TcpFlags::FIN);
        let inbound_ack_packet = make_tcp(dst, src, TcpFlags::ACK);

        // Initiate the connection
        let conn_state = conntrack
            .track_outbound_ip_packet::<Ipv4Packet>(None, &outbound_syn_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::New);
        assert_eq!(conntrack.tcp.lock().len(), 1);

        // Second oubound SYN packet shouldn't change conn state to established
        let conn_state = conntrack
            .track_outbound_ip_packet::<Ipv4Packet>(None, &outbound_syn_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::New);
        assert_eq!(conntrack.tcp.lock().len(), 1);

        // Outbound SYN-ACK packet shouldn't change the state to established
        let conn_state = conntrack
            .track_outbound_ip_packet::<Ipv4Packet>(None, &outbound_syn_ack_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::New);
        assert_eq!(conntrack.tcp.lock().len(), 1);

        // Inbound packet should change conn state to established
        let conn_state = conntrack
            .track_inbound_ip_packet::<Ipv4Packet>(None, &inbound_syn_ack_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::Established);
        assert_eq!(conntrack.tcp.lock().len(), 1);

        // First FIN packet should still get state Established
        let conn_state = conntrack
            .track_inbound_ip_packet::<Ipv4Packet>(None, &inbound_fin_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::Established);
        assert_eq!(conntrack.tcp.lock().len(), 1);

        // Second FIN from the same direction should also get Established
        let conn_state = conntrack
            .track_inbound_ip_packet::<Ipv4Packet>(None, &inbound_fin_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::Established);
        assert_eq!(conntrack.tcp.lock().len(), 1);

        // Inbound FIN should change connection state to closed
        let conn_state = conntrack
            .track_outbound_ip_packet::<Ipv4Packet>(None, &outbound_fin_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::Closed);
        assert_eq!(conntrack.tcp.lock().len(), 1);

        // Inbound ACK should remove the connection entry
        let conn_state = conntrack
            .track_inbound_ip_packet::<Ipv4Packet>(None, &inbound_ack_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::Closed);
        assert_eq!(conntrack.tcp.lock().len(), 0);
    }

    #[test]
    fn conntrack_establishing_inbound_tcp_connection() {
        let conntrack = Conntrack::new();

        let src = "127.0.0.1:1111";
        let dst = "8.8.8.8:8888";

        let inbound_syn_packet = make_tcp(dst, src, TcpFlags::SYN);
        let inbound_syn_ack_packet = make_tcp(dst, src, TcpFlags::SYN | TcpFlags::ACK);
        let outbound_syn_ack_packet = make_tcp(src, dst, TcpFlags::SYN | TcpFlags::ACK);
        let outbound_fin_packet = make_tcp(src, dst, TcpFlags::FIN);
        let inbound_fin_packet = make_tcp(dst, src, TcpFlags::FIN);

        // Initiate the connection
        let conn_state = conntrack
            .track_inbound_ip_packet::<Ipv4Packet>(None, &inbound_syn_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::New);
        assert_eq!(conntrack.tcp.lock().len(), 1);

        let conn_state = conntrack
            .track_inbound_ip_packet::<Ipv4Packet>(None, &inbound_syn_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::New);
        assert_eq!(conntrack.tcp.lock().len(), 1);

        // Inbound SYN-ACK packet shouldn't change the state to established
        let conn_state = conntrack
            .track_inbound_ip_packet::<Ipv4Packet>(None, &inbound_syn_ack_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::New);
        assert_eq!(conntrack.tcp.lock().len(), 1);

        // Outbound packet should change conn state to established
        let conn_state = conntrack
            .track_outbound_ip_packet::<Ipv4Packet>(None, &outbound_syn_ack_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::Established);
        assert_eq!(conntrack.tcp.lock().len(), 1);

        // First FIN packet should still get state Established
        let conn_state = conntrack
            .track_outbound_ip_packet::<Ipv4Packet>(None, &outbound_fin_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::Established);
        assert_eq!(conntrack.tcp.lock().len(), 1);

        // Second FIN from the same direction should also get Established
        let conn_state = conntrack
            .track_outbound_ip_packet::<Ipv4Packet>(None, &outbound_fin_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::Established);
        assert_eq!(conntrack.tcp.lock().len(), 1);

        // Inbound FIN should change connection state to closed
        let conn_state = conntrack
            .track_inbound_ip_packet::<Ipv4Packet>(None, &inbound_fin_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::Closed);
        assert_eq!(conntrack.tcp.lock().len(), 1);
    }

    #[test]
    fn conntrack_remote_initiated_tcp_stays_in_new_if_no_outbound_synack() {
        struct TestInput {
            src: &'static str,
            dst: &'static str,
            make_tcp: fn(&str, &str, u8) -> Vec<u8>,
            handle_inbound_packet: fn(&Conntrack, &[u8], Option<&[u8]>) -> ConnectionState,
            handle_outbound_packet: fn(&Conntrack, &[u8], Option<&[u8]>) -> ConnectionState,
        }

        let test_inputs = [
            TestInput {
                src: "127.0.0.1:1111",
                dst: "8.8.8.8:8888",
                make_tcp: make_tcp,
                handle_inbound_packet: handle_inbound_ipv4_packet,
                handle_outbound_packet: handle_outbound_ipv4_packet,
            },
            TestInput {
                src: "[::1]:1111",
                dst: "[2001:4860:4860::8888]:8888",
                make_tcp: make_tcp6,
                handle_inbound_packet: handle_inbound_ipv6_packet,
                handle_outbound_packet: handle_outbound_ipv6_packet,
            },
        ];

        for outbound_flags in
            (0..u8::max_value()).filter(|flags| *flags & (TcpFlags::SYN | TcpFlags::ACK) == 0)
        {
            for TestInput {
                src,
                dst,
                make_tcp,
                handle_inbound_packet,
                handle_outbound_packet,
            } in &test_inputs
            {
                let conntrack = Conntrack::new();

                assert_eq!(
                    handle_inbound_packet(&conntrack, &make_tcp(dst, src, TcpFlags::SYN), None),
                    ConnectionState::New
                );

                assert_eq!(
                    handle_outbound_packet(&conntrack, &make_tcp(src, dst, outbound_flags), None),
                    ConnectionState::New
                );
            }
        }
    }

    #[test]
    fn conntrack_locally_initiated_tcp_stays_in_new_if_no_outbound_synack() {
        struct TestInput {
            src: &'static str,
            dst: &'static str,
            make_tcp: fn(&str, &str, u8) -> Vec<u8>,
            handle_inbound_packet: fn(&Conntrack, &[u8], Option<&[u8]>) -> ConnectionState,
            handle_outbound_packet: fn(&Conntrack, &[u8], Option<&[u8]>) -> ConnectionState,
        }

        let test_inputs = [
            TestInput {
                src: "8.8.8.8:8888",
                dst: "127.0.0.1:1111",
                make_tcp: make_tcp,
                handle_inbound_packet: handle_inbound_ipv4_packet,
                handle_outbound_packet: handle_outbound_ipv4_packet,
            },
            TestInput {
                src: "[2001:4860:4860::8888]:8888",
                dst: "[::1]:1111",
                make_tcp: make_tcp6,
                handle_inbound_packet: handle_inbound_ipv6_packet,
                handle_outbound_packet: handle_outbound_ipv6_packet,
            },
        ];

        for outbound_flags in
            (0..u8::max_value()).filter(|flags| *flags & (TcpFlags::SYN | TcpFlags::ACK) == 0)
        {
            for TestInput {
                src,
                dst,
                make_tcp,
                handle_inbound_packet,
                handle_outbound_packet,
            } in &test_inputs
            {
                let conntrack = Conntrack::new();

                assert_eq!(
                    handle_outbound_packet(&conntrack, &make_tcp(dst, src, TcpFlags::SYN), None),
                    ConnectionState::New
                );

                assert_eq!(
                    handle_inbound_packet(&conntrack, &make_tcp(src, dst, outbound_flags), None),
                    ConnectionState::New
                );
            }
        }
    }

    #[rustfmt::skip]
    #[test]
    fn multiple_tcp_connections() {
        struct TestInput {
            src1: &'static str,
            src2: &'static str,
            src3: &'static str,
            src4: &'static str,
            src5: &'static str,
            dst1: &'static str,
            dst2: &'static str,
            make_tcp: fn(&str, &str, u8) -> Vec<u8>,
            handle_inbound_packet: fn(&Conntrack, &[u8], Option<&[u8]>) -> ConnectionState,
            handle_outbound_packet: fn(&Conntrack, &[u8], Option<&[u8]>) -> ConnectionState,
        }
        

        let test_inputs = vec![
            TestInput{
                src1: "127.0.0.1:1111", src2: "127.0.0.1:2222",
                src3: "127.0.0.1:3333", src4: "127.0.0.1:4444",
                src5: "127.0.0.1:5555",
                dst1: "8.8.8.8:8888", dst2: "8.8.8.8:7777",
                make_tcp: make_tcp,
                handle_inbound_packet: handle_inbound_ipv4_packet,
                handle_outbound_packet: handle_outbound_ipv4_packet,
            },
            TestInput{
                src1: "[::1]:1111", src2: "[::1]:2222",
                src3: "[::1]:3333", src4: "[::1]:4444",
                src5: "[::1]:5555",
                dst1: "[2001:4860:4860::8888]:8888",
                dst2: "[2001:4860:4860::8888]:7777",
                make_tcp: make_tcp6,
                handle_inbound_packet: handle_inbound_ipv6_packet,
                handle_outbound_packet: handle_outbound_ipv6_packet,
            },
        ];

        for TestInput { src1, src2, src3, src4, src5, dst1, dst2, make_tcp, handle_inbound_packet, handle_outbound_packet } in test_inputs {
            let ctk = Conntrack::new_with_capacity_and_ttl(3, LRU_CACHE_TCP_TTL, LRU_CACHE_UDP_TTL, LRU_CACHE_ICMP_TTL);

            assert_eq!(handle_inbound_packet(&ctk, &make_tcp(dst1, src1, TcpFlags::SYN), None), ConnectionState::New);
            assert_eq!(ctk.tcp.lock().len(), 1);
            assert_eq!(handle_inbound_packet(&ctk, &make_tcp(dst1, src2, TcpFlags::SYN), None), ConnectionState::New);
            assert_eq!(ctk.tcp.lock().len(), 2);
            assert_eq!(handle_inbound_packet(&ctk, &make_tcp(dst1, src3, TcpFlags::SYN), None), ConnectionState::New);
            assert_eq!(ctk.tcp.lock().len(), 3);
            assert_eq!(handle_inbound_packet(&ctk, &make_tcp(dst1, src4, TcpFlags::SYN), None), ConnectionState::New);
            assert_eq!(ctk.tcp.lock().len(), 3);

            assert_eq!(handle_outbound_packet(&ctk, &make_tcp(src1, dst1, TcpFlags::SYN), None), ConnectionState::New);
            assert_eq!(handle_outbound_packet(&ctk, &make_tcp(src2, dst1, TcpFlags::SYN), None), ConnectionState::New);
            assert_eq!(handle_outbound_packet(&ctk, &make_tcp(src1, dst1, TcpFlags::SYN), None), ConnectionState::New);
            assert_eq!(handle_outbound_packet(&ctk, &make_tcp(src3, dst1, TcpFlags::SYN), None), ConnectionState::New);
            assert_eq!(handle_outbound_packet(&ctk, &make_tcp(src1, dst1, TcpFlags::SYN), None), ConnectionState::New);
            assert_eq!(handle_outbound_packet(&ctk, &make_tcp(src4, dst1, TcpFlags::SYN), None), ConnectionState::New);
            assert_eq!(handle_outbound_packet(&ctk, &make_tcp(src1, dst1, TcpFlags::SYN), None), ConnectionState::New);
            assert_eq!(ctk.tcp.lock().len(), 3);

            // After this bunch of outbound connection src1 -> dst1, src4 -> dst1, src3 -> dst1 should be left
            assert_eq!(handle_inbound_packet(&ctk, &make_tcp(dst1, src4, TcpFlags::SYN | TcpFlags::ACK), None), ConnectionState::Established);
            assert_eq!(handle_inbound_packet(&ctk, &make_tcp(dst1, src3, TcpFlags::SYN | TcpFlags::ACK), None), ConnectionState::Established);
            assert_eq!(handle_inbound_packet(&ctk, &make_tcp(dst1, src1, TcpFlags::SYN | TcpFlags::ACK), None), ConnectionState::Established);
            assert_eq!(ctk.tcp.lock().len(), 3);

            // This should be already out of the cache
            assert_eq!(handle_inbound_packet(&ctk, &make_tcp(dst1, src2, TcpFlags::SYN), None), ConnectionState::New);
            assert_eq!(handle_inbound_packet(&ctk, &make_tcp(dst1, src5, TcpFlags::SYN), None), ConnectionState::New);
            assert_eq!(handle_inbound_packet(&ctk, &make_tcp(dst2, src1, TcpFlags::SYN), None), ConnectionState::New);
            assert_eq!(handle_inbound_packet(&ctk, &make_tcp(src1, dst1, TcpFlags::SYN), None), ConnectionState::New);
            assert_eq!(ctk.tcp.lock().len(), 3);
        }
    }

    #[test]
    fn conntrack_establishing_outbound_icmp_connection() {
        let conntrack = Conntrack::new();

        let src = Ipv4Addr::new(127, 0, 0, 1);
        let dst = Ipv4Addr::new(8, 8, 8, 8);

        let outbound_icmp_req_packet =
            make_icmp4(src, dst, IcmpTypes::EchoRequest, IcmpCodes::NoCode);
        let inbound_icmp_reply_packet =
            make_icmp4(dst, src, IcmpTypes::EchoReply, IcmpCodes::NoCode);
        let inbound_icmp_req_packet: Vec<u8> =
            make_icmp4(dst, src, IcmpTypes::EchoRequest, IcmpCodes::NoCode);

        let conn_state = conntrack
            .track_outbound_ip_packet::<Ipv4Packet>(None, &outbound_icmp_req_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::New);
        assert_eq!(conntrack.icmp.lock().len(), 1);

        // Inbound request packet should create another connection
        let conn_state = conntrack
            .track_outbound_ip_packet::<Ipv4Packet>(None, &inbound_icmp_req_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::New);

        // So we should have 2 connections in cache now
        assert_eq!(conntrack.icmp.lock().len(), 2);

        // It should be established for the first reply
        let conn_state = conntrack
            .track_inbound_ip_packet::<Ipv4Packet>(None, &inbound_icmp_reply_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::Established);
        assert_eq!(conntrack.icmp.lock().len(), 2);

        // After the first reply is processed, the ICMP connection should still be established
        let conn_state = conntrack
            .track_inbound_ip_packet::<Ipv4Packet>(None, &inbound_icmp_reply_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::Established);
        assert_eq!(conntrack.icmp.lock().len(), 2);
    }

    #[test]
    fn conntrack_icmp_reply_without_entry_is_invalid_inbound() {
        let local_addr = Ipv4Addr::new(127, 0, 0, 1);
        let remote_addr = Ipv4Addr::new(8, 8, 8, 8);

        let icmp_type_pairs = [
            (IcmpTypes::EchoRequest, IcmpTypes::EchoReply),
            (IcmpTypes::Timestamp, IcmpTypes::TimestampReply),
            (IcmpTypes::AddressMaskRequest, IcmpTypes::AddressMaskReply),
            (IcmpTypes::InformationRequest, IcmpTypes::InformationReply),
        ];

        for (req_type, reply_type) in icmp_type_pairs {
            let conntrack = Conntrack::new();

            let reply_icmp_req_packet =
                make_icmp4(remote_addr, local_addr, reply_type, IcmpCodes::NoCode);
            let request_icmp_resp_packet =
                make_icmp4(local_addr, remote_addr, req_type, IcmpCodes::NoCode);

            // Reply without conntrack entry should be invalid
            let conn_state = conntrack
                .track_inbound_ip_packet::<Ipv4Packet>(None, &reply_icmp_req_packet)
                .expect("Unexpected conntrack error");
            assert_eq!(conn_state, ConnectionState::Invalid);
            // It's a rare case when conntrack just doesn't add an entry
            assert_eq!(conntrack.icmp.lock().len(), 0);

            // The connection for outbound request should be new
            let conn_state = conntrack
                .track_outbound_ip_packet::<Ipv4Packet>(None, &request_icmp_resp_packet)
                .expect("Unexpected conntrack error");
            assert_eq!(conn_state, ConnectionState::New);
            assert_eq!(conntrack.icmp.lock().len(), 1);

            // Reply has corresponding conntrack entry so it should get ESTABLISHED state
            let conn_state = conntrack
                .track_inbound_ip_packet::<Ipv4Packet>(None, &reply_icmp_req_packet)
                .expect("Unexpected conntrack error");
            assert_eq!(conn_state, ConnectionState::Established);
            assert_eq!(conntrack.icmp.lock().len(), 1);

            // Next reply should also get ESTABLISHED state
            let conn_state = conntrack
                .track_inbound_ip_packet::<Ipv4Packet>(None, &reply_icmp_req_packet)
                .expect("Unexpected conntrack error");
            assert_eq!(conn_state, ConnectionState::Established);
            assert_eq!(conntrack.icmp.lock().len(), 1);
        }
    }

    #[test]
    fn conntrack_icmp_reply_without_entry_is_invalid_outbound() {
        let local_addr = Ipv4Addr::new(127, 0, 0, 1);
        let remote_addr = Ipv4Addr::new(8, 8, 8, 8);

        let icmp_type_pairs = [
            (IcmpTypes::EchoRequest, IcmpTypes::EchoReply),
            (IcmpTypes::Timestamp, IcmpTypes::TimestampReply),
            (IcmpTypes::AddressMaskRequest, IcmpTypes::AddressMaskReply),
            (IcmpTypes::InformationRequest, IcmpTypes::InformationReply),
        ];

        for (req_type, reply_type) in icmp_type_pairs {
            let conntrack = Conntrack::new();

            let reply_icmp_req_packet =
                make_icmp4(local_addr, remote_addr, reply_type, IcmpCodes::NoCode);
            let request_icmp_resp_packet =
                make_icmp4(remote_addr, local_addr, req_type, IcmpCodes::NoCode);

            // Reply without conntrack entry should be invalid
            let conn_state = conntrack
                .track_outbound_ip_packet::<Ipv4Packet>(None, &reply_icmp_req_packet)
                .expect("Unexpected conntrack error");
            assert_eq!(conn_state, ConnectionState::Invalid);
            assert_eq!(conntrack.icmp.lock().len(), 0);

            // The connection for inbound request should be new
            let conn_state = conntrack
                .track_inbound_ip_packet::<Ipv4Packet>(None, &request_icmp_resp_packet)
                .expect("Unexpected conntrack error");
            assert_eq!(conn_state, ConnectionState::New);
            assert_eq!(conntrack.icmp.lock().len(), 1);

            // Next reply has corresponding conntrack entry so it should get ESTABLISHED state
            let conn_state = conntrack
                .track_outbound_ip_packet::<Ipv4Packet>(None, &reply_icmp_req_packet)
                .expect("Unexpected conntrack error");
            assert_eq!(conn_state, ConnectionState::Established);
            assert_eq!(conntrack.icmp.lock().len(), 1);
        }
    }

    #[test]
    fn conntrack_icmpv6_reply_without_entry_is_invalid_inbound() {
        let local_addr = Ipv6Addr::new(0, 0, 0, 0, 0, 0, 0, 1);
        let remote_addr = Ipv6Addr::new(2001, 4860, 4860, 0, 0, 0, 0, 8888);

        let conntrack = Conntrack::new();

        let reply_icmp_req_packet = make_icmp6_echo_reply(remote_addr, local_addr, 1, 0);
        let request_icmp_resp_packet = make_icmp6_echo_request(local_addr, remote_addr, 1, 0);

        // Reply without conntrack entry should be invalid
        let conn_state = conntrack
            .track_inbound_ip_packet::<Ipv6Packet>(None, &reply_icmp_req_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::Invalid);
        // It's a rare case when conntrack just doesn't add an entry
        assert_eq!(conntrack.icmp.lock().len(), 0);

        // The connection for outbound request should be new
        let conn_state = conntrack
            .track_outbound_ip_packet::<Ipv6Packet>(None, &request_icmp_resp_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::New);
        assert_eq!(conntrack.icmp.lock().len(), 1);

        // Reply has corresponding conntrack entry so it should get ESTABLISHED state
        let conn_state = conntrack
            .track_inbound_ip_packet::<Ipv6Packet>(None, &reply_icmp_req_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::Established);
        assert_eq!(conntrack.icmp.lock().len(), 1);

        // Next reply should also get ESTABLISHED state
        let conn_state = conntrack
            .track_inbound_ip_packet::<Ipv6Packet>(None, &reply_icmp_req_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::Established);
        assert_eq!(conntrack.icmp.lock().len(), 1);
    }

    #[test]
    fn conntrack_icmpv6_reply_without_entry_is_invalid_outbound() {
        let local_addr = Ipv6Addr::new(0, 0, 0, 0, 0, 0, 0, 1);
        let remote_addr = Ipv6Addr::new(2001, 4860, 4860, 0, 0, 0, 0, 8888);

        let conntrack = Conntrack::new();

        let reply_icmp_req_packet = make_icmp6_echo_reply(local_addr, remote_addr, 1, 0);
        let request_icmp_resp_packet = make_icmp6_echo_request(remote_addr, local_addr, 1, 0);

        // Reply without conntrack entry should be invalid
        let conn_state = conntrack
            .track_outbound_ip_packet::<Ipv6Packet>(None, &reply_icmp_req_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::Invalid);
        assert_eq!(conntrack.icmp.lock().len(), 0);

        // The connection for inbound request should be new
        let conn_state = conntrack
            .track_inbound_ip_packet::<Ipv6Packet>(None, &request_icmp_resp_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::New);
        assert_eq!(conntrack.icmp.lock().len(), 1);

        // Next reply has corresponding conntrack entry so it should get ESTABLISHED state
        let conn_state = conntrack
            .track_outbound_ip_packet::<Ipv6Packet>(None, &reply_icmp_req_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::Established);
        assert_eq!(conntrack.icmp.lock().len(), 1);
    }

    #[test]
    fn conntrack_icmp_cache_error_response() {
        let conntrack = Conntrack::new();
        let src = Ipv4Addr::new(127, 0, 0, 1);
        let dst = Ipv4Addr::new(8, 8, 8, 8);

        let actual_request = make_icmp4_echo_request(src, dst, 1, 1);
        let other_request = make_icmp4_echo_request(src, dst, 2, 3);

        let mut actual_response_body = vec![0; 4];
        actual_response_body.extend_from_slice(&actual_request);
        let actual_response = make_icmp4_with_body(
            dst,
            src,
            IcmpTypes::DestinationUnreachable,
            IcmpCodes::NoCode,
            &actual_response_body,
            true,
        );

        let mut other_response_body = vec![0; 4];
        other_response_body.extend_from_slice(&other_request);
        let other_response = make_icmp4_with_body(
            dst,
            src,
            IcmpTypes::DestinationUnreachable,
            IcmpCodes::NoCode,
            &other_response_body,
            true,
        );

        assert_eq!(
            conntrack.track_outbound_ip_packet::<Ipv4Packet>(None, &actual_request),
            Ok(ConnectionState::New)
        );
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv4Packet>(None, &other_response),
            Ok(ConnectionState::Invalid)
        );
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv4Packet>(None, &actual_response),
            Ok(ConnectionState::Related)
        );
    }

    #[test]
    fn conntrack_icmpv6_cache_error_response() {
        let conntrack = Conntrack::new();
        let src = Ipv6Addr::new(0, 0, 0, 0, 0, 0, 0, 1);
        let dst = Ipv6Addr::new(2001, 4860, 4860, 0, 0, 0, 0, 8888);

        let actual_request = make_icmp6_echo_request(src, dst, 1, 1);
        let other_request = make_icmp6_echo_request(src, dst, 2, 3);

        let mut actual_response_body = vec![0; 4];
        actual_response_body.extend_from_slice(&actual_request);
        let actual_response = make_icmp6_with_body(
            dst,
            src,
            Icmpv6Types::DestinationUnreachable,
            Icmpv6Codes::NoCode,
            &actual_response_body,
            true,
        );

        let mut other_response_body = vec![0; 4];
        other_response_body.extend_from_slice(&other_request);
        let other_response = make_icmp6_with_body(
            dst,
            src,
            Icmpv6Types::DestinationUnreachable,
            Icmpv6Codes::NoCode,
            &other_response_body,
            true,
        );

        assert_eq!(
            conntrack.track_outbound_ip_packet::<Ipv6Packet>(None, &actual_request),
            Ok(ConnectionState::New)
        );
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv6Packet>(None, &other_response),
            Ok(ConnectionState::Invalid)
        );
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv6Packet>(None, &actual_response),
            Ok(ConnectionState::Related)
        );
    }

    #[test]
    fn removing_udp_entry_on_matching_icmp_error() {
        let conntrack = Conntrack::new();

        let udp_src = "127.0.0.1:1111";
        let udp_dst = "8.8.8.8:8888";

        let icmp_src = Ipv4Addr::new(127, 0, 0, 1);
        let icmp_dst = Ipv4Addr::new(8, 8, 8, 8);

        let outbound_udp_packet = make_udp(udp_dst, udp_src);
        let inbound_udp_packet = make_udp(udp_src, udp_dst);

        let mut data = vec![1, 0, 0, 1];
        data.extend_from_slice(&outbound_udp_packet);
        let inbound_icmp_resp_packet = make_icmp4_with_body(
            icmp_src,
            icmp_dst,
            IcmpTypes::DestinationUnreachable.into(),
            IcmpCodes::NoCode,
            &data,
            true,
        );

        // Create non-matching ICMP error (different port)
        let other_udp_packet = make_udp("8.8.8.8:9999", "127.0.0.1:1111");
        let mut other_data = vec![1, 0, 0, 1];
        other_data.extend_from_slice(&other_udp_packet);
        let non_matching_icmp_error = make_icmp4_with_body(
            icmp_src,
            icmp_dst,
            IcmpTypes::DestinationUnreachable.into(),
            IcmpCodes::NoCode,
            &other_data,
            true,
        );

        // Initiate the outbound connection
        assert_eq!(
            conntrack.track_outbound_ip_packet::<Ipv4Packet>(None, &outbound_udp_packet),
            Ok(ConnectionState::New)
        );
        assert_eq!(conntrack.udp.lock().len(), 1);

        // First inbound packet should make it established
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv4Packet>(None, &inbound_udp_packet),
            Ok(ConnectionState::Established)
        );
        assert_eq!(conntrack.udp.lock().len(), 1);

        // Non-matching ICMP error should be invalid
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv4Packet>(None, &non_matching_icmp_error),
            Ok(ConnectionState::Invalid)
        );
        assert_eq!(conntrack.udp.lock().len(), 1);

        // ICMP error message should also be treated as a part of established connection
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv4Packet>(None, &inbound_icmp_resp_packet),
            Ok(ConnectionState::Related)
        );
        assert_eq!(conntrack.udp.lock().len(), 0);

        // But after ICMP error message the connection should be removed
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv4Packet>(None, &inbound_udp_packet),
            Ok(ConnectionState::New)
        );
        assert_eq!(conntrack.udp.lock().len(), 1);
    }

    #[test]
    fn removing_udp_entry_on_matching_icmp6_error() {
        let conntrack = Conntrack::new();

        let udp_src = "[::1]:1111";
        let udp_dst = "[2001:4860:4860::8888]:8888";

        let icmp_src = Ipv6Addr::new(0, 0, 0, 0, 0, 0, 0, 1);
        let icmp_dst = Ipv6Addr::new(2001, 4860, 4860, 0, 0, 0, 0, 8888);

        let outbound_udp_packet = make_udp6(udp_dst, udp_src);
        let inbound_udp_packet = make_udp6(udp_src, udp_dst);

        let mut data = vec![1, 0, 0, 1];
        data.extend_from_slice(&outbound_udp_packet);
        let inbound_icmp_resp_packet = make_icmp6_with_body(
            icmp_src,
            icmp_dst,
            Icmpv6Types::DestinationUnreachable,
            Icmpv6Codes::NoCode,
            &data,
            true,
        );

        // Create non-matching ICMP error (different port)
        let other_udp_packet = make_udp6("[2001:4860:4860::8888]:9999", "[::1]:1111");
        let mut other_data = vec![1, 0, 0, 1];
        other_data.extend_from_slice(&other_udp_packet);
        let non_matching_icmp_error = make_icmp6_with_body(
            icmp_src,
            icmp_dst,
            Icmpv6Types::DestinationUnreachable,
            Icmpv6Codes::NoCode,
            &other_data,
            true,
        );

        // Initiate the outbound connection
        assert_eq!(
            conntrack.track_outbound_ip_packet::<Ipv6Packet>(None, &outbound_udp_packet),
            Ok(ConnectionState::New)
        );
        assert_eq!(conntrack.udp.lock().len(), 1);

        // First inbound packet should make it established
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv6Packet>(None, &inbound_udp_packet),
            Ok(ConnectionState::Established)
        );
        assert_eq!(conntrack.udp.lock().len(), 1);

        // Non-matching ICMP error should be invalid
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv6Packet>(None, &non_matching_icmp_error),
            Ok(ConnectionState::Invalid)
        );
        assert_eq!(conntrack.udp.lock().len(), 1);

        // ICMP error message should also be treated as a part of established connection
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv6Packet>(None, &inbound_icmp_resp_packet),
            Ok(ConnectionState::Related)
        );
        assert_eq!(conntrack.udp.lock().len(), 0);

        // But after ICMP error message the connection should be removed
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv6Packet>(None, &inbound_udp_packet),
            Ok(ConnectionState::New)
        );
        assert_eq!(conntrack.udp.lock().len(), 1);
    }

    #[test]
    fn removing_tcp_entry_on_matching_icmp_error() {
        let conntrack = Conntrack::new();

        let tcp_src = "127.0.0.1:1111";
        let tcp_dst = "8.8.8.8:8888";

        let icmp_src = Ipv4Addr::new(8, 8, 8, 8);
        let icmp_dst = Ipv4Addr::new(127, 0, 0, 1);

        let outbound_tcp_packet = make_tcp(tcp_src, tcp_dst, TcpFlags::SYN);
        let inbound_tcp_packet = make_tcp(tcp_dst, tcp_src, TcpFlags::SYN | TcpFlags::ACK);

        let mut data = vec![1, 0, 0, 1];
        data.extend_from_slice(&outbound_tcp_packet);
        let inbound_icmp_resp_packet = make_icmp4_with_body(
            icmp_src,
            icmp_dst,
            IcmpTypes::DestinationUnreachable,
            IcmpCodes::NoCode,
            &data,
            true,
        );

        // Create non-matching ICMP error (different port)
        let other_tcp_packet = make_tcp("127.0.0.1:1111", "8.8.8.8:9999", TcpFlags::SYN);
        let mut other_data = vec![1, 0, 0, 1];
        other_data.extend_from_slice(&other_tcp_packet);
        let non_matching_icmp_error = make_icmp4_with_body(
            icmp_src,
            icmp_dst,
            IcmpTypes::DestinationUnreachable,
            IcmpCodes::NoCode,
            &other_data,
            true,
        );

        // Initiate the outbound connection
        assert_eq!(
            conntrack.track_outbound_ip_packet::<Ipv4Packet>(None, &outbound_tcp_packet),
            Ok(ConnectionState::New)
        );
        assert_eq!(conntrack.tcp.lock().len(), 1);

        // First inbound packet should make it established
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv4Packet>(None, &inbound_tcp_packet),
            Ok(ConnectionState::Established)
        );
        assert_eq!(conntrack.tcp.lock().len(), 1);

        // Non-matching ICMP error should be invalid
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv4Packet>(None, &non_matching_icmp_error),
            Ok(ConnectionState::Invalid)
        );
        assert_eq!(conntrack.tcp.lock().len(), 1);

        // ICMP error message should also be treated as a part of established connection
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv4Packet>(None, &inbound_icmp_resp_packet),
            Ok(ConnectionState::Related)
        );
        assert_eq!(conntrack.tcp.lock().len(), 0);
    }

    #[test]
    fn removing_tcp_entry_on_matching_icmp6_error() {
        let conntrack = Conntrack::new();

        let tcp_src = "[::1]:1111";
        let tcp_dst = "[2001:4860:4860::8888]:8888";

        let icmp_src = Ipv6Addr::new(2001, 4860, 4860, 0, 0, 0, 0, 8888);
        let icmp_dst = Ipv6Addr::new(0, 0, 0, 0, 0, 0, 0, 1);

        let outbound_tcp_packet = make_tcp6(tcp_src, tcp_dst, TcpFlags::SYN);
        let inbound_tcp_packet = make_tcp6(tcp_dst, tcp_src, TcpFlags::SYN | TcpFlags::ACK);

        let mut data = vec![1, 0, 0, 1];
        data.extend_from_slice(&outbound_tcp_packet);
        let inbound_icmp_resp_packet = make_icmp6_with_body(
            icmp_src,
            icmp_dst,
            Icmpv6Types::DestinationUnreachable,
            Icmpv6Codes::NoCode,
            &data,
            true,
        );

        // Create non-matching ICMP error (different port)
        let other_tcp_packet =
            make_tcp6("[::1]:1111", "[2001:4860:4860::8888]:9999", TcpFlags::SYN);
        let mut other_data = vec![1, 0, 0, 1];
        other_data.extend_from_slice(&other_tcp_packet);
        let non_matching_icmp_error = make_icmp6_with_body(
            icmp_src,
            icmp_dst,
            Icmpv6Types::DestinationUnreachable,
            Icmpv6Codes::NoCode,
            &other_data,
            true,
        );

        // Initiate the outbound connection
        assert_eq!(
            conntrack.track_outbound_ip_packet::<Ipv6Packet>(None, &outbound_tcp_packet),
            Ok(ConnectionState::New)
        );
        assert_eq!(conntrack.tcp.lock().len(), 1);

        // First inbound packet should make it established
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv6Packet>(None, &inbound_tcp_packet),
            Ok(ConnectionState::Established)
        );
        assert_eq!(conntrack.tcp.lock().len(), 1);

        // Non-matching ICMP error should be invalid
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv6Packet>(None, &non_matching_icmp_error),
            Ok(ConnectionState::Invalid)
        );
        assert_eq!(conntrack.tcp.lock().len(), 1);

        // ICMP error message should also be treated as a part of established connection
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv6Packet>(None, &inbound_icmp_resp_packet),
            Ok(ConnectionState::Related)
        );
        assert_eq!(conntrack.tcp.lock().len(), 0);
    }

    #[test]
    fn removing_tcp_entry_on_rst() {
        let conntrack = Conntrack::new();

        let src = "127.0.0.1:1111";
        let dst = "8.8.8.8:8888";

        let outbound_syn_packet = make_tcp(src, dst, TcpFlags::SYN);
        let inbound_syn_ack_packet = make_tcp(dst, src, TcpFlags::SYN | TcpFlags::ACK);
        let inbound_rst_packet = make_tcp(dst, src, TcpFlags::RST);

        // Just normal connection establishment
        let conn_state = conntrack
            .track_outbound_ip_packet::<Ipv4Packet>(None, &outbound_syn_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::New);
        let conn_state = conntrack
            .track_inbound_ip_packet::<Ipv4Packet>(None, &inbound_syn_ack_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::Established);
        assert_eq!(conntrack.tcp.lock().len(), 1);

        // Inbound RST packet - connection should be closed immediately
        let conn_state = conntrack
            .track_inbound_ip_packet::<Ipv4Packet>(None, &inbound_rst_packet)
            .expect("Unexpected conntrack error");
        // It should remove the state in which connection was when RST was received
        assert_eq!(conn_state, ConnectionState::Established);

        // After that conntrack entry should be removed
        assert_eq!(conntrack.tcp.lock().len(), 0);
        let conn_state = conntrack
            .track_inbound_ip_packet::<Ipv4Packet>(None, &inbound_syn_ack_packet)
            .expect("Unexpected conntrack error");
        assert_eq!(conn_state, ConnectionState::New);
    }

    #[test]
    fn firewall_tcp_connection_timeout_is_not_updated_after_fin() {
        struct TestInput {
            us: &'static str,
            them: &'static str,
            make_tcp: fn(&str, &str, u8) -> Vec<u8>,
            handle_inbound_packet: fn(&Conntrack, &[u8], Option<&[u8]>) -> ConnectionState,
            handle_outbound_packet: fn(&Conntrack, &[u8], Option<&[u8]>) -> ConnectionState,
        }

        let test_inputs = vec![
            TestInput {
                us: "127.0.0.1:1111",
                them: "8.8.8.8:8888",
                make_tcp: make_tcp,
                handle_inbound_packet: handle_inbound_ipv4_packet,
                handle_outbound_packet: handle_outbound_ipv4_packet,
            },
            TestInput {
                us: "[::1]:1111",
                them: "[2001:4860:4860::8888]:8888",
                make_tcp: make_tcp6,
                handle_inbound_packet: handle_inbound_ipv6_packet,
                handle_outbound_packet: handle_outbound_ipv6_packet,
            },
        ];
        for TestInput {
            us,
            them,
            make_tcp,
            handle_inbound_packet,
            handle_outbound_packet,
        } in test_inputs
        {
            let ttl = Duration::from_millis(20);
            let conntrack = Conntrack::new_with_capacity_and_ttl(3, ttl, ttl, ttl);

            let outgoing_init_packet = make_tcp(us, them, TcpFlags::SYN);
            assert_eq!(
                handle_outbound_packet(&conntrack, &outgoing_init_packet, None),
                ConnectionState::New
            );

            assert_eq!(
                handle_inbound_packet(
                    &conntrack,
                    &make_tcp(them, us, TcpFlags::SYN | TcpFlags::ACK),
                    None
                ),
                ConnectionState::Established
            );
            assert_eq!(
                handle_inbound_packet(&conntrack, &make_tcp(them, us, TcpFlags::FIN), None),
                ConnectionState::Established
            );

            assert_eq!(
                handle_outbound_packet(&conntrack, &make_tcp(us, them, TcpFlags::FIN), None),
                ConnectionState::Closed
            );

            // process inbound packet (should not update ttl, because not ACK, but entry should still exist)
            advance_time(ttl / 2);
            assert_eq!(
                handle_inbound_packet(&conntrack, &make_tcp(them, us, TcpFlags::PSH), None),
                ConnectionState::Closed
            );

            // process inbound packet (should not update ttl, because not ACK, and entry should be removed after timeout, so this should be a new connection)
            advance_time(ttl / 2 + Duration::from_millis(1));
            assert_eq!(
                handle_inbound_packet(&conntrack, &make_tcp(them, us, TcpFlags::PSH), None),
                ConnectionState::New
            );
        }
    }

    #[test]
    fn conntrack_icmp_cache_general() {
        let conntrack = Conntrack::new_with_capacity_and_ttl(
            2,
            LRU_CACHE_TCP_TTL,
            LRU_CACHE_UDP_TTL,
            LRU_CACHE_ICMP_TTL,
        );

        let src1 = Ipv4Addr::new(8, 8, 8, 8);
        let src2 = Ipv4Addr::new(8, 8, 4, 4);
        let src3 = Ipv4Addr::new(4, 4, 4, 4);
        let dst = Ipv4Addr::new(127, 0, 0, 1);

        let request1 = make_icmp4_echo_request(dst, src1, 1, 1);
        let request2 = make_icmp4_echo_request(dst, src2, 1, 1);
        let request3 = make_icmp4_echo_request(dst, src3, 1, 1);
        let reply1 = make_icmp4_echo_reply(src1, dst, 1, 1);
        let reply2 = make_icmp4_echo_reply(src2, dst, 1, 1);
        let reply3 = make_icmp4_echo_reply(src3, dst, 1, 1);

        // Add src1, src2 and src3 to the cache, with cache limit of 2 (src1 gets evicted)
        assert_eq!(conntrack.icmp.lock().len(), 0);
        assert_eq!(
            conntrack.track_outbound_ip_packet::<Ipv4Packet>(None, &request1),
            Ok(ConnectionState::New)
        );
        assert_eq!(conntrack.icmp.lock().len(), 1);
        assert_eq!(
            conntrack.track_outbound_ip_packet::<Ipv4Packet>(None, &request2),
            Ok(ConnectionState::New)
        );
        assert_eq!(conntrack.icmp.lock().len(), 2);
        assert_eq!(
            conntrack.track_outbound_ip_packet::<Ipv4Packet>(None, &request3),
            Ok(ConnectionState::New)
        );
        assert_eq!(conntrack.icmp.lock().len(), 2); // src1 evicted due to LRU

        // Only replies for src2 and src3 should find their connections in the cache
        // reply1 should be Invalid since src1 was evicted
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv4Packet>(None, &reply1),
            Ok(ConnectionState::Invalid)
        );
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv4Packet>(None, &reply2),
            Ok(ConnectionState::Established)
        );
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv4Packet>(None, &reply3),
            Ok(ConnectionState::Established)
        );

        // src2 and src3 should still be established, src1 remains invalid
        assert_eq!(conntrack.icmp.lock().len(), 2);
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv4Packet>(None, &reply1),
            Ok(ConnectionState::Invalid)
        );
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv4Packet>(None, &reply2),
            Ok(ConnectionState::Established)
        );
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv4Packet>(None, &reply3),
            Ok(ConnectionState::Established)
        );
        assert_eq!(conntrack.icmp.lock().len(), 2);
    }

    #[test]
    fn conntrack_icmpv6_cache_general() {
        let conntrack = Conntrack::new_with_capacity_and_ttl(
            2,
            LRU_CACHE_TCP_TTL,
            LRU_CACHE_UDP_TTL,
            LRU_CACHE_ICMP_TTL,
        );

        let src1 = Ipv6Addr::new(2001, 4860, 4860, 0, 0, 0, 0, 8888);
        let src2 = Ipv6Addr::new(2001, 4860, 4860, 0, 0, 0, 0, 8844);
        let src3 = Ipv6Addr::new(2001, 4860, 4860, 0, 0, 0, 0, 4444);
        let dst = Ipv6Addr::new(0, 0, 0, 0, 0, 0, 0, 1);

        let request1 = make_icmp6_echo_request(dst, src1, 1, 1);
        let request2 = make_icmp6_echo_request(dst, src2, 1, 1);
        let request3 = make_icmp6_echo_request(dst, src3, 1, 1);
        let reply1 = make_icmp6_echo_reply(src1, dst, 1, 1);
        let reply2 = make_icmp6_echo_reply(src2, dst, 1, 1);
        let reply3 = make_icmp6_echo_reply(src3, dst, 1, 1);

        // Add src1, src2 and src3 to the cache, with cache limit of 2 (src1 gets evicted)
        assert_eq!(conntrack.icmp.lock().len(), 0);
        assert_eq!(
            conntrack.track_outbound_ip_packet::<Ipv6Packet>(None, &request1),
            Ok(ConnectionState::New)
        );
        assert_eq!(conntrack.icmp.lock().len(), 1);
        assert_eq!(
            conntrack.track_outbound_ip_packet::<Ipv6Packet>(None, &request2),
            Ok(ConnectionState::New)
        );
        assert_eq!(conntrack.icmp.lock().len(), 2);
        assert_eq!(
            conntrack.track_outbound_ip_packet::<Ipv6Packet>(None, &request3),
            Ok(ConnectionState::New)
        );
        assert_eq!(conntrack.icmp.lock().len(), 2); // src1 evicted due to LRU

        // Only replies for src2 and src3 should find their connections in the cache
        // reply1 should be Invalid since src1 was evicted
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv6Packet>(None, &reply1),
            Ok(ConnectionState::Invalid)
        );
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv6Packet>(None, &reply2),
            Ok(ConnectionState::Established)
        );
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv6Packet>(None, &reply3),
            Ok(ConnectionState::Established)
        );

        // src2 and src3 should still be established, src1 remains invalid
        assert_eq!(conntrack.icmp.lock().len(), 2);
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv6Packet>(None, &reply1),
            Ok(ConnectionState::Invalid)
        );
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv6Packet>(None, &reply2),
            Ok(ConnectionState::Established)
        );
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv6Packet>(None, &reply3),
            Ok(ConnectionState::Established)
        );
        assert_eq!(conntrack.icmp.lock().len(), 2);
    }

    #[test]
    fn conntrack_icmp_cache_identifier_and_sequence() {
        let conntrack = Conntrack::new();

        let src = Ipv4Addr::new(8, 8, 8, 8);
        let dst = Ipv4Addr::new(127, 0, 0, 1);

        let request = make_icmp4_echo_request(dst, src, 1, 1);
        let reply_same_id_same_seq = make_icmp4_echo_reply(src, dst, 1, 1);
        let reply_same_id_diff_seq = make_icmp4_echo_reply(src, dst, 1, 2);
        let reply_diff_id = make_icmp4_echo_reply(src, dst, 2, 1);

        // Send request
        assert_eq!(conntrack.icmp.lock().len(), 0);
        assert_eq!(
            conntrack.track_outbound_ip_packet::<Ipv4Packet>(None, &request),
            Ok(ConnectionState::New)
        );
        assert_eq!(conntrack.icmp.lock().len(), 1);

        // Reply with same identifier but different sequence should still be Established
        // (conntrack only checks identifier, not sequence)
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv4Packet>(None, &reply_same_id_diff_seq),
            Ok(ConnectionState::Established)
        );
        assert_eq!(conntrack.icmp.lock().len(), 1);

        // Reply with wrong identifier should be Invalid
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv4Packet>(None, &reply_diff_id),
            Ok(ConnectionState::Invalid)
        );
        assert_eq!(conntrack.icmp.lock().len(), 1);

        // Reply with correct identifier (any sequence) should be Established
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv4Packet>(None, &reply_same_id_same_seq),
            Ok(ConnectionState::Established)
        );
        assert_eq!(conntrack.icmp.lock().len(), 1);
    }

    #[test]
    fn conntrack_icmpv6_cache_identifier_and_sequence() {
        let conntrack = Conntrack::new();

        let src = Ipv6Addr::new(2001, 4860, 4860, 0, 0, 0, 0, 8888);
        let dst = Ipv6Addr::new(0, 0, 0, 0, 0, 0, 0, 1);

        let request = make_icmp6_echo_request(dst, src, 1, 1);
        let reply_same_id_same_seq = make_icmp6_echo_reply(src, dst, 1, 1);
        let reply_same_id_diff_seq = make_icmp6_echo_reply(src, dst, 1, 2);
        let reply_diff_id = make_icmp6_echo_reply(src, dst, 2, 1);

        // Send request
        assert_eq!(conntrack.icmp.lock().len(), 0);
        assert_eq!(
            conntrack.track_outbound_ip_packet::<Ipv6Packet>(None, &request),
            Ok(ConnectionState::New)
        );
        assert_eq!(conntrack.icmp.lock().len(), 1);

        // Reply with same identifier but different sequence should still be Established
        // (conntrack only checks identifier, not sequence)
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv6Packet>(None, &reply_same_id_diff_seq),
            Ok(ConnectionState::Established)
        );
        assert_eq!(conntrack.icmp.lock().len(), 1);

        // Reply with wrong identifier should be Invalid
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv6Packet>(None, &reply_diff_id),
            Ok(ConnectionState::Invalid)
        );
        assert_eq!(conntrack.icmp.lock().len(), 1);

        // Reply with correct identifier (any sequence) should be Established
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv6Packet>(None, &reply_same_id_same_seq),
            Ok(ConnectionState::Established)
        );
        assert_eq!(conntrack.icmp.lock().len(), 1);
    }

    #[test]
    fn conntrack_icmpv6_cache_key_by_type() {
        use crate::packet::{GenericIcmpType, IcmpPacketGroup};

        let src = Ipv6Addr::new(2001, 4860, 4860, 0, 0, 0, 0, 8888);
        let dst = Ipv6Addr::new(0, 0, 0, 0, 0, 0, 0, 1);

        let test_cases = vec![
            (Icmpv6Types::EchoRequest, Some(Icmpv6Types::EchoReply)),
            (Icmpv6Types::EchoReply, None),
        ];

        for (request_type, matching_reply) in &test_cases {
            for (reply_type, _) in &test_cases {
                if request_type == reply_type {
                    continue;
                }

                // Create new conntrack instance for each test pair
                let conntrack = Conntrack::new();

                let outbound = make_icmp6_with_body(
                    dst,
                    src,
                    *request_type,
                    Icmpv6Codes::NoCode,
                    &[1, 0, 1, 0],
                    true,
                );
                let inbound = make_icmp6_with_body(
                    src,
                    dst,
                    *reply_type,
                    Icmpv6Codes::NoCode,
                    &[1, 0, 1, 0],
                    true,
                );

                let expected_outbound_state = match request_type.get_packet_group() {
                    Some(IcmpPacketGroup::Request) => ConnectionState::New,
                    Some(IcmpPacketGroup::Reply) => ConnectionState::Invalid,
                    Some(IcmpPacketGroup::Error) => {
                        panic!("Error packets should not be in test_cases")
                    }
                    None => panic!("Unknown ICMP type"),
                };

                assert_eq!(
                    conntrack.track_outbound_ip_packet::<Ipv6Packet>(None, &outbound),
                    Ok(expected_outbound_state)
                );

                let expected_state = if matches!(matching_reply, Some(rt) if rt == reply_type) {
                    ConnectionState::Established
                } else {
                    match reply_type.get_packet_group() {
                        Some(IcmpPacketGroup::Request) => ConnectionState::New,
                        Some(IcmpPacketGroup::Reply) => ConnectionState::Invalid,
                        Some(IcmpPacketGroup::Error) => {
                            panic!("Error packets should not be in test_cases")
                        }
                        None => panic!("Unknown ICMP type"),
                    }
                };

                assert_eq!(
                    conntrack.track_inbound_ip_packet::<Ipv6Packet>(None, &inbound),
                    Ok(expected_state)
                );
            }
        }
    }

    #[test]
    fn firewall_tcp_conns_reset() {
        let conntrack = Conntrack::new();

        // Outbound half opened connection
        assert_eq!(
            conntrack
                .track_outbound_ip_packet::<Ipv4Packet>(
                    None,
                    &make_tcp("127.0.0.1:12345", "104.104.104.104:54321", TcpFlags::SYN),
                )
                .expect("Unexpected error during packet tracking"),
            ConnectionState::New
        );
        // Outbound opened connection
        assert_eq!(
            conntrack
                .track_outbound_ip_packet::<Ipv4Packet>(
                    None,
                    &make_tcp("127.0.0.1:12345", "103.103.103.103:54321", TcpFlags::SYN),
                )
                .expect("Unexpected error during packet tracking"),
            ConnectionState::New
        );
        assert_eq!(
            conntrack
                .track_inbound_ip_packet::<Ipv4Packet>(
                    None,
                    &make_tcp(
                        "103.103.103.103:54321",
                        "127.0.0.1:12345",
                        TcpFlags::SYN | TcpFlags::ACK
                    ),
                )
                .expect("Unexpected error during packet tracking"),
            ConnectionState::Established
        );
        // Inbound connection
        assert_eq!(
            conntrack
                .track_inbound_ip_packet::<Ipv4Packet>(
                    None,
                    &make_tcp(
                        "100.100.100.100:12345",
                        &format!("127.0.0.1:{FILE_SEND_PORT}"),
                        TcpFlags::SYN
                    ),
                )
                .expect("Unexpected error during packet tracking"),
            ConnectionState::New
        );
        // Inbound with data
        assert_eq!(
            conntrack
                .track_inbound_ip_packet::<Ipv4Packet>(
                    None,
                    &make_tcp(
                        "102.102.102.102:12345",
                        &format!("127.0.0.1:{FILE_SEND_PORT}"),
                        TcpFlags::SYN
                    ),
                )
                .expect("Unexpected error during packet tracking"),
            ConnectionState::New
        );
        assert_eq!(
            conntrack
                .track_inbound_ip_packet::<Ipv4Packet>(
                    None,
                    &make_tcp(
                        "102.102.102.102:12345",
                        &format!("127.0.0.1:{FILE_SEND_PORT}"),
                        TcpFlags::PSH
                    ),
                )
                .expect("Unexpected error during packet tracking"),
            ConnectionState::New
        );

        // Outbound IPv6 opened connection
        assert_eq!(
            conntrack
                .track_outbound_ip_packet::<Ipv6Packet>(
                    None,
                    &make_tcp6("[::1]:12345", "[1234::2]:54321", TcpFlags::SYN),
                )
                .expect("Unexpected error during packet tracking"),
            ConnectionState::New
        );
        assert_eq!(
            conntrack
                .track_inbound_ip_packet::<Ipv6Packet>(
                    None,
                    &make_tcp6(
                        "[1234::2]:54321",
                        "[::1]:12345",
                        TcpFlags::SYN | TcpFlags::ACK
                    ),
                )
                .expect("Unexpected error during packet tracking"),
            ConnectionState::Established
        );
        // Inbound IPv6 connection
        assert_eq!(
            conntrack
                .track_inbound_ip_packet::<Ipv6Packet>(
                    None,
                    &make_tcp6(
                        "[4321::2]:54321",
                        &format!("[::1]:{FILE_SEND_PORT}"),
                        TcpFlags::SYN
                    ),
                )
                .expect("Unexpected error during packet tracking"),
            ConnectionState::New
        );

        // Let's establish a connection for a different assoc data
        let diff_assoc_data = b"different_assoc_data";
        assert_eq!(
            conntrack
                .track_outbound_ip_packet::<Ipv4Packet>(
                    Some(diff_assoc_data),
                    &make_tcp("127.0.0.1:12354", "104.104.104.104:54312", TcpFlags::SYN),
                )
                .expect("Unexpected error during packet tracking"),
            ConnectionState::New
        );
        assert_eq!(
            conntrack
                .track_inbound_ip_packet::<Ipv4Packet>(
                    Some(diff_assoc_data),
                    &make_tcp(
                        "104.104.104.104:54312",
                        "127.0.0.1:12354",
                        TcpFlags::SYN | TcpFlags::ACK
                    ),
                )
                .expect("Unexpected error during packet tracking"),
            ConnectionState::Established
        );

        #[derive(Default)]
        struct Sink {
            pkgs: Vec<Vec<u8>>,
        }

        impl io::Write for Sink {
            fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
                self.pkgs.push(buf.to_vec());
                Ok(buf.len())
            }

            fn flush(&mut self) -> io::Result<()> {
                Ok(())
            }
        }

        let mut sink = Sink::default();
        conntrack
            .reset_tcp_conns(None, |packet| {
                sink.write(packet).expect("Error on writing into sink");
            })
            .expect("Unexpected error on resetting TCP connections");

        // Connections are stored in LinkedHashMap which keeps insertion order
        assert_eq!(sink.pkgs.len(), 5);

        let ippkgs: Vec<_> = sink
            .pkgs
            .iter()
            .take(3)
            .map(|buf| Ipv4Packet::new(&buf).unwrap())
            .collect();

        let tcppkgs: Vec<_> = ippkgs
            .iter()
            .map(|ip| TcpPacket::new(ip.payload()).unwrap())
            .collect();

        // Check common conditions
        for ip in &ippkgs {
            assert_eq!(ip.get_version(), 4);
            assert_eq!(ip.get_header_length(), 5);
            assert_eq!(ip.get_next_level_protocol(), IpNextHeaderProtocols::Tcp);
        }

        for tcp in &tcppkgs {
            assert_eq!(tcp.get_flags(), TcpFlags::RST);
            assert_eq!(tcp.get_data_offset(), 5);
        }

        // Check specifics
        assert_eq!(ippkgs[0].get_source(), Ipv4Addr::new(103, 103, 103, 103));
        assert_eq!(ippkgs[0].get_destination(), Ipv4Addr::new(127, 0, 0, 1));

        assert_eq!(tcppkgs[0].get_source(), 54321);
        assert_eq!(tcppkgs[0].get_destination(), 12345);
        assert_eq!(tcppkgs[0].get_sequence(), 1);

        assert_eq!(ippkgs[1].get_source(), Ipv4Addr::new(100, 100, 100, 100));
        assert_eq!(ippkgs[1].get_destination(), Ipv4Addr::new(127, 0, 0, 1));

        assert_eq!(tcppkgs[1].get_source(), 12345);
        assert_eq!(tcppkgs[1].get_destination(), FILE_SEND_PORT);
        assert_eq!(tcppkgs[1].get_sequence(), 1);

        assert_eq!(ippkgs[2].get_source(), Ipv4Addr::new(102, 102, 102, 102));
        assert_eq!(ippkgs[2].get_destination(), Ipv4Addr::new(127, 0, 0, 1));

        assert_eq!(tcppkgs[2].get_source(), 12345);
        assert_eq!(tcppkgs[2].get_destination(), FILE_SEND_PORT);
        assert_eq!(tcppkgs[2].get_sequence(), 12);

        let ip6pkgs: Vec<_> = sink
            .pkgs
            .iter()
            .skip(3)
            .take(2)
            .map(|buf| Ipv6Packet::new(&buf).unwrap())
            .collect();

        let tcp6pkgs: Vec<_> = ip6pkgs
            .iter()
            .map(|ip| TcpPacket::new(ip.payload()).unwrap())
            .collect();

        // Check common conditions
        for ip in &ip6pkgs {
            assert_eq!(ip.get_version(), 6);
            assert_eq!(ip.get_next_level_protocol(), IpNextHeaderProtocols::Tcp);
        }

        for tcp in &tcp6pkgs {
            assert_eq!(tcp.get_flags(), TcpFlags::RST);
            assert_eq!(tcp.get_data_offset(), 5);
        }

        // Check specifics
        assert_eq!(
            ip6pkgs[0].get_source(),
            "1234::2".parse::<Ipv6Addr>().unwrap()
        );
        assert_eq!(ip6pkgs[0].get_destination(), Ipv6Addr::LOCALHOST);

        assert_eq!(tcp6pkgs[0].get_source(), 54321);
        assert_eq!(tcp6pkgs[0].get_destination(), 12345);
        assert_eq!(tcp6pkgs[0].get_sequence(), 1);

        assert_eq!(
            ip6pkgs[1].get_source(),
            "4321::2".parse::<Ipv6Addr>().unwrap()
        );
        assert_eq!(ip6pkgs[1].get_destination(), Ipv6Addr::LOCALHOST);

        assert_eq!(tcp6pkgs[1].get_source(), 54321);
        assert_eq!(tcp6pkgs[1].get_destination(), FILE_SEND_PORT);
        assert_eq!(tcp6pkgs[1].get_sequence(), 1);

        // Only TCP entry for different assoc data should be still present
        assert_eq!(conntrack.get_connections_count().0, 1);
    }

    #[test]
    fn firewall_udp_conns_reset() {
        let conntrack = Conntrack::new();

        let test_udppkg = make_udp("127.0.0.2:12345", "127.0.0.3:54321");
        let test_udp6pkg = make_udp6("[::2]:12345", "[::3]:54321");

        // Outbound connections
        assert_eq!(
            conntrack
                .track_outbound_ip_packet::<Ipv4Packet>(None, &test_udppkg,)
                .expect("Conntrack tracking error"),
            ConnectionState::New
        );
        assert_eq!(
            conntrack
                .track_outbound_ip_packet::<Ipv6Packet>(None, &test_udp6pkg,)
                .expect("Conntrack tracking error"),
            ConnectionState::New
        );

        // Let's add an entry for a different assoc data
        let diff_assoc_data = b"different_assoc_data";
        assert_eq!(
            conntrack
                .track_outbound_ip_packet::<Ipv4Packet>(Some(diff_assoc_data), &test_udppkg,)
                .expect("Conntrack tracking error"),
            ConnectionState::New
        );

        #[derive(Default)]
        struct Sink {
            pkgs: Vec<Vec<u8>>,
        }

        impl io::Write for Sink {
            fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
                self.pkgs.push(buf.to_vec());
                Ok(buf.len())
            }

            fn flush(&mut self) -> io::Result<()> {
                Ok(())
            }
        }

        let mut sink = Sink::default();
        conntrack
            .reset_udp_conns(None, |packet| {
                sink.write(packet).expect("Error on sink write!");
            })
            .expect("Unexpected Conntrack error on resetting UDP conns");

        // Connections are stored in LinkedHashMap which keeps insertion order
        assert_eq!(sink.pkgs.len(), 2);

        let ip = Ipv4Packet::new(&sink.pkgs[0]).unwrap();
        assert_eq!(ip.get_version(), 4);
        assert_eq!(ip.get_header_length(), 5);
        assert_eq!(ip.get_next_level_protocol(), IpNextHeaderProtocols::Icmp);
        assert_eq!(ip.get_source(), Ipv4Addr::new(127, 0, 0, 3));
        assert_eq!(ip.get_destination(), Ipv4Addr::new(127, 0, 0, 2));

        let icmp = IcmpPacket::new(ip.payload()).unwrap();
        assert_eq!(icmp.get_icmp_type(), IcmpTypes::DestinationUnreachable);
        assert_eq!(
            icmp.get_icmp_code(),
            destination_unreachable::IcmpCodes::DestinationPortUnreachable
        );
        let icmp = DestinationUnreachablePacket::new(ip.payload()).unwrap();
        assert_eq!(icmp.payload(), &test_udppkg);

        let ip6 = Ipv6Packet::new(&sink.pkgs[1]).unwrap();
        assert_eq!(ip6.get_version(), 6);
        assert_eq!(ip6.get_next_level_protocol(), IpNextHeaderProtocols::Icmpv6);
        assert_eq!(ip6.get_source(), Ipv6Addr::new(0, 0, 0, 0, 0, 0, 0, 3));
        assert_eq!(ip6.get_destination(), Ipv6Addr::new(0, 0, 0, 0, 0, 0, 0, 2));

        let icmp6 = Icmpv6Packet::new(ip6.payload()).unwrap();
        assert_eq!(icmp6.get_icmpv6_type(), Icmpv6Types::DestinationUnreachable);
        assert_eq!(icmp6.get_icmpv6_code(), Icmpv6Code::new(4)); // DestinationPortUnreachable

        // Only entry for the different assoc data should stay
        assert_eq!(conntrack.get_connections_count().1, 1);
    }

    #[rustfmt::skip]
    #[test]
    fn firewall_pinhole_timeout() {
        let capacity = 3;
        let ttl = Duration::from_millis(100);

        let conntrack = Conntrack::new_with_capacity_and_ttl(capacity, ttl, ttl, ttl);

        let src = "127.0.0.1:1111";
        let dst = "8.8.8.8:8888";

            assert_eq!(conntrack.track_outbound_ip_packet::<Ipv4Packet>(None, &make_udp(src, dst)).expect("Conntrack tracking error"), ConnectionState::New);
            assert_eq!(conntrack.track_inbound_ip_packet::<Ipv4Packet>(None, &make_udp(dst, src)).expect("Conntrack tracking error"), ConnectionState::Established);
            advance_time(Duration::from_millis(200));
            assert_eq!(conntrack.track_inbound_ip_packet::<Ipv4Packet>(None, &make_udp(dst, src)).expect("Conntrack tracking error"), ConnectionState::New);
    }

    #[test]
    fn firewall_pinhole_timeout_extending() {
        let capacity = 3;
        let ttl = Duration::from_millis(20);

        let conntrack = Conntrack::new_with_capacity_and_ttl(capacity, ttl, ttl, ttl);

        let src = "127.0.0.1:1111";
        let dst = "8.8.8.8:8888";

        let conn_state = conntrack
            .track_outbound_ip_packet::<Ipv4Packet>(None, &make_udp(src, dst))
            .expect("Conntrack tracking error");
        assert_eq!(conn_state, ConnectionState::New);

        // Connection should move to established
        let conn_state = conntrack
            .track_inbound_ip_packet::<Ipv4Packet>(None, &make_udp(dst, src))
            .expect("Conntrack tracking error");
        assert_eq!(conn_state, ConnectionState::Established);

        // And flowing packets ...
        advance_time(Duration::from_millis(15));
        let conn_state = conntrack
            .track_inbound_ip_packet::<Ipv4Packet>(None, &make_udp(dst, src))
            .expect("Conntrack tracking error");
        assert_eq!(conn_state, ConnectionState::Established);

        // ... should keep it in established state
        advance_time(Duration::from_millis(15));
        let conn_state = conntrack
            .track_inbound_ip_packet::<Ipv4Packet>(None, &make_udp(dst, src))
            .expect("Conntrack tracking error");
        assert_eq!(conn_state, ConnectionState::Established);

        // After exceeding LRU TTL the connection entry should be removed
        advance_time(Duration::from_millis(30));
        let conn_state = conntrack
            .track_inbound_ip_packet::<Ipv4Packet>(None, &make_udp(dst, src))
            .expect("Conntrack tracking error");
        assert_eq!(conn_state, ConnectionState::New);
    }

    #[test]
    fn firewall_icmp_ddos_vulnerability() {
        let dst1 = "8.8.8.8:8888";
        let dst2 = Ipv4Addr::new(8, 8, 8, 8);
        let src1 = "127.0.0.1:2000";
        let src2 = Ipv4Addr::new(127, 0, 0, 1);

        let conntrack = Conntrack::new();

        let good_peer = [1, 2, 3, 4];
        let bad_peer = [5, 6, 7, 8];

        let tcp_syn_outbound = make_tcp(src1, dst1, TcpFlags::SYN);
        let tcp_synack_inbound = make_tcp(dst1, src1, TcpFlags::SYN | TcpFlags::ACK);

        let mut data = vec![1, 0, 0, 1];
        let tcp = make_tcp(src1, dst1, TcpFlags::ACK);
        data.extend_from_slice(&tcp);

        let icmp_tcp_error_inbound = make_icmp4_with_body(
            dst2,
            src2,
            IcmpTypes::DestinationUnreachable.into(),
            IcmpCodes::NoCode,
            &data,
            true,
        );

        // tcp & udp outbound: allowed
        assert_eq!(
            conntrack.track_outbound_ip_packet::<Ipv4Packet>(Some(&good_peer), &tcp_syn_outbound),
            Ok(ConnectionState::New)
        );

        // inject error package from bad peer
        assert_eq!(
            conntrack
                .track_inbound_ip_packet::<Ipv4Packet>(Some(&bad_peer), &icmp_tcp_error_inbound),
            Ok(ConnectionState::Invalid)
        );

        // expect that connection continues normally
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv4Packet>(Some(&good_peer), &tcp_synack_inbound),
            Ok(ConnectionState::Established)
        );
    }

    #[test]
    fn firewall_tcp_rst_attack_vulnerability() {
        let us = "127.0.0.1:1111";
        let them = "8.8.8.8:8888";

        let conntrack = Conntrack::new();

        let outgoing_init_packet = make_tcp(us, them, TcpFlags::SYN);
        let incoming_rst_packet = make_tcp(them, us, TcpFlags::RST);

        let good_peer = [1, 2, 3, 4];
        let bad_peer = [5, 6, 7, 8];

        assert_eq!(
            conntrack
                .track_outbound_ip_packet::<Ipv4Packet>(Some(&good_peer), &outgoing_init_packet),
            Ok(ConnectionState::New)
        );
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv4Packet>(Some(&bad_peer), &incoming_rst_packet),
            Ok(ConnectionState::New)
        );
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv4Packet>(
                Some(&good_peer),
                &make_tcp(them, us, TcpFlags::SYN | TcpFlags::ACK)
            ),
            Ok(ConnectionState::Established)
        );
        assert_eq!(
            conntrack.track_inbound_ip_packet::<Ipv4Packet>(
                Some(&good_peer),
                &make_tcp(them, us, TcpFlags::SYN)
            ),
            Ok(ConnectionState::Established)
        );
    }

    #[test]
    fn firewall_udp_spoffing_and_injection_vulnerability() {
        struct TestInput {
            src: &'static str,
            dst: &'static str,
            make_udp: fn(&str, &str) -> Vec<u8>,
            handle_inbound_packet: fn(&Conntrack, &[u8], Option<&[u8]>) -> ConnectionState,
            handle_outbound_packet: fn(&Conntrack, &[u8], Option<&[u8]>) -> ConnectionState,
        }

        let test_inputs = [
            TestInput {
                src: "127.0.0.1:1111",
                dst: "8.8.8.8:8888",
                make_udp: make_udp,
                handle_inbound_packet: handle_inbound_ipv4_packet,
                handle_outbound_packet: handle_outbound_ipv4_packet,
            },
            TestInput {
                src: "[::1]:1111",
                dst: "[2001:4860:4860::8888]:8888",
                make_udp: make_udp6,
                handle_inbound_packet: handle_inbound_ipv6_packet,
                handle_outbound_packet: handle_outbound_ipv6_packet,
            },
        ];

        for TestInput {
            src,
            dst,
            make_udp,
            handle_inbound_packet,
            handle_outbound_packet,
        } in &test_inputs
        {
            let conntrack = Conntrack::new();
            let outgoing_packet = make_udp(src, dst);
            let incoming_packet = make_udp(dst, src);

            let good_peer = [1, 2, 3, 4];
            let bad_peer = [5, 6, 7, 8];

            // Should Pass as it is inbound for trusted peer
            assert_eq!(
                handle_inbound_packet(&conntrack, &incoming_packet, Some(&good_peer)),
                ConnectionState::New
            );
            assert_eq!(
                handle_outbound_packet(&conntrack, &outgoing_packet, Some(&good_peer)),
                ConnectionState::Established
            );

            // This is another connection
            assert_eq!(
                handle_inbound_packet(&conntrack, &incoming_packet, Some(&bad_peer)),
                ConnectionState::New
            );

            // Good peer should still have established connection
            assert_eq!(
                handle_inbound_packet(&conntrack, &incoming_packet, Some(&good_peer)),
                ConnectionState::Established
            );
            assert_eq!(
                handle_outbound_packet(&conntrack, &outgoing_packet, Some(&good_peer)),
                ConnectionState::Established
            );
        }
    }
}
