use std::pin::Pin;
use std::{fmt::Debug, mem::ManuallyDrop, net::IpAddr, ptr::null_mut};
...
impl From<&NetworkFilterData> for LibfwNetworkFilter {
    fn from(value: &NetworkFilterData) -> Self {
        ...
        LibfwNetworkFilter {
            ...
            port_range_start: value.ports.0,
            port_range_end: value.ports.1,
        }
    }
}
...
impl From<&Filter> for (LibfwFilter, Option<PinnedAssocData>) {
    fn from(value: &Filter) -> (LibfwFilter, Option<PinnedAssocData>) {
...
impl From<&[Rule]> for FfiChainGuard {
    fn from(value: &[Rule]) -> Self {
        let (ffi_rules, _ffi_filters): (
            Vec<LibfwRule>,
            Vec<(PinnedFilters, Vec<Option<PinnedAssocData>>)>,
...
pub mod tests {
    use std::{mem::ManuallyDrop, net::Ipv4Addr, ptr::null_mut};
